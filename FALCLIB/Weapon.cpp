...
int GetWeaponScore (int w, uchar* dam, int mt, int range)
{
    ...
    return (WeaponDataTable[w].Strength * WeaponDataTable[w].FireRate * dam[(int)WeaponDataTable[w].DamageType] * GetWeaponHitChance(w,mt,range))/100;
}
...
int GetWeaponScore (int w, uchar* dam, int mt, int range, int wrange)
{
    ShiAssert(w>= 0 && w < NumWeaponTypes);
    if(w< 0 || w >= NumWeaponTypes) return 0;
    return (WeaponDataTable[w].Strength * WeaponDataTable[w].FireRate * dam[(int)WeaponDataTable[w].DamageType] * GetWeaponHitChance(w,mt,range, wrange))/100;
}
