...
Prof_extern_C void  Prof_dumpFile()
{
	...
	for (j=0; j < NUM_HEADER; ++j)
		if (pob->header[j])
			fprintf(fp,"%s,",pob->header[j]);
	...
	{
		char buf[256], *b = buf;
		Prof_Report_Record *r = &pob->record[i];

		for (in = 0; in < r->indent; in++)
			fprintf(fp," ");
...
#define MAX_LINE_LENGTH 100
	...
	for (i=0; i < NUM_TITLE; ++i)
	{
		if (pob->title[i])
		{
			...
			strcpy(line[linecount],pob->title[i]);
			linecount++;
		}
	}
#else
	int index = 0;
	// pob->title[0] holds info bout frametime, avg fps etc - shoud be there everytime
	// pob->title[1] (may) hold the speedstep-like timer warning thingie..
	...
	for (i = 0; i < pob->num_record; i++)
	{
		...
		if (r->indent > 0)
		{
			strcpy(line[linecount]," ");
			for (in = 1; in < r->indent; in++)
				strcat(line[linecount]," ");
			if (r->prefix)
				strcat(line[linecount],"+");
			else
				strcat(line[linecount]," ");
			strcat(line[linecount],r->name);
		}
		else
		{
			if (r->prefix)
...
static Prof_Report *allocate_buffer(int n)
{
   ...
   memset(pob, 0, sizeof(*pob));
   pob->num_record = n;
   pob->record = (Prof_Report_Record *) malloc(sizeof(*pob->record) * pob->num_record);
   memset(pob->record, 0, sizeof(*pob->record) * pob->num_record);
...
//Date			
//------------------------------------------------------------------------------
Prof_extern_C int Prof_get_cursor()
{
	return cursor;
}
