...
void TheaterList::SetCurrentTheater(TheaterDef *td)
{
    ...
    fp = CreateFile("curTheater.bin", GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
    if (fp == INVALID_HANDLE_VALUE)
        return;

    WriteFile(fp, td->m_name, sizeof(strlen(td->m_name)), &br, NULL);
    CloseHandle(fp);
#else
	HKEY	theKey;
...
#if 1 //RERUN. Why is this not part of logbook?
    HANDLE fp;
    DWORD br;

    fp = CreateFile("curTheater.bin", GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (fp == INVALID_HANDLE_VALUE)
		strcpy(TheaterName, "Korea");
	else
	    ReadFile(fp, TheaterName, sizeof(_MAX_PATH), &br, NULL);
