...
int FalconSendUnitData::Decode (VU_BYTE **buf, long *rem) {
	...
	if (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_NEED_UNIT_DATA) {
		...
		TheCampaign.Flags &= ~(ulong)CampStatusEnum::CAMP_NEED_UNIT_DATA;
		...
		if(gMainHandler){
			PostMessage(gMainHandler->GetAppWnd(),FM_GOT_CAMPAIGN_DATA, (ulong)CampStatusEnum::CAMP_NEED_UNIT_DATA,0);
		}
	}
	//return size;
	return init - *rem;
