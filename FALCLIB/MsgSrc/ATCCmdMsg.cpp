		...
		if(acBrain && atcBrain){
			...
			switch (dataBlock.type)
			{
					case TakePosition:
						...
						desAlt = atcBrain->GetAltitude(aircraft, AtcStatusEnum::lTakingPosition);
						acBrain->SetATCStatus(AtcStatusEnum::lTakingPosition);
					...
					case Hold:
						...
						acBrain->SetATCStatus(AtcStatusEnum::lHolding);
					...
					case ToFirstLeg:
							if(aircraft->OnGround())
								break;
							if(acBrain->ATCStatus() < AtcStatusEnum::lFirstLeg)
							{
								acBrain->SetTaxiPoint(GetFirstPt(dataBlock.rwindex));
								atcBrain->FindFinalPt(aircraft, dataBlock.rwindex, &finalX, &finalY);
								cosAngle = atcBrain->DetermineAngle(aircraft, dataBlock.rwindex, AtcStatusEnum::lFirstLeg);
								...
								if(status != AtcStatusEnum::lFirstLeg)
					...
					case ToBase:
							if(aircraft->OnGround())
								break;
							if(acBrain->ATCStatus() < AtcStatusEnum::lToBase)
							{
								atcBrain->FindFinalPt(aircraft, dataBlock.rwindex, &finalX, &finalY);
								atcBrain->FindBasePt(aircraft, dataBlock.rwindex, finalX, finalY, &baseX, &baseY);
					...
					case ToFinal:
							if(aircraft->OnGround())
								break;
							if(acBrain->ATCStatus() < AtcStatusEnum::lToFinal)
							{
								atcBrain->FindFinalPt(aircraft, dataBlock.rwindex, &finalX, &finalY);
					...
					case Landed:
					{
						...
						if (atcs < AtcStatusEnum::lCrashed){
					...
					case TaxiOff:
						...
						if(acBrain->ATCStatus() < AtcStatusEnum::lTaxiOff){
							acBrain->SetATCStatus(AtcStatusEnum::lTaxiOff);
						}
						break;
					case EmerToBase:
							if(aircraft->OnGround())
					...
					case Wait:
							...
							acBrain->ClearATCFlag(DigitalBrain::PermitTakeRunway);
							acBrain->ClearATCFlag(DigitalBrain::PermitTakeoff);
							acBrain->SetATCStatus(AtcStatusEnum::tWait);
							acBrain->SetWaitTimer(acBrain->CalcWaitTime(atcBrain));
							break;
								...
								break;
							{
								...
								if (divertBase){
									if(divertBase->GetType () == TYPE_AIRBASE){
										...
										rangediv = NM_TO_KM * ((float)sqrt(pow(diffx,2) + pow(diffy,2)) * FT_TO_NM);

										az = RTD * (float)atan2 ((double)divy - ytest, (double)divx - xtest);
										if (az < -0.6f)
											az += 360.0f;
								...
								acBrain->SetATCStatus(AtcStatusEnum::lIngressing);
								break;
							}
