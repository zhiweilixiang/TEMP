		...
		while(p){
				...
				(plane->GetTeam() == p->GetTeam()))
			{
				...
				if(fRange <= fRangeMax)		
				{
					...
					switch(missionType)
					{
						case MissionTypeEnum::AMIS_BARCAP:	// BARCAP missions to protect a target area
						case MissionTypeEnum::AMIS_BARCAP2:	// BARCAP missions to defend a border
						...
						// case AMIS_INTERCEPT:
						case MissionTypeEnum::AMIS_ONCALLCAS:	// On call CAS
						// case AMIS_PRPLANCAS:	// Pre planned CAS
						case MissionTypeEnum::AMIS_CAS:	// Immediate CAS
						// case AMIS_RECON:
						case MissionTypeEnum::AMIS_BDA:
						case MissionTypeEnum::AMIS_PATROL:
						case MissionTypeEnum::AMIS_RECONPATROL:	// Recon for enemy ground vehicles
						{
...
// 2002-03-07 MN of course only AWACS from our team - doh!
					if (cf->GetUnitMission() == MissionTypeEnum::AMIS_AWACS && cf->GetTeam() == plane->GetTeam())
					{
							awacs = (Flight) cf;
							break;
					}
...
//			SendCallToAWACS(plane, rcUNABLE);
			...
			if(plane->IsLocal())
				CheckDivertStatus (AWACSMsgEnum::DIVERT_REPLY_NO);
			//rcUNABLE
			break;
...
//			SendCallToAWACS(plane, rcCOPY);
			// KCK: Reply to any pending diverts
			if(plane->IsLocal())
				CheckDivertStatus (AWACSMsgEnum::DIVERT_REPLY_YES);
			//rcCOPY
			break;
...
//				SendCallToAWACS(plane, rcREQUESTDIVERT);
		...
		case OnStation:
			...
			//TJL 12/14/03 Begin the new Check-in/PseudoFAC code	
			{
...
#else
				if (flight->GetUnitCurrentRole() != ARO_GA &&
					((flight->GetUnitMission() != MissionTypeEnum::AMIS_ONCALLCAS)&&
					 (flight->GetUnitMission() != MissionTypeEnum::AMIS_PRPLANCAS)&&
					 (flight->GetUnitMission() != MissionTypeEnum::AMIS_CAS)&&
					 (flight->GetUnitMission() != MissionTypeEnum::AMIS_SAD)&&
					 (flight->GetUnitMission() != MissionTypeEnum::AMIS_INT)&&
					 (flight->GetUnitMission() != MissionTypeEnum::AMIS_BAI)))
				...
				if (MissionData[(uchar)flight->GetUnitMission()].flags & AMIS_EXPECT_DIVERT)
				{
					flight->SetUnitPriority(0);
