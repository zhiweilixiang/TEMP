...
int FalconSendObjData::Process(uchar autodisp)
{
	...
	if (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_NEED_OBJ_DELTAS)
	{
		CampaignJoinKeepAlive ();
		...
		TheCampaign.Flags &= ~(ulong)CampStatusEnum::CAMP_NEED_OBJ_DELTAS;
		...
		if(gMainHandler)
			PostMessage(gMainHandler->GetAppWnd(),FM_GOT_CAMPAIGN_DATA, (ulong)CampStatusEnum::CAMP_NEED_OBJ_DELTAS,0);
	}
	else
	{
