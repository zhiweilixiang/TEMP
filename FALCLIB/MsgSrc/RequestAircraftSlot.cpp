...
int UI_RequestAircraftSlot::AddFlightSlot(Flight flight)
{
	...
	if (!flight)
	{
		if (FalconLocalGame->GetGameType() == game_Dogfight)
		{
			...
			//			flight->SetFinal(1);
			flight->SetUnitMissionID((ushort)MissionTypeEnum::AMIS_SWEEP); //RERUN why is the mission type suddenly used as mission id?
		...
		if (FalconLocalGame->GetGameType() == game_Dogfight)
		{
			for (i=0; i<PILOTS_PER_FLIGHT && !retval; i++)
			{
				if (flight->plane_stats[i] == AIRCRAFT_NOT_ASSIGNED)
				{
					...
					{
						MonoPrint ("UI_RequestAircraftSlot::AddFlightSlot - What happened ????");
						flight->player_slots[i] = 255;
