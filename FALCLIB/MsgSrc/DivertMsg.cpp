...
static AWACSMsgEnum sLastReply = AWACSMsgEnum::DIVERT_NO_DIVERT;
...
{	
	...
	dataBlock.mission = MissionTypeEnum::AMIS_NONE;
}
...
int FalconDivertMessage::Process(uchar autodisp)
{
	...
	if (dataBlock.mission != MissionTypeEnum::AMIS_NONE)
	{
		...
		if ((!target || (target->IsUnit() && ((UnitClass*)target)->IsDead())) && dataBlock.mission != MissionTypeEnum::AMIS_NONE)
			return 0;
		...
		if (flight->IsLocal() && (dataBlock.flags & REQF_NEEDRESPONSE))
		{
			...
			if (e->IsUnit())
				((Unit)e)->SendUnitMessage(dataBlock.targetID,FalconUnitMessage::unitRequestMet,(uchar)dataBlock.mission,flight->GetTeam(),0);	
		}
		...
		if (flight->GetComponentLead() == FalconLocalSession->GetPlayerEntity() && flight == FalconLocalSession->GetPlayerFlight())
		{
			...
			sLastReply = AWACSMsgEnum::DIVERT_WAIT_FOR_REPLY;
		...
		PlayDivertRadioCalls(NULL,MissionTypeEnum::AMIS_NONE,flight,0, AWACSMsgEnum::DIVERT_DENIED);
...
int CheckDivertStatus (AWACSMsgEnum reply)
{
	if (!sNextRepost)
		return -1;
	else if (sNextRepost < vuxGameTime || reply != AWACSMsgEnum::DIVERT_NO_DIVERT)
	{
		...
		{
			sLastReply = AWACSMsgEnum::DIVERT_REPLY_YES;
			ApplyDivert (flight, &sLastDivert);
			return 1;
		}
		else if (reply == AWACSMsgEnum::DIVERT_REPLY_YES)
		{
			sLastReply = AWACSMsgEnum::DIVERT_REPLY_YES;
			//ApplyDivert (flight, &sLastDivert); this is only for AI flights
			return 1;
		}
		else if (reply == AWACSMsgEnum::DIVERT_REPLY_NO)
		{
			...
			sLastReply = AWACSMsgEnum::DIVERT_REPLY_NO;
			return 1;
		}
		else
		{
...
void ApplyDivert (Flight flight, FalconDivertMessage *fdm)
{
	...
	if (flight->IsLocal())
	{
		...
		else if (mis.mission == MissionTypeEnum::AMIS_CAS)
	...
	if (doUI && oldmission == MissionTypeEnum::AMIS_ALERT && fdm->dataBlock.mission == MissionTypeEnum::AMIS_INTERCEPT && flight->GetUnitSquadronID() == FalconLocalSession->GetPlayerSquadronID())
	{
...
void PlayDivertRadioCalls (CampEntity target, MissionTypeEnum mission, Flight flight, int broadcast, AWACSMsgEnum awacsreply)
{
	...
	if (mission == MissionTypeEnum::AMIS_NONE || !target)
	{
		if (awacsreply == AWACSMsgEnum::DIVERT_DENIED)
			SendCallFromAwacs(flight, rcNOTASKING, to);				// Divert denied
		else if (awacsreply == AWACSMsgEnum::DIVERT_CANCELED)
			SendCallFromAwacs(flight, rcENDDIVERTDIRECTIVE, to);		// Divert canceled
		else if (awacsreply == AWACSMsgEnum::DIVERT_SUCCEEDED)
	...
	else if (mission == MissionTypeEnum::AMIS_INTERCEPT)
	{
	...
	else if (mission == MissionTypeEnum::AMIS_CAS)
	{
	...
	else if (mission == MissionTypeEnum::AMIS_ALERT)
	{
