...
int FalconSendVC::Process(uchar autodisp)
{
	...
	if (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_NEED_VC)
	{
		...
		TheCampaign.Flags &= ~(ulong)CampStatusEnum::CAMP_NEED_VC;

		// Let the UI know we've received some data
		if(gMainHandler)
			PostMessage(gMainHandler->GetAppWnd(),FM_GOT_CAMPAIGN_DATA, (ulong)CampStatusEnum::CAMP_NEED_VC,0);
	}
