...
int FalconSendCampaign::Process(uchar autodisp)
{
	...
	if (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_NEED_PRELOAD){
		CampEnterCriticalSection();
		gCampDataVersion = gCurrentDataVersion;
		TheCampaign.Decode(&bufptr, &bufSize);
		TheCampaign.Flags &= ~(ulong)CampStatusEnum::CAMP_NEED_PRELOAD;
		TheCampaign.Flags |= (ulong)CampStatusEnum::CAMP_PRELOADED;
		CampLeaveCriticalSection();

		if(gMainHandler){
			PostMessage(gMainHandler->GetAppWnd(),FM_GOT_CAMPAIGN_DATA, (ulong)CampStatusEnum::CAMP_NEED_PRELOAD,0);
		}
		return 1;
	}
	else {
