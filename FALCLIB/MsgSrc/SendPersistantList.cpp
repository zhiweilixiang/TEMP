...
int FalconSendPersistantList::Process(uchar autodisp)
{
	...
	if (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_NEED_PERSIST)
	{
		CampaignJoinKeepAlive();
		...
		TheCampaign.Flags &= ~(ulong)CampStatusEnum::CAMP_NEED_PERSIST;
		if(gMainHandler)
			PostMessage(gMainHandler->GetAppWnd(),FM_GOT_CAMPAIGN_DATA, (ulong)CampStatusEnum::CAMP_NEED_PERSIST,0);
