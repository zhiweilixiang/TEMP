...
int FalconRequestCampaignData::Process(uchar autodisp)
	...
	if (dataBlock.dataNeeded & (ulong)CampStatusEnum::CAMP_GAME_FULL)
	{
	...
	if (dataBlock.dataNeeded & (ulong)CampStatusEnum::DF_MATCH_IN_PROGRESS)
	{
...
void SendRequestedData (void){	
	...
	for (FalconRequestCampaignData *request = gRequestQueue; request != NULL; request = gRequestQueue){
		...
		if ((requester != NULL) && TheCampaign.IsLoaded()){
			...
			if ((!(request->dataBlock.dataNeeded & (ulong)CampStatusEnum::CAMP_NEED_PRELOAD)) && (CheckNumberPlayers () < 0)){
				...
				msg->dataBlock.dataNeeded = (ulong)CampStatusEnum::CAMP_GAME_FULL;
				FalconSendMessage (msg,TRUE);
			}
			else if ((!(request->dataBlock.dataNeeded & (ulong)CampStatusEnum::CAMP_NEED_PRELOAD)) && (MatchPlayStarted())){
				FalconRequestCampaignData *msg;
				MonoPrint ("Send Match Play In Progress");
				msg = new FalconRequestCampaignData(requester->Id(), requester);
				msg->dataBlock.who = vuLocalSessionEntity->Id();
				msg->dataBlock.dataNeeded = (ulong)CampStatusEnum::DF_MATCH_IN_PROGRESS;
				FalconSendMessage (msg,TRUE);
			}
			else {
				MonoPrint ("Data Needed %08x\n", request->dataBlock.dataNeeded);
				// Send back what was requested:
				if (request->dataBlock.dataNeeded & (ulong)CampStatusEnum::CAMP_NEED_PRELOAD){
				...
				if (request->dataBlock.dataNeeded & (ulong)CampStatusEnum::CAMP_NEED_WEATHER){
					MonoPrint("Sending Weather\n");
					((WeatherClass*)realWeather)->SendWeather(requester);
				}

				if (request->dataBlock.dataNeeded & (ulong)CampStatusEnum::CAMP_NEED_PERSIST){
				...
				if (request->dataBlock.dataNeeded & (ulong)CampStatusEnum::CAMP_NEED_OBJ_DELTAS){
				...
				if (request->dataBlock.dataNeeded & (ulong)CampStatusEnum::CAMP_NEED_UNIT_DATA){
				...
				if (request->dataBlock.dataNeeded & (ulong)CampStatusEnum::CAMP_NEED_ENTITIES && request->dataBlock.who != vuLocalSession){
	...
	*/
				...
				if (request->dataBlock.dataNeeded & (ulong)CampStatusEnum::CAMP_NEED_TEAM_DATA && request->dataBlock.who != vuLocalSession){
				...
				if (request->dataBlock.dataNeeded & (ulong)CampStatusEnum::CAMP_NEED_VC && request->dataBlock.who != vuLocalSession){
					MonoPrint ("Sending VC Data\n");
					SendVCData(requester);
				}

				if (request->dataBlock.dataNeeded & (ulong)CampStatusEnum::CAMP_NEED_PRIORITIES){
