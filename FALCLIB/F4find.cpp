...
HKEY theKey;
   ...
   size = dataSize;
#if 1 //RERUN
   retval = RegOpenKeyEx(HKEY_LOCAL_MACHINE, FALCON_REGISTRY_KEY, 0, KEY_READ, &theKey);
#else
	   ...
	   buffer[bufSize-1] = 0;
	...
	else
		{
			...
			if (GetPrivateProfileString ("Files", filename, "", tmpStr, 1024, path))
			{
				...
				{
					FILE*		fp;
					sprintf (path, "%s\\FreeFalcon.exe", FalconDataDirectory);
					if (fp = fopen(path, "rb"))
					{
						fclose(fp);
						if (fp = fopen(path, "w"))
					...
					sprintf (path, "%s\\FFViper Config Editor.exe", FalconDataDirectory);
					if (fp = fopen(path, "rb"))
					{
						fclose(fp);
						if (fp = fopen(path, "w"))
...
int F4SaveData (char path[], void* buffer, int offset, int length)
	...
	if (fwrite(buffer,length,1,fp) != 1)
		{
		sprintf(ebuf,"Failed to write file: %s\n",path);
		F4Warning(ebuf);
		fclose(fp);
		return -1;
		}
