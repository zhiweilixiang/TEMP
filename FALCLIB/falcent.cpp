...
int FalconEntity::calc_dirty_bucket(int dirty_score) {
	...
	// sfr : new
	int bin = 0;
	while ((dirty_score & 0x1) == 0){ ++bin; dirty_score >>= 4; }
	return bin;
...
void FalconEntity::MakeDirty (Dirty_Class bits, Dirtyness score) {
	...
	if (
		...
		(score <= dirty_score) || 
		!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_LOADED)
	){
		return;
	}
...
void FalconEntity::DoCampaignDirtyData(VU_TIME realTime) {
	...
	if (!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_LOADED) || ((realTime - lastSent) < SIMDIRTYDATA_INTERVAL)){
		return;
	}
	lastSent = realTime;
	...
	for (int bucket = MAX_DIRTY_BUCKETS - 1; (bucket >= 0); --bucket){
...
#endif
		{
			...
			if (bucket >= 7){ campDirtyMsg->RequestOutOfBandTransmit(); }
...
void FalconEntity::DoSimDirtyData(VU_TIME realTime) {
	...
	if (!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_LOADED) || (simDirtyBuckets == NULL)){
		return;
	}
