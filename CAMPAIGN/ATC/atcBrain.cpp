...
ATCBrain::ATCBrain (ObjectiveClass* mySelf)
{
	...
	pLastTraffic = NULL;
	trafficCheck = TrafficStatusEnum::noTraffic;
	trafficRange = 0;
	oldTrafficRange = 0;
	trafficAltitude = 0;
...
void ATCBrain::ProcessInbound(void)
{
	...
	if (inboundQueue && F4IsBadReadPtr(inboundQueue, sizeof(runwayQueueStruct))) // JB 010801 CTD
		return;
...
{
	int i, rwindex;
	runwayQueueStruct* nextLand[4] = { 0 }; //room for bigger airbases :)

	for(i = 0; i < min(numRwys,4); i++) //RERUN no overrun possible
	{
	...
	while (session)
	{
		...
		if (player && player->IsAirplane())
		{
			...
			if(nextLand[queue])
			{
				...
				if(rwindex && player->af->vt < 30.0F*KNOTS_TO_FTPSEC && ( player->DBrain()->ATCStatus() == AtcStatusEnum::lCrashed ||
								(player->DBrain()->ATCStatus() > AtcStatusEnum::lLanded && deltaTime < LAND_TIME_DELTA/2) ||
								(player->DBrain()->ATCStatus() == AtcStatusEnum::lLanded && deltaTime < LAND_TIME_DELTA/4 && player->af->vt < 5.0F)))
				{
	...
	for(i = 0; i < numRwys; i++)
	{
		...
		while(nextLand[i] && !runwayStats[i].rnwyInUse && info)
		{
			...
			if (info->status == AtcStatusEnum::lCrashed || info->status == AtcStatusEnum::tTakeRunway || info->status == AtcStatusEnum::tTakeoff)
			{				
				aircraft = (AircraftClass*)vuDatabase->Find(info->aircraftID);
				if(aircraft && aircraft->IsAirplane() && aircraft->af->vt < 70.0F*KNOTS_TO_FTPSEC)
				{
		...
		if(aircraft && aircraft->IsAirplane()){
			...
			if(!aircraft->IsPlayer() && SimLibElapsedTime > info->schedTime + FalconLocalGame->rules.AiPullTime){
				...
				case AtcStatusEnum::tWait:
					if(nextTakeoff && (nextTakeoff->aircraftID == info->aircraftID) )
					{
						if(info->prev)
						{
							...
							if(	info->schedTime < SimLibElapsedTime + 30*CampaignSeconds &&
								...
								(accelerateTakeoffs || runwayQueue[queue]->status == AtcStatusEnum::lLanded))
							{
								info->status = AtcStatusEnum::tTakeRunway;
								SendCmdMessage(aircraft, info);
								GiveOrderToWingman(aircraft, info->status);
								if(aircraft->vehicleInUnit < 2)
									GiveOrderToSection(aircraft, AtcStatusEnum::tPrepToTakeRunway, 1);
							}
							// RAS - if previous aircraft has requested T/O, then enter here
							else if(info->prev->status < AtcStatusEnum::tReqTaxi)
							{
								info->status = AtcStatusEnum::tHoldShort;
								SendCmdMessage(aircraft, info);

								radioMessage = CreateCallFromATC (self, aircraft, rcHOLDSHORT, FalconLocalGame);
								radioMessage->dataBlock.edata[3] = 0;
								if(info->prev->status < AtcStatusEnum::lCrashed)
						...
						{
							info->status = AtcStatusEnum::tTakeoff;
							SendCmdMessage(aircraft, info);
							GiveOrderToWingman(aircraft, info->status);
							if(aircraft->vehicleInUnit < 2)
								GiveOrderToSection(aircraft, AtcStatusEnum::tPrepToTakeRunway, 1);
						...
						else
						{
							...
							radioMessage = CreateCallFromATC (self, aircraft, rcHOLDSHORT, FalconLocalGame);
							radioMessage->dataBlock.edata[3] = 0;
							if(info->prev && info->prev->status < AtcStatusEnum::lCrashed)
								radioMessage->dataBlock.edata[3] = (short)(1 + (rand()%4));
							FalconSendMessage(radioMessage, FALSE);
						}
					}
				...
				case AtcStatusEnum::tTaxi:
					...
					if( nextTakeoff && (nextTakeoff->aircraftID == info->aircraftID) )
					{
						...
						if(info->prev)
						{
							...
							else if(info->prev->status < AtcStatusEnum::tReqTaxi && info->prev->status > AtcStatusEnum::noATC)
							{
								info->status = AtcStatusEnum::tHoldShort;
								SendCmdMessage(aircraft, info);
					...
					//RAS - need to get here if taxi order is messed up I believe
					...
					//and next acft to T/O is not current obj???, and next acft to T/O is not on rwy, then
							...
							pNTOAircraft->af->vt < 5.0F * KNOTS_TO_FTPSEC &&	info->schedTime < SimLibElapsedTime  &&
							pNTOAircraft->GetCampaignObject() != aircraft->GetCampaignObject() && !IsOnRunway(pNTOAircraft) )
					{
						...
						{
							ReorderFlight(queue, (Flight)pNTOAircraft->GetCampaignObject(), AtcStatusEnum::tWait);
				...
				case AtcStatusEnum::tTakeRunway:
					...
					if(waitforlanding && !IsOnRunway(aircraft) )
					{
						...
						info->status = AtcStatusEnum::tPrepToTakeRunway;
						SendCmdMessage(aircraft, info);
					...
					else if(nextTakeoff && (nextTakeoff->aircraftID == info->aircraftID ))
					{
				...
				case AtcStatusEnum::tEmerStop:
					...
					{
						info->status = AtcStatusEnum::tTaxi;
						SendCmdMessage(aircraft, info);
					}
					info = info->next;
					if(!aircraft->IsPlayer() && !aircraft->OnGround())
				...
				case AtcStatusEnum::lToBase:
					...
					{
						info->status = AtcStatusEnum::lAborted;
						SendCmdMessage(aircraft,info);	
					}
					info = info->next;
					if(!aircraft->IsPlayer() && aircraft->OnGround())
				...
				case AtcStatusEnum::lClearToLand:
					...
					while(temp)
					{
						if(temp->status >= AtcStatusEnum::lReqClearance && temp->status < AtcStatusEnum::lLanded )
						{
							break;
						}
						temp = temp->next;
...
void ATCBrain::ProcessPlayers(void)
{
	...
	while (session)
	{
		...
		if (player && player->IsAirplane())
		{
			...
			if((pow(dx,2) + pow(dy,2)) < (APPROACH_RANGE * pow(NM_TO_FT,2)))
			{
				...
				if( playerInfo )
				{
					...
					CalculateMinMaxTime(player, playerInfo->rwindex, playerInfo->status, &min, &max, cosAngle);
					...
					case AtcStatusEnum::lHolding:
						...
						if(playerInfo->schedTime < SimLibElapsedTime + max - CampaignSeconds * 5)
						{
							if(cosAngle < 0.0F)
							{
								playerInfo->status 	= AtcStatusEnum::lToBase;
								SendCmdMessage(player, playerInfo);
							}
							else
							{
								playerInfo->status 	= AtcStatusEnum::lToFinal;
								SendCmdMessage(player, playerInfo);
							}
						}
						break;
...
//						CheckForTraffic(player, playerInfo);	//RAS - 20Jan04 -  add later
					...
					case AtcStatusEnum::tTakeoff:
						...
						else if(!player->af->IsSet(AirframeClass::GearBroken) && playerInfo->lastContacted + 60 * CampaignSeconds < SimLibElapsedTime)
						{
							...
							else if(SimLibElapsedTime > playerInfo->schedTime + 60 * CampaignSeconds)	 // 06FEB04 - FRB - was 15 seconds
							{
								...
								if(runwayQueue[queue]->status == AtcStatusEnum::lOnFinal)
					...
					case AtcStatusEnum::tFlyOut:
						...
						{
							player->DBrain()->SetATCStatus(AtcStatusEnum::noATC);
							//remove from list
							RemoveTraffic(player->Id(), PtHeaderDataTable[playerInfo->rwindex].runwayNum);
						}
						break;
							...
							else
							{
								...
								if(!player->DBrain()->IsSetATC(DigitalBrain::Landed))
								{
									...
									if(!curObj){
										...
										landInfo.rwindex = 0;
										landInfo.status = AtcStatusEnum::lLanded;
						...
						else
						{
							...
							if(player->DBrain()->IsSetATC(DigitalBrain::Landed))
							{
								...
								if(!curObj){
									...
									info.status = AtcStatusEnum::tFlyOut;
...
{	
	...
	if( !addflight )
	{
		...
		cosAngle = DetermineAngle(approaching, rwindex, AtcStatusEnum::lHolding);
		CalculateMinMaxTime(approaching, rwindex, AtcStatusEnum::lReqClearance, &min, &max, cosAngle);
		landTime = GetNextAvailRunwayTime(queue, SimLibElapsedTime + min, LAND_TIME_DELTA);

		info = AddTraffic(approaching->Id(), AtcStatusEnum::lHolding, rwindex, landTime);
		if(!info)
			return;

		if(landTime - SimLibElapsedTime < FINAL_TIME)
		{
			info->status = AtcStatusEnum::lOnFinal;
		}
	...
	else
	{
		...
		while(aircraft)
		{
			if(!aircraft->OnGround())
			{
				...
				cosAngle = DetermineAngle(aircraft, rwindex, AtcStatusEnum::lHolding);
				CalculateMinMaxTime(aircraft, rwindex, AtcStatusEnum::lHolding, &min, &max, cosAngle);
				landTime = GetNextAvailRunwayTime(queue, SimLibElapsedTime + min, LAND_TIME_DELTA);

				info = AddTraffic(aircraft->Id(), AtcStatusEnum::lHolding, rwindex, landTime);
				if(!info)
					return;
...
void ATCBrain::RequestEmerClearance(AircraftClass* approaching)
{
	...
	if(info)
	{
		if(info->status >= AtcStatusEnum::lEmergencyToBase && info->status <= AtcStatusEnum::lEmergencyOnFinal)
		{
	...
	cosAngle = DetermineAngle(approaching, rwindex, AtcStatusEnum::lHolding);
	CalculateMinMaxTime(approaching, rwindex, AtcStatusEnum::lHolding, &min, &max, cosAngle);
	...
	if(runwayStats[queue].nextEmergency != FalconNullId)
	{
		...
		while(info)
		{
			if(info->status >= AtcStatusEnum::lEmergencyToBase && info->status <= AtcStatusEnum::lEmergencyOnFinal && landTime + EMER_SLOT*2 < info->schedTime)
				break;
			if(info->status >= AtcStatusEnum::lEmergencyToBase && info->status <= AtcStatusEnum::lEmergencyOnFinal && landTime < info->schedTime + EMER_SLOT*2)
	...
	if(cosAngle < 0.0F)
		status = AtcStatusEnum::lEmergencyToBase;
	else
		status = AtcStatusEnum::lEmergencyToFinal;
...
void ATCBrain::RequestTakeoff(AircraftClass* departing)
{
	...
	else
	{
		...
		if( info )
		{
			if(info->schedTime + 60 * CampaignSeconds > SimLibElapsedTime ||	//RAS - Change to 60sec prior to takeoff???
				(info->status == AtcStatusEnum::tTakeoff ))
			{
				int takeoffNum;
				switch(info->status)
		...
		while(aircraft)
		{
			if(aircraft->OnGround())
			{
				if(!FindBestTakeoffRunway(TRUE))
				{
					...
					if(rwindex)
					{
						runwayQueueStruct *info = AddTraffic(aircraft->Id(), AtcStatusEnum::tTaxiBack, rwindex, SimLibElapsedTime );
						if(info)
							SendCmdMessage(aircraft, info);
					}
					aircraft = (AircraftClass*) flightIter.GetNext();
				...
				runwayQueueStruct *info = AddTraffic(aircraft->Id(), AtcStatusEnum::tTaxi, rwindex, time );
				...
				if(runwayStats[queue].nextEmergency != FalconNullId)
					info->status = AtcStatusEnum::tEmerStop;

				SendCmdMessage(aircraft, info);
				if(aircraft->IsPlayer() || aircraft->vehicleInUnit == 0)
				{
					if(info->status == AtcStatusEnum::tTaxi)
					{
					...
					else if(info->status == AtcStatusEnum::tHoldShort)
					{
...
void ATCBrain::RequestTaxi(AircraftClass* departing)
{
			...
			(info->status == AtcStatusEnum::tTakeoff ))
		{
			...
			case AtcStatusEnum::tTakeoff:
				if(info->schedTime + 120 * CampaignSeconds < SimLibElapsedTime)	 // 06FEB04 - FRB - was 60 seconds
				{
					...
					if(!F4IsBadReadPtr(*runwayQueue, sizeof(runwayQueueStruct)) && runwayQueue[queue]->status == AtcStatusEnum::lOnFinal) // JB 010326 CTD
						radioMessage->dataBlock.edata[3] = 1;
					else
						radioMessage->dataBlock.edata[3] = 0;
					FalconSendMessage(radioMessage, FALSE);
			...
			case AtcStatusEnum::tWait:
			case AtcStatusEnum::tHoldShort:
				//RERUN test later to move holdshort away from clear to taxi
			case AtcStatusEnum::tPrepToTakeRunway:
				//Cobra Holdshort is often first call and doesn't give the runway to taxi to
				radioMessage = CreateCallFromATC (self, departing, rcCLEARTOTAXI, FalconLocalGame);
				radioMessage->dataBlock.edata[2] = (short)GetRunwayName(info->rwindex);
	...
	if(rwindex)
	{
		...
		info = AddTraffic(departing->Id(), AtcStatusEnum::tTaxi, rwindex, takeoffTime );
		if(!info)
			return;
...
void ATCBrain::AbortApproach(AircraftClass* approaching)
{
	...
	approaching->DBrain()->SetATCStatus(AtcStatusEnum::noATC);
	//Cobra
...
void ATCBrain::FindNextEmergency(int queue)
{
	...
	{
		if(info->status >= AtcStatusEnum::lEmergencyToBase && info->status <= AtcStatusEnum::lCrashed)
		{
...
void ATCBrain::SetEmergency(int queue)
{
   ...
   if (!emer)
	...
	listPtr = emer->next;
	while(listPtr &&  ( listPtr->prev->status < AtcStatusEnum::lEmergencyToBase || listPtr->prev->status > AtcStatusEnum::lEmergencyOnFinal) )
	{
		info = listPtr;
	...
	while(listPtr)
	{
		...
		if(listPtr->status >= AtcStatusEnum::tReqTaxi)
			RequestTakeoff(aircraft);
		else if(listPtr->status > AtcStatusEnum::noATC)
...
void ATCBrain::ReschedulePlanes(int queue)
{
	...
	{
		aircraft = (AircraftClass*)vuDatabase->Find(info->aircraftID);
		if(aircraft && info->status != AtcStatusEnum::noATC)
		{
			if(info->status > AtcStatusEnum::lCrashed)
			{
				info->status = AtcStatusEnum::tTaxiBack;
				SendCmdMessage(aircraft, info);
				info = info->next;
				RemoveTraffic(deleteInfo->aircraftID, queue);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void ATCBrain::RescheduleFlightTakeoff(int queue, FlightClass *flight)
{
	AircraftClass		*aircraft = NULL;
	runwayQueueStruct	*info = NULL;
	runwayQueueStruct	*deleteInfo = NULL;
	...
	while(aircraft)
	{
		if(aircraft->OnGround())
		{
			...
			info = AddTraffic(aircraft->Id(), AtcStatusEnum::tTaxi, rwindex, takeoffTime );
			if(!info)
			{
				aircraft = (AircraftClass*) flightIter.GetNext();
				continue;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void ATCBrain::ReorderFlight(int queue, FlightClass *flight, AtcStatusEnum status)
{
	AircraftClass		*aircraft = NULL;
	runwayQueueStruct	*info = NULL;
	FalconRadioChatterMessage	*radioMessage = NULL;
	...
	while(info)
	{
		...
		{
			info->status = status;
			if(status == AtcStatusEnum::tHoldShort)
			{
				status = AtcStatusEnum::tTaxi;
				radioMessage = CreateCallFromATC (self, aircraft, rcHOLDSHORT, FalconLocalGame);
				radioMessage->dataBlock.edata[3] = 0;
				FalconSendMessage(radioMessage, FALSE);
			}
...
int ATCBrain::FindBestTakeoffRunway(int checklist)
{
	...
	if((!checklist) || (!runwayQueue))
		return rwindex;

	info = runwayQueue[PtHeaderDataTable[rwindex].runwayNum];
	while(info && info->status == AtcStatusEnum::noATC)
		info = info->next;
	...
	else if(numRwys == 2 )
	{
		...
		if(info)
		{
			...
			if(delta < 91)
			{
				if(info->status < AtcStatusEnum::tReqTaxi && info->status != AtcStatusEnum::noATC && GetOppositeRunway(rwindex))
					rwindex = GetOppositeRunway(rwindex);
			}
			else
			{
				if(info->status >= AtcStatusEnum::tReqTaxi && GetOppositeRunway(rwindex))
					rwindex = GetOppositeRunway(rwindex);
			}
		}
	}
...
void ATCBrain::CalculateMinMaxTime(AircraftClass* aircraft, int rwindex, AtcStatusEnum status, CampaignTime *min, CampaignTime *max, float cosAngle)
{
	...
	decelTime = (float)(fabs((double)aircraft->af->vt - (double)PatternSpd)*0.2);
...
void ATCBrain::CalcRunwayDimensions(int index)
{
	...
	runwayStats[queue].halfwidth = (float)sqrt(pow(((double)x1 - (double)x2),2) + pow(((double)y1 - (double)y2),2))*0.5F;
	runwayStats[queue].halfheight = (float)sqrt(pow(((double)x2 - (double)x3),2) + pow(((double)y2 - (double)y3),2))*0.5F;
...
float ATCBrain::DetermineAngle(AircraftClass* aircraft, int rwindex, AtcStatusEnum status)
{
	...
	if (aircraft)
	{
		dx = aircraft->XPos() - px;
		dy = aircraft->YPos() - py;
		norm = (float)(1.0F/sqrt(pow(dx,2) + pow(dy,2)));
		dx *= norm;
		dy *= norm;
...
ulong ATCBrain::FindFlightTakeoffTime(FlightClass *flight, int queue)
{
	...
	if(cur->aircraftID == runwayStats[queue].nextEmergency || (cur->status >= AtcStatusEnum::lEmergencyToBase && cur->status <= AtcStatusEnum::lEmergencyOnFinal))
...
int ATCBrain::FindBestLandingRunway(FalconEntity* landing, int checklist){
	...
	if(numRwys == 2 && rwindex2 && PtHeaderDataTable[rwindex2].runwayNum != PtHeaderDataTable[rwindex].runwayNum){
	...
	if(info){
		if(info->status >= AtcStatusEnum::tReqTaxi && GetOppositeRunway(info->rwindex)){
			rwindex = GetOppositeRunway(info->rwindex);
		}
		else{
			rwindex = info->rwindex;
...
void ATCBrain::FindEmergencyLandingRunway(int *queue, int *rwindex, FalconEntity* landing)
{
	...
	headingfrom = (float)atan2((double)self->XPos() - (double)landing->XPos(), (double)self->YPos() - (double)landing->YPos());
...
runwayQueueStruct* ATCBrain::NextToTakeoff( int queue)
{
	...
	while(temp)
	{
		if(temp->status == AtcStatusEnum::noATC && !holdshort && !taxi && !wait)
		{
			return temp;
		}
		else if(temp->status >= AtcStatusEnum::tPrepToTakeRunway && temp->status <= AtcStatusEnum::tTakeoff )
		{
			return temp;
		}
		else if(!holdshort && temp->status == AtcStatusEnum::tHoldShort)
		{
			holdshort = temp;
		}
		else if(!wait && temp->status == AtcStatusEnum::tWait)
		{
			wait = temp;
		}
		else if(!taxi && temp->status >= AtcStatusEnum::tTaxi && temp->status < AtcStatusEnum::tHoldShort)
		{
			taxi = temp;
		}
		temp = temp->next;
...
int ATCBrain::CheckVector(AircraftClass *aircraft, runwayQueueStruct* info)
{
	...
	vt = (float)sqrt(pow(aircraft->XDelta(),2) + pow(aircraft->YDelta(),2));
	...
	{
	case AtcStatusEnum::lFirstLeg:
		if(relx < turnDist && fabs(rely) < turnDist*3.0 && info->lastContacted < SimLibElapsedTime)
		{
			...
			if(cosAngle < 0.0F)
			{
				...
				FindBasePt(aircraft, info->rwindex, finalX, finalY, &baseX, &baseY);
				aircraft->DBrain()->SetATCStatus(AtcStatusEnum::lToBase);
				aircraft->DBrain()->SetTrackPoint(baseX, baseY, GetAltitude(aircraft, AtcStatusEnum::lToBase));
				aircraft->DBrain()->CalculateNextTurnDistance();
				info->status = AtcStatusEnum::lToBase;
			}
			else
			{
				FindFinalPt(aircraft, info->rwindex, &finalX, &finalY);
				aircraft->DBrain()->SetATCStatus(AtcStatusEnum::lToFinal);
				aircraft->DBrain()->SetTrackPoint(finalX, finalY, GetAltitude(aircraft, AtcStatusEnum::lToFinal));
				aircraft->DBrain()->CalculateNextTurnDistance();
				info->status = AtcStatusEnum::lToFinal;
			}
	...
	case AtcStatusEnum::lToFinal:
		...
		if(relx < 3.0F*NM_TO_FT && relx > -1.0F*NM_TO_FT && fabs(rely) < turnDist && info->lastContacted < SimLibElapsedTime)
		{
			...
			aircraft->DBrain()->SetATCStatus(AtcStatusEnum::lOnFinal);
			TranslatePointData (self, GetFirstPt(info->rwindex), &x, &y);
			aircraft->DBrain()->SetTrackPoint(x, y, GetAltitude(aircraft, AtcStatusEnum::lOnFinal));
			info->status = AtcStatusEnum::lOnFinal;
	...
	{
    if(info->status < AtcStatusEnum::lLanded && fabs((double)speed - (double)vt) > 30.0F)	//haven't landed and speed is off by > 30kts
		{
			//we're not heading for our track point
			MakeVectorCall(aircraft, FalconLocalGame);
			info->lastContacted = SimLibElapsedTime;
...
AtcStatusEnum	ATCBrain::FindFirstLegPt(AircraftClass* approaching, int rwindex, ulong schedTime, float pointX, float pointY, int usebase, float *x, float *y)
{
	...
	decelTime = (float)fabs((double)approaching->af->vt - (double)PatternSpd)*0.2F;
	...
	*y = approaching->YPos() + Trig.sin * totalDist * 0.5F;
	...
	return AtcStatusEnum::lFirstLeg;
...
float ATCBrain::GetAltitude(AircraftClass* aircraft, AtcStatusEnum status)
{
	...
	switch(status)
	{
		case AtcStatusEnum::lReqClearance:
		case AtcStatusEnum::lAborted:
		case AtcStatusEnum::noATC: // JPO - when landing but outside airspace.
			alt = pos.z - 4000.0F;
			break;
		case AtcStatusEnum::lIngressing:
		case AtcStatusEnum::lTakingPosition:
...
int ATCBrain::CalculateStandRateTurnToPt(AircraftClass *aircraft, float x, float y, float *finalHdg)
{
	...
	if((float)fabs(deltaHdg) < 5.0f*DTR)
		dir = -1; //no turn
	else if(deltaHdg < -PI)
		dir = 1;
	else if(deltaHdg > PI)
...
runwayQueueStruct* ATCBrain::InList(VU_ID aircraftID)
{
	...
	if (F4IsBadReadPtr(this, sizeof(ATCBrain)) || 
		F4IsBadReadPtr(runwayQueue, sizeof(runwayQueueStruct*)) || !runwayQueue) // JB 010317 CTD
	{
		return NULL;
	}
...
runwayQueueStruct * ATCBrain::AddTraffic(VU_ID aircraftID, AtcStatusEnum status, int rwindex, long schedTime)
{
	...
	if(status > AtcStatusEnum::lCrashed)
	{
...
void ATCBrain::AddInbound(AircraftClass *aircraft)
{
	...
	info->status		= AtcStatusEnum::lTakingPosition;			//at what point in the landing/takeoff process
...
void ATCBrain::GiveOrderToWingman(AircraftClass *us, AtcStatusEnum status)
{
	...
	else if(wingmanInfo && (status == AtcStatusEnum::tTakeoff || status == AtcStatusEnum::tTakeRunway) && wingmanInfo->status < status)
	{
		wingmanInfo->status = AtcStatusEnum::tPrepToTakeRunway;
		SendCmdMessage(wingman, wingmanInfo);
	}
}
...
void ATCBrain::MakeVectorCall(AircraftClass *aircraft, VuTargetEntity *target)
{
	...
	{
	case AtcStatusEnum::lFirstLeg:
		if(fabs((double)aircraft->af->vcas - (double)speed) > 10.0F)
	...
	case AtcStatusEnum::lToBase:
		if(rwtime > SimLibElapsedTime)
		{
			...
			dx = aircraft->XPos() - x;
			dy = aircraft->YPos() - y;
			speed = (float)sqrt(pow(dx, 2) + pow(dy,2))/deltaTime * FTPSEC_TO_KNOTS;
		...
		desAlt = GetAltitude(aircraft, AtcStatusEnum::lToBase);	//RAS-3Oct04-Changed from lToFinal to lToBase
		
		if(fabs((double)aircraft->ZPos() - (double)desAlt) < 800.0F)	//RAS-23Jan04-changed from 100 to 800
		...
		if(aircraft->DBrain()->lastVectorTypeCall < SimLibElapsedTime)
		{
            ...
            if(radioMessage->dataBlock.edata[3] == -1 && radioMessage->dataBlock.edata[4] == -1)
	...
	case AtcStatusEnum::lToFinal:
		...
		desAlt = GetAltitude(aircraft, AtcStatusEnum::lToFinal);
		
		if(fabs((double)aircraft->ZPos() - (double)desAlt) < 500.0F)	//RAS-23Jan04- changed from 100 to 500
			index = -1;									//RAS-3Oct04-Maintain Altitude-changed from 2 to -1
...
int	ATCBrain::GetLandingNumber(runwayQueueStruct* landInfo)
{
	...
	if(landInfo && landInfo->rwindex)
	{
		...
		while(info)
		{
			...
			if(info->status < AtcStatusEnum::tReqTaxi && info->status >= AtcStatusEnum::lReqClearance)
				pos++;
...
int	ATCBrain::GetTakeoffNumber(runwayQueueStruct* takeoffInfo)
{
	...
	if(takeoffInfo->status <= AtcStatusEnum::lCrashed)
		return -1;
	...
	if(takeoffInfo && takeoffInfo->rwindex)
	{
		...
		while(info)
		{
			...
			if(info->status > AtcStatusEnum::lCrashed && info->status < AtcStatusEnum::tTakeoff)
				pos++;
...
SimBaseClass* CheckTaxiPointGlobal(AircraftClass *self, float x, float y)
{
	...
	if (self && self->af->GetParkType() == LargeParkPt)
		BigBoy = true;
	...
	{		
		...
		// if object is within a given range of the point return object
		if ( testDist < testRad * testRad && self && testObject->GetVt() < self->GetVt() + 20.0F)
		{
			if(!closest)
			{
				closest = testObject;
...
int	ATCBrain::CheckLanding(AircraftClass *aircraft, runwayQueueStruct* landInfo)
{
	...
	if(aircraft->OnGround() && !aircraft->DBrain()->IsSetATC(DigitalBrain::Landed))
	{
		if(landInfo)
		{
			if(landInfo->status != AtcStatusEnum::lEmergencyToBase && landInfo->status != AtcStatusEnum::lEmergencyToFinal &&
			   landInfo->status != AtcStatusEnum::lEmergencyOnFinal && next != landInfo &&
				 landInfo->status != AtcStatusEnum::lLanded) // JB 010713 If we're already landed, then what's the big deal?
			{
			...
			if(landInfo->status != AtcStatusEnum::lLanded)
			{
				landInfo->status = AtcStatusEnum::lLanded;
...
int	ATCBrain::CheckTakeoff(AircraftClass *aircraft, runwayQueueStruct* info)
{
	...
	if(!aircraft->OnGround() && aircraft->DBrain()->IsSetATC(DigitalBrain::Landed))
	{
		if(info)
		{
			// If you are in this routine, then you are airborne because
			// !aircraft->OnGround() means you are flying.  if you are flying, then you should
...
void ATCBrain::CheckFinalApproach(AircraftClass *aircraft, runwayQueueStruct *info)
{
	...
	else if(dist < 0.8F * aircraft->af->MinVcas() * KNOTS_TO_FTPSEC * 60.0F)
	{
		...
		else if(!aircraft->DBrain()->IsSetATC(DigitalBrain::ClearToLand) && nextLand == info)
		{
			...
			info->status = AtcStatusEnum::lClearToLand;
			SendCmdMessage(aircraft,info);
			//info->status = lOnFinal;
		}
	}
	else if(info->next && info->next->schedTime + 15 * CampaignSeconds < SimLibElapsedTime && info->next->status == AtcStatusEnum::lOnFinal)
	{
		info->status = AtcStatusEnum::lAborted;
		SendCmdMessage(aircraft,info);	
	}
	...
	FalconRadioChatterMessage	*radioMessage = NULL;
	if (aircraft->IsPlayer() && (dist < 12000.0f && dist > 6000.0f) && aircraft->af->gearPos < 0.5f) //rcLANDINGCHECK
	{ 
		radioMessage = CreateCallFromATC( self, aircraft, rcLANDINGCHECK, FalconLocalSession);		
		FalconSendMessage(radioMessage, FALSE);

	}
	else if (aircraft->IsPlayer() && dist < 6000.0f && aircraft->af->gearPos < 0.5f) //rcATCGOAROUND
	{
		info->status = AtcStatusEnum::lAborted;
		SendCmdMessage(aircraft,info);
	}
...
int	ATCBrain::CheckIfBlockingRunway(AircraftClass *aircraft, runwayQueueStruct* info)
{
	...
	if(info && info->status == AtcStatusEnum::lLanded)
	{		
		// OW: Jackals "scold-on-bounce" fix
		if(!rwindex && aircraft->OnGround())
		{
			info->status = AtcStatusEnum::lTaxiOff;
		}
		else if( info->lastContacted + 90 * CampaignSeconds < SimLibElapsedTime &&	 // 06FEB04 - FRB - was 45 seconds 
	...
	else if(rwindex && !info && aircraft->DBrain()->WaitTime() + 90 *CampaignSeconds < SimLibElapsedTime)	 // 06FEB04 - FRB - was 45 seconds
	{
		//yell at them to get off runway
			if(!nextOnRunway)
				SendCallFromATC (self, aircraft, rcTAXICLEAR, FalconLocalGame);
	...
	else if( rwindex && info && info->lastContacted + 90 *CampaignSeconds < SimLibElapsedTime )	 // 06FEB04 - FRB - was 45 seconds
	{					
		//yell at them to get off runway
			if(!nextOnRunway)
				SendCallFromATC (self, aircraft, rcTAXICLEAR, FalconLocalGame);
...
void ATCBrain::FindAbortPt(AircraftClass* aircraft, float *x, float *y, float *z)
{
	...
	*z = GetAltitude(aircraft, AtcStatusEnum::lAborted);
}
...
void ATCBrain::CheckForTraffic(AircraftClass *aircraft, runwayQueueStruct *playerInfo)
{
	...
	if (self->brain->checkTrafficTime < SimLibElapsedTime) {
		...
		if (self->brain->pLastTraffic != NULL)
		...
		if (traffic == NULL)
			self->brain->trafficCheck = TrafficStatusEnum::noTraffic;		//if no traffic set status to none	
		
		switch (self->brain->trafficCheck)
		{
		case TrafficStatusEnum::noTraffic:
			self->brain->trafficInSightFlag = FALSE;	// no traffic to see
			break;

		case TrafficStatusEnum::priorityTraffic:		// not used right now, but may use it later
		case TrafficStatusEnum::newTraffic:			// traffic not called out last time
		...
		case TrafficStatusEnum::oldTraffic:			// Same traffic called last time
		...
		if (self->brain->trafficInSightFlag == TRUE && traffic == self->brain->pLastTraffic)
			return;

		
		// Begin O'Clock code - convert to compass angle
		if ((radioMessage) && (self->brain->trafficCheck != TrafficStatusEnum::noTraffic))
		{
			...
			//eRangeLast
			radioMessage->dataBlock.edata[4] = SimToGrid((float)sqrt(self->brain->trafficRange));
