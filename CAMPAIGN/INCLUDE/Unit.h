...
private:
	...
	*/
	...
	short				odds;			// How bad things can become
	int					dirty_unit;
...
public:
	...
	virtual int Father() const										{ return 0; }
	virtual int Real (void)											{ return 0; }
	virtual float AdjustForSupply(void)								{ return 1.0F; }
	virtual int GetUnitSpeed() const								{ return GetMaxSpeed(); }
	...
	virtual int GetVehicleDeagData(SimInitDataClass*, int)			{ return 0; }
	...
	virtual int IsDead() const						{ return (int)unit_flags & U_DEAD; }		
	//int Dead() const								{ return IsDead(); }		
	int Assigned() const							{ return (int)unit_flags & U_ASSIGNED; }	
	int Ordered() const								{ return (int)unit_flags & U_ORDERED; }	
	int DontPlan() const							{ return (int)unit_flags & U_NO_PLANNING; }	
	int Parent() const								{ return (int)unit_flags & U_PARENT; }		
	int Engaged() const								{ return (int)unit_flags & U_ENGAGED; }	
	int Scripted ()	 const							{ return (int)unit_flags & U_SCRIPTED; }
	int Commando () const								{ return (int)unit_flags & U_COMMANDO; }
	int Moving() const								{ return (int)unit_flags & U_MOVING; }
	int Refused () const								{ return (int)unit_flags & U_REFUSED; }
	int Cargo () const								{ return (int)unit_flags & U_CARGO; }
	int Combat () const								{ return (int)unit_flags & U_COMBAT; }		
	int Broken () const								{ return (int)unit_flags & U_BROKEN; }		
	int Aborted () const								{ return (int)unit_flags & U_BROKEN; }		
	int Losses () const								{ return (int)unit_flags & U_LOSSES; }		
	int Inactive () const								{ return (int)unit_flags & U_INACTIVE; }
	int Fragment () const								{ return (int)unit_flags & U_FRAGMENTED; }
	int Targeted () const								{ return (int)unit_flags & U_TARGETED; }
	int Retreating () const							{ return (int)unit_flags & U_RETREATING; }
	int Detached () const								{ return (int)unit_flags & U_DETACHED; }
	int Supported () const							{ return (int)unit_flags & U_SUPPORTED; }
	int TempDest () const								{ return (int)unit_flags & U_TEMP_DEST; }
	int Final () const								{ return (int)unit_flags & U_FINAL; }
	...
	void SetUnitAltitude (int alt)					{ SetPosition(XPos(),YPos(),-1.0F * (float)alt); MakeCampBaseDirty (DIRTY_ALTITUDE, DDP[181].priority); }
	...
	virtual void SimSetLocation (float x, float y, float z)	{ SetPosition(x,y,z); MakeCampBaseDirty (DIRTY_POSITION, DDP[182].priority); MakeCampBaseDirty (DIRTY_ALTITUDE, DDP[183].priority); }
	...
	void SetNumVehicles(int vg, int n)				{ SetRoster((roster & ~(3<<(vg*2))) | ((n & 0x03) << (vg*2))); }
	...
	int GetNumVehicles (int vg)						{ return (int)((roster >> (vg*2)) & 0x03); }
	...
	int GetUnitMoved() const						{ return moved; }
	int GetUnitTactic() const						{ return tactic; }
	...
	WayPoint GetFirstUnitWP()				{ return wp_list; }
	...
	virtual MissionTypeEnum GetUnitMission (void)				{ return MissionTypeEnum::AMIS_NONE; }
	virtual int GetUnitNormalRole (void)						{ return 0; }
	virtual int GetUnitCurrentRole() const						{ return 0; }
	...
	// Battalion virtuals
	virtual Unit GetUnitParent() const							{ return NULL; }
	...
	// Brigade virtuals
	virtual Unit GetFirstUnitElement() const					{ return NULL; }
	virtual Unit GetNextUnitElement() const						{ return NULL; }
...
extern DamType GetDamageType (Unit u);
