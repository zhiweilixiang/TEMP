...
class PackageClass : public AirUnitClass 
   ...
   public:
      ...
      void *operator new(size_t size) { ShiAssert( size == sizeof(PackageClass) ); return MemAllocFS(pool);	};
      void operator delete(void *mem) { if (mem) MemFreeFS(mem); };
      static void InitializeStorage()	{ pool = MemPoolInitFS( sizeof(PackageClass), 200, 0 ); };
	...
	public:
		...
		VU_ID GetInterceptor (void)						{ return interceptor; }
		VU_ID GetAwacs (void)							{ return awacs; }
		VU_ID GetJStar (void)							{ return jstar; }
		VU_ID GetECM (void)								{ return ecm; }
/* 2001-04-07 ADDED BY S.G. */ void SetECM (VU_ID newEcm) { ecm = newEcm; }
		VU_ID GetTanker (void)							{ return tanker; }
		Flight GetFACFlight (void);
		CampaignTime GetTakeoff (void)					{ return takeoff; }
		CampaignTime GetTPTime (void)					{ return tp_time; }
		uchar GetFlights (void)							{ return flights; }
		short GetResponses (void)						{ return responses; }
		WayPoint GetIngress (void)						{ return ingress; }
		WayPoint GetEgress (void)						{ return egress; }
		short GetAAStrength (void)						{ return aa_strength; }
	...
	public:
		...
		virtual int IsPackage (void)									{	return TRUE; }
		...
		void SetPackageType (MissionTypeEnum p) 									{	mis_request.mission = p; }
		MissionTypeEnum GetPackageType (void)										{	return mis_request.mission; }
