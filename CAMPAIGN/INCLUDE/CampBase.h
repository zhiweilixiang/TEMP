...
public:
	...
	~IdNamespace(){ VuxDestroyMutex(m); }
	/** resets namespace */
	void Reset(){ 
		VuScopeLock l(m); curId = lowWrap; 
	}
	/** sets the id if higher than current */
	void UseId(VU_ID_NUMBER id){
		if (id >= curId){
			++curId;
			if (curId > hiWrap){ curId = lowWrap; }
		}
	}
	/** get an id */
	VU_ID_NUMBER GetId(){ 
...
public:
	// Access Functions
	CampaignTime GetSpotTime() const			{	return spotTime; }
	short GetSpotted() const 					{	return spotted; }
	short GetBaseFlags() const 					{	return base_flags; }
	short GetCampId() const						{	return camp_id; }
	short GetLocalFlags() const					{	return local_flags; }
	TailInsertList *GetComponents() const		{	return components; }
	VU_ID GetDeagOwner() const					{	return deag_owner; }

	void SetBaseFlags(short);
	virtual void SetOwner(Control);
	void SetCampId(short);
	void SetLocalFlags(void) {}; //RERUN no function definition. Created empty one for the moment
...
public:
	...
	virtual int GetMissilesFlying (void)				{ return 0; } // MLR 10/3/2004 - finishing what //me123 started
	...
	// Queries
	virtual bool IsCampBase() { return true; }
	...
	// Getters
	uchar GetDomain (void)	const						{	return (EntityType())->classInfo_[VU_DOMAIN]; }
	uchar GetClass (void)	const						{	return (EntityType())->classInfo_[VU_CLASS]; }
	uchar GetType()	const						{	return (EntityType())->classInfo_[VU_TYPE]; }
	uchar GetSType (void)	const						{	return (EntityType())->classInfo_[VU_STYPE]; }
	uchar GetSPType (void) const						{	return (EntityType())->classInfo_[VU_SPTYPE]; }
