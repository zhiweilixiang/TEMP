...
public:
		...
		void *operator new(size_t size)	{ ShiAssert( size == sizeof(FlightClass) ); return MemAllocFS(pool);	};
		void operator delete(void *mem)	{ if (mem) MemFreeFS(mem); };
		static void InitializeStorage()	{ pool = MemPoolInitFS( sizeof(FlightClass), 200, 0 ); };
...
private:
		...
		MissionTypeEnum         mission;     				// Unit's mission
		MissionTypeEnum         old_mission;				// Previous mission, if we've been diverted
...
public:
		...
		// Access Functions
		CampaignTime GetLastMove (void)				{ return last_move; }
		CampaignTime GetLastCombat (void)			{ return last_combat; }
		CampaignTime GetTimeOnTarget (void)			{ return time_on_target; }
		CampaignTime GetMissionOverTime (void)		{ return mission_over_time; }
		CampaignTime GetLastEnemyLockTime (void)	{ return last_enemy_lock_time; }
		VU_ID GetAssignedTarget (void)				{ return assigned_target; }
		VU_ID GetEnemyLocker (void)					{ return enemy_locker; }
		WayPoint GetOverrideWP (void);
		LoadoutStruct *GetLoadout (void)			{ return loadout; }
		uchar GetLoadouts (void)					{ return loadouts; }
		MissionTypeEnum GetOriginalMission (void)	{ return old_mission; }
		uchar GetLastDirection (void)				{ return last_direction; }
		uchar GetEvalFlags (void)					{ return eval_flags; }
		uchar GetMissionContext (void)				{ return mission_context; }
		VU_ID GetRequesterID (void)					{ return requester; }
...
#if HOTSPOT_FIX
		virtual CampaignTime MaxUpdateTime() const				{ return FLIGHT_MOVE_CHECK_INTERVAL*CampaignSeconds; }
#else
		virtual CampaignTime UpdateTime (void)					{ return FLIGHT_MOVE_CHECK_INTERVAL*CampaignSeconds; }
#endif
		virtual CampaignTime CombatTime (void)					{ return FLIGHT_COMBAT_CHECK_INTERVAL*CampaignSeconds; }
		virtual int IsFlight() const							{ return TRUE; }
		virtual int GetDetectionRange (int mt);
		virtual int GetRadarMode (void)							{ return FEC_RADAR_SEARCH_100; }
		...
		virtual void SetBurntFuel (long fuel)					{ fuel_burnt = fuel; }
		virtual void SetUnitMission (MissionTypeEnum mis);
		virtual void SetUnitPriority (int p)					{ priority		= (uchar)p; }
		virtual void SetUnitMissionID (int id)					{ mission_id	= (uchar)id; }
		virtual void SetUnitMissionTarget (VU_ID id)			{ mission_target = id; }
		virtual void SetUnitTOT (CampaignTime tot)				{ time_on_target = tot; }
		virtual void SetUnitSquadron (VU_ID ID)					{ squadron = ID; }
		...
		virtual int GetNumberOfLoadouts (void)					{ return loadouts; }
		...
		virtual int GetBurntFuel (void)							{ return fuel_burnt; }
		virtual MissionTypeEnum GetUnitMission (void)						{ return (MissionTypeEnum)mission; }
		virtual int GetUnitCurrentRole() const;
		virtual int GetUnitPriority (void)						{ return (int)priority; }
		virtual int GetUnitWeaponId (int hp, int slot);
		virtual int GetUnitWeaponCount (int hp, int slot);
		virtual int GetUnitMissionID (void)						{ return (int)mission_id; }
		virtual CampEntity GetUnitMissionTarget (void)			{ return (CampEntity)vuDatabase->Find(mission_target); }
		virtual VU_ID GetUnitMissionTargetID (void)				{ return mission_target; }
		virtual CampaignTime GetUnitTOT	(void)					{ return time_on_target; }
		virtual Unit GetUnitSquadron (void)						{ return FindUnit(squadron); }
		virtual VU_ID GetUnitSquadronID (void)					{ return squadron; }
		...
		virtual Unit GetUnitParent (void)						{ return (Unit)vuDatabase->Find(package); }
		virtual VU_ID GetUnitParentID (void)					{ return package; }
		virtual void SetUnitParent (Unit p)						{ SetPackage(p->Id()); }
		...
		int GetPilotSquadronID (int pilotSlot)					{ return pilots[pilotSlot]; }
