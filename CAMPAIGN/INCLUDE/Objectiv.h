...
public:
	...
	int IsGCI (void)										{ return (int)(O_IS_GCI & obj_data.obj_flags); }	// 2002-02-13 ADDED BY S.G.
	int HasNCTR (void)										{ return (int)(O_HAS_NCTR & obj_data.obj_flags); }	// 2002-02-13 ADDED BY S.G.
	...
	void SetObjectiveStatus (uchar s)						{	if (obj_data.status > s) obj_data.last_repair = Camp_GetCurrentTime(); obj_data.status = s; MakeObjectiveDirty (DIRTY_STATUS, DDP[180].priority); }
...
extern DamType GetDamageType (Objective o, int f);
