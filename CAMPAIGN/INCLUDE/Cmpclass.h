...
// Campaign flags
enum class CampStatusEnum : ulong {
...
const unsigned int CAMP_NAME_SIZE=40;						// Size of name string arrays for scenario name and such

// wParam values for FM_JOIN_CAMPAIGN messages:
enum class JoinStatusEnum : int {
	JOIN_NOT_JOINING=0,
	JOIN_PRELOAD_ONLY,
	JOIN_REQUEST_ALL_DATA,
	JOIN_CAMP_DATA_ONLY
};
...
public:
	...
	volatile ulong		Flags; //RERUN check the volatile once...
...
public:
	...
	void SetCurrentTime (CampaignTime newTime)				{ CurrentTime = newTime; }
	void SetTEStartTime (CampaignTime newTime)				{ TE_StartTime = newTime; }
	void SetTETimeLimitTime (CampaignTime newTime)			{ TE_TimeLimit = newTime; }
	CampaignTime GetCampaignTime (void)						{ return CurrentTime; }
	CampaignTime GetTEStartTime (void)						{ return TE_StartTime; }
	CampaignTime GetTETimeLimitTime (void)					{ return TE_TimeLimit; }
	int GetActiveTeams (void)								{ return ActiveTeams; }
	int GetCampaignDay (void);
	int GetCurrentDay (void);
	int GetMinutesSinceMidnight (void);
	char* GetTheaterName (void)								{ return TheaterName; }
	int SetTheater (char* name);
	int SetScenario (char* scenario);
	char* GetScenario (void)								{ return Scenario; }
	char* GetSavedName (void)								{ return SaveFile; }
	void SetPlayerSquadronID (VU_ID id)						{ PlayerSquadronID = id; }
	VU_ID GetPlayerSquadronID (void)						{ return PlayerSquadronID; }
	void GetPlayerLocation (short *x, short *y);
	void GotJoinData (void);
	int IsRunning (void)									{ return Flags & (ulong)CampStatusEnum::CAMP_RUNNING; }
	int IsLoaded (void)										{ return Flags & (ulong)CampStatusEnum::CAMP_LOADED; }
	int IsPreLoaded (void)									{ return Flags & (ulong)CampStatusEnum::CAMP_PRELOADED; }
	int IsSuspended (void)									{ return Flags & (ulong)CampStatusEnum::CAMP_SUSPENDED; }
	int IsMaster (void);
	int IsOnline (void)										{ return Flags & (ulong)CampStatusEnum::CAMP_ONLINE; }
...
#define NEW_END_CAMPAIGN 1
	...
	long GetCreatorIP()				{ return(CreatorIP); }
	long GetCreationTime()			{ return(CreationTime); }
	long GetCreationIter()			{ return(CreationRand); }

	void SetTEVictoryPoints(long points) { TE_VictoryPoints=points; }
