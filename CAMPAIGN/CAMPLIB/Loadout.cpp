...
extern uchar		DefaultDamageMods[(int)DamType::OtherDam+1];
...
int WeaponLoadScore (int wid, int lw, uchar *dam, MoveType mt, int type_flags, int guide_flags, int randomize)
{
	...
	if (WeaponDataTable[wid].DamageType == DamType::NuclearDam)
		score = 0;
	...
	if (type_flags & WEAP_BAI_LOADOUT)
	{
		...
		if (WeaponDataTable[wid].GuidanceFlags == WEAP_VISUALONLY && WeaponDataTable[wid].DamageType == DamType::PenetrationDam)
			score = 0;
	...
	if (type_flags & WEAP_DEAD_LOADOUT) {
		...
		if (WeaponDataTable[wid].GuidanceFlags ==  WEAP_VISUALONLY && WeaponDataTable[wid].DamageType == DamType::PenetrationDam)
			score = 0;
		if (WeaponDataTable[wid].GuidanceFlags ==  WEAP_LASER && WeaponDataTable[wid].DamageType == DamType::PenetrationDam)
			score = 0;
	}
