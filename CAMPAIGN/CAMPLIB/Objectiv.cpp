...
int ObjectiveClass::ApplyDamage (DamType d, int *str, int f, short flags)
{
	...
	while (*str > 0 && count < MAX_DAMAGE_TRIES)
	{
		...
		if (f >= static_data.class_data->Features || f < 0 || GetFeatureStatus(f) == VIS_DESTROYED || !GetFeatureID(f))
		{
			...
			{
				if (GetFeatureStatus(fid) != VIS_DESTROYED && GetFeatureClassData(GetFeatureID(fid))->DamageMod[(int)d] > 0)
					f = fid;
			}
		...
		if (fc && fc->DamageMod[(int)d] > 0)
		{
			hp = fc->HitPoints * 100 / fc->DamageMod[(int)d];
			// Check if high explosive damage will do more
			if ((flags & WEAP_AREA) && fc->DamageMod[(int)DamType::HighExplosiveDam] > fc->DamageMod[(int)d])
				hp = fc->HitPoints * 100 / fc->DamageMod[(int)DamType::HighExplosiveDam];
...
short ObjectiveClass::GetAdjustedDataRate (void)
{
	...
	adjustedDataRate = (short)((GetObjectiveStatus()+almost) * static_data.class_data->DataRate / 100);
	return adjustedDataRate;
}
