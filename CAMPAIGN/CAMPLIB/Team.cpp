...
priority_structure Priorities[(uchar)MissionTypeEnum::AMIS_OTHER] = {0};
...
uchar RoEData[ROE_NAVAL_BOMBARD+1][War+1] = {
	...
	{ ROE_NOT_ALLOWED,	ROE_ALLOWED,		ROE_NOT_ALLOWED,	ROE_NOT_ALLOWED,	ROE_NOT_ALLOWED,	ROE_NOT_ALLOWED	},	// ROE_AIR_USE_BASES
...
uchar	DefaultMissionPriority[TAT_CAS][(uchar)MissionTypeEnum::AMIS_OTHER];
...
: FalconEntity(VU_LAST_ENTITY_TYPE, GetIdFromNamespace(VolatileNS))
{
	...
	memcpychk(mission_priority, stream, sizeof(uchar)* (uchar)MissionTypeEnum::AMIS_OTHER, rem);
...
//#endif
	...
	if (gCampDataVersion < 30)
		...
		fread(mission_priority, sizeof(uchar), (uchar)MissionTypeEnum::AMIS_OTHER, file);
	...
	if (gCampDataVersion < 46)
	{
		...
		memcpy(mission_priority,DefaultMissionPriority[TAT_CAS-1],sizeof(uchar)* (uchar)MissionTypeEnum::AMIS_OTHER);
	}
...
int TeamClass::SaveSize()
{
	return sizeof(VU_ID)
			...
			+ sizeof(uchar)* (uchar)MissionTypeEnum::AMIS_OTHER
			+ sizeof(uchar)*4
			+ sizeof(uchar)
			+ sizeof(uchar)
			+ sizeof(uchar)
...
int TeamClass::Save(VU_BYTE **stream)
{
	...
	memcpy(*stream, mission_priority, sizeof(uchar)* (uchar)MissionTypeEnum::AMIS_OTHER);		*stream += sizeof(uchar)* (uchar)MissionTypeEnum::AMIS_OTHER;
...
int TeamClass::Save(FILE *file)
{
	...
	if (file) 
	{
		...
		retval += fwrite(mission_priority, sizeof(uchar), (uchar)MissionTypeEnum::AMIS_OTHER, file);
...
int TeamClass::Handle(VuFullUpdateEvent *event)
{
	...
	memcpy(mission_priority, tmpTeam->mission_priority, sizeof(uchar)* (uchar)MissionTypeEnum::AMIS_OTHER);
	...
	if (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_SLAVE)
	{
		TheCampaign.Flags &= ~(ulong)CampStatusEnum::CAMP_NEED_TEAM_DATA;
...
int LoadTeams (char* scenario){
	...
	// Should make no difference to Campaign. - RH
	 ...
	 for (; i<NUM_TEAMS; i++) //RERUN tryout
	 {
		 AddTeam(i, NoRelations);
	 }
...
void LoadPriorityTables (void)
{
	...
	memset(DefaultMissionPriority,0,TAT_CAS* (uchar)MissionTypeEnum::AMIS_OTHER);
...
int GetPriority (MissionRequest mis)
{
	...
	if (mis->flags & REQF_PART_OF_ACTION)
		mission_priority = DefaultMissionPriority[mis->action_type-1][(uchar)mis->mission];
	else
	...
	if (MissionData[(uchar)mis->mission].flags & AMIS_NO_DIST_BONUS)
	...
	else
	{
		...
		if (d > 100 && !mis->action_type && mis->mission == MissionTypeEnum::AMIS_SWEEP)
		{
			priority = -1;
			return -1;
		}
	...
	if ((mis->mission == MissionTypeEnum::AMIS_CAS || mis->mission == MissionTypeEnum::AMIS_ONCALLCAS ||
		 mis->mission == MissionTypeEnum::AMIS_PRPLANCAS || mis->mission == MissionTypeEnum::AMIS_BAI) && mis->vs)
	{
	...
	strcpy(Priorities[(uchar)mis->mission].name,MissStr[(uchar)mis->mission]);
	Priorities[(uchar)mis->mission].number_queried++;
	Priorities[(uchar)mis->mission].total_score += priority;
	Priorities[(uchar)mis->mission].total_distance += distance_priority;
	Priorities[(uchar)mis->mission].total_target += target_priority;
	Priorities[(uchar)mis->mission].total_mission += mission_priority;
	Priorities[(uchar)mis->mission].total_random += random_priority;
	Priorities[(uchar)mis->mission].total_pak += pak_priority;
	Priorities[(uchar)mis->mission].total_bonus += bonus;;
	Priorities[(uchar)mis->mission].average_score = Priorities[(uchar)mis->mission].total_score / Priorities[(uchar)mis->mission].number_queried;
	Priorities[(uchar)mis->mission].average_distance = Priorities[(uchar)mis->mission].total_distance / Priorities[(uchar)mis->mission].number_queried;
	Priorities[(uchar)mis->mission].average_target = Priorities[(uchar)mis->mission].total_target / Priorities[(uchar)mis->mission].number_queried;
	Priorities[(uchar)mis->mission].average_mission = Priorities[(uchar)mis->mission].total_mission / Priorities[(uchar)mis->mission].number_queried;
	Priorities[(uchar)mis->mission].average_random = Priorities[(uchar)mis->mission].total_random / Priorities[(uchar)mis->mission].number_queried;
	Priorities[(uchar)mis->mission].average_pak = Priorities[(uchar)mis->mission].total_pak / Priorities[(uchar)mis->mission].number_queried;
	Priorities[(uchar)mis->mission].average_bonus = Priorities[(uchar)mis->mission].total_bonus / Priorities[(uchar)mis->mission].number_queried;
	RemoveFromSortedList (&Priorities[(uchar)mis->mission]);
	InsertIntoSortedList (&Priorities[(uchar)mis->mission]);
...
void TeamClass::SetMissionPriority (MissionTypeEnum i, uchar c)
{
	mission_priority[(uchar)i] = c;
