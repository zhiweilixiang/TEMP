...
CampBaseClass::CampBaseClass(VU_BYTE **stream, long *rem) : FalconEntity(VU_LAST_ENTITY_TYPE, 0)
{
	...
	if (load_log){
		fprintf (load_log, "%08x CampBaseClass ", (int)(*stream) - start_load_stream);
		fflush (load_log);
	}
	...
	if (load_log) {
		fprintf(load_log, "(%08x%08x) ", (int)share_.id_, (int)share_.entityType_); //RERUN 
		fflush (load_log);
	}
...
int CampBaseClass::Save(VU_BYTE **stream)
{
	...
	if (save_log)
	{
		fprintf(save_log, "%08x CampBaseClass (%08x%08x) ", (int)(*stream) - start_save_stream, (int)share_.id_, (int)share_.entityType_); //RERUN
		fflush (save_log);
	}
