...
UnitClass::UnitClass(VU_BYTE **stream, long *rem) : CampBaseClass (stream, rem){
	...
	if (load_log) {
		fprintf (load_log, "%08x UnitClass ", (int)(*stream) - start_load_stream);
		fflush (load_log);
	}
...
//#endif
	...
	if (load_log){
		fprintf (load_log, "%08x Waypoints ", (int)(*stream) - start_load_stream);
		fflush (load_log);
	}
...
int UnitClass::Save (VU_BYTE **stream)
{
	...
	if (save_log)
	{
		fprintf (save_log, "%08x UnitClass ", (int)(*stream) - start_save_stream);
		fflush (save_log);
	}
	...
	if (save_log)
	{
		fprintf (save_log, "%08x Waypoints ", (int)(*stream) - start_save_stream);
		fflush (save_log);
	}
...
int UnitClass::ApplyDamage (DamType d, int* str, int where, short flags)
{
	...
	while (*str > 0 && vehs && count < MAX_DAMAGE_TRIES){
		...
		if (n > 0 && uc->VehicleType[v])
		{
			...
			if (vc->DamageMod[(int)d])
			{
				// Normalize hitpoints by damage type
				hp = vc->HitPoints * 100 / vc->DamageMod[(int)d];
				// Check if high explosive damage will do more
				if ((flags & WEAP_AREA) && vc->DamageMod[(int)DamType::HighExplosiveDam] > vc->DamageMod[(int)d])
					hp = vc->HitPoints * 100 / vc->DamageMod[(int)DamType::HighExplosiveDam];
	...
	if (base != NULL && GetCurrentWaypoint() == 1){
		...
		if (base->IsObjective()){
			...
			if (brain){
				...
				brain->AddTraffic(this->Id(), AtcStatusEnum::noATC, rwIndex, nextTOTime);
			}
		}
	}
...
void UnitClass::KillUnit (void)
{
	...
	if (this == FalconLocalSession->GetPlayerFlight())
	{
		...
		{
			MonoPrint("We're killing the player flight inappropriately\n");
		}
	}
#endif
...
//		throw std::InvalidBufferException(err);
	...
	while (count) {
		...
		// Special case for tactical engagement
