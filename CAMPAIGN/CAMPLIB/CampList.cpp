...
CampBaseMap::~CampBaseMap(){ F4DestroyCriticalSection(mutex); }
		...
		while (o){
			...
			if (fseed && do_barcaps){
				...
				if (barsweep == 1){
					...
					mis.mission = MissionTypeEnum::AMIS_BARCAP;
				}
				else {
					mis.mission = MissionTypeEnum::AMIS_BARCAP2;
				}
				...
				if (!e->IsObjective()){
					...
					while (a && emit){
						...
						if (a->GetTeam() == team && !a->IsObjective()){
							...
							else if ((range > d + r) && (a->GetRadarMode() < FEC_RADAR_SEARCH_1)){
								a->SetEmitting(0);
							}
						}
						a = next;
...
int CheckObjProxyOK(int X, int Y)
{
    ...
    for (
		...
		if (F4IsBadReadPtr(o, sizeof *o)){ return 0; }
    }
    return 1;
}
...
int CheckUnitProxyOK(int X, int Y)
{
    ...
    for (
		...
		if (F4IsBadReadPtr(u, sizeof *u)){ return 0; }
    }
    return 1;
}
