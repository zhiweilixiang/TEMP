					...
					if (!HasDoneFLightData)
						{
							...
							_TCHAR Bufferb[40] = "";
							AddIndexedStringToBuffer(300+(uchar)flight_ptr->mission, Bufferb);
							sprintf(logstring, "\nMission Type: %s \n", Bufferb);
							fputs(logstring, fp);

							sprintf(logstring, "Flight Unique Id: %s\n", flight_ptr->name);
							fputs(logstring, fp);
...
void MissionEvaluationClass::FindPotentialTargets (void)
	...
	if (package_mission == MissionTypeEnum::AMIS_BARCAP || package_mission == MissionTypeEnum::AMIS_BARCAP2)
		...
		VuGridIterator*		myit = new VuGridIterator(ObjProxList,(BIG_SCALAR)GridToSim(tx),(BIG_SCALAR)GridToSim(ty),(BIG_SCALAR)GridToSim(MissionData[(uchar)package_mission].mindistance));
#else
			...
			if (wfm->dataBlock.fPilotID == pilot_data->pilot_slot)
				...
				for (wn=0; wn<pilot_data->weapon_types; wn++)
					...
					if (wfm->dataBlock.fWeaponID == windex) // && wfm->dataBlock.fireOnOff == 1)
						...
						else if (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_LIGHT)
							{
				...
				while (pilot_data)
					...
					for (wn=0; wn<pilot_data->weapon_types; wn++)
						...
						if (dtm->dataBlock.fPilotID == pilot_data->pilot_slot && dtm->dataBlock.fWeaponID == windex)
							...
							if (tmpevent || !(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_LIGHT))
								{
								// Make a new event if we need to
								if (!tmpevent)
									{
		...
		while (pilot_data)
		{
				...
				fpsm->dataBlock.pilotID == pilot_data->pilot_slot)
			{
...

				{
					...
					chat->dataBlock.size = (short)((strlen (theEvent->eventString) + 1) * sizeof (char));
...
void MissionEvaluationClass::RegisterEvent (GridIndex x, GridIndex y, int eteam, int type, _TCHAR *event)
	...
	role = MissionData[(uchar)package_mission].skill;
