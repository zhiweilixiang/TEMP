...
JoinStatusEnum		gCampJoinStatus = JoinStatusEnum::JOIN_NOT_JOINING;		// This stores what stage of loading we're currently in
...
void StartCampaignGame (int local, int game_type)
	...
	if (local)
		...
		// Join a Campaign
		gCampDataVersion = gCurrentDataVersion;	
		gCampJoinStatus = JoinStatusEnum::JOIN_REQUEST_ALL_DATA;
		...
		SendMessage(FalconDisplay.appWin,FM_JOIN_CAMPAIGN, (int)JoinStatusEnum::JOIN_REQUEST_ALL_DATA,game_type);
		}
	}
...
{
	if (remote_game && !TheCampaign.IsLoaded() && gCampJoinStatus == JoinStatusEnum::JOIN_REQUEST_ALL_DATA)
	{
		// We want the rest of the data too.
		gCampJoinStatus = JoinStatusEnum::JOIN_CAMP_DATA_ONLY;
		PostMessage(FalconDisplay.appWin,FM_JOIN_CAMPAIGN, (int)JoinStatusEnum::JOIN_CAMP_DATA_ONLY,gCampJoinGameType);
	}
}
