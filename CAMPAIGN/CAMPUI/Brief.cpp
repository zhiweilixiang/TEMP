...
int BuildDebriefString (C_Window *win, _TCHAR *brief)
	...
	ShiAssert(flight_data->mission != MissionTypeEnum::AMIS_TRAINING);
...
int GetBriefingData (int query, int data, _TCHAR *buffer, int len)
	...
	switch (query)
		...
		case GBD_PACKAGE_ELEMENT_TASK:
			...
			if (flight_ptr)
				AddIndexedStringToBuffer(300+(uchar)flight_ptr->mission, buffer);
			else
				return -1;
			break;
...
#endif
		    CBX = (short)(CLineStart + (int)strlen(CCurrentLine));
		}
	buffer[0] = 0;
	}
			...
			if (strncmp(token,"#IF",3)==0)
				...
				if (strcmp(token,"#IF_HAVE_TARGET")==0)
					...
					while (sptr && atoi(sptr))
						{
						if (atoi(sptr) == (uchar)mec->package_mission)
							stack_active[curr_stack] = 1;
						sptr = strchr(sptr,' ');
						if (sptr)
							sptr++;
					...
					while (sptr && atoi(sptr))
						{
						if (atoi(sptr) == (uchar)flight_data->mission)
							stack_active[curr_stack] = 1;
						sptr = strchr(sptr,' ');
						if (sptr)
							sptr++;
					...
					while (sptr && atoi(sptr))
						{
						if (atoi(sptr) == (uchar)flight_data->old_mission)
							stack_active[curr_stack] = 1;
						sptr = strchr(sptr,' ');
						if (sptr)
							sptr++;
...
*/
			...
			// Add all our script tokens here
				...
				AddIndexedStringToBuffer(300+ (uchar)flight_data->mission, current_line);
			else if (strcmp(token,"OLD_MISSION_NAME")==0)
				AddIndexedStringToBuffer(300+ (uchar)flight_data->old_mission, current_line);
				...
				if (sptr[0] == ' ')
					sptr++;
				if (sptr[i] == 'O')
					AddIndexedStringToBuffer(400+i*50+ (uchar)flight_data->old_mission, current_line);
				else
					AddIndexedStringToBuffer(400+i*50+ (uchar)flight_data->mission, current_line);
				}
				...
				sptr = token + 27;
				i = sptr[0] - '1';
				AddIndexedStringToBuffer(900+i*50+ (uchar)mec->package_mission, current_line);
				}
