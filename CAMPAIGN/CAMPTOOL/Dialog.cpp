...
BOOL WINAPI MissionTriggerProc(HWND hDlg,UINT message,WPARAM wParam,LPARAM lParam)   
	{
	RECT					rect;
	static int			tottype,toteam,vsteam,targettype;
	static MissionTypeEnum mission;
	...
	switch (message)
		...
		case WM_INITDIALOG:             		/* message: initialize dialog box */
			...
			for (i=0; i< (uchar)MissionTypeEnum::AMIS_OTHER; i++)
		...
		case WM_PAINT:
			...
			if (GetUpdateRect(hDlg, &rect, FALSE))
				...
				mission = (MissionTypeEnum)atoi(buffer);
				SetWindowText(GetDlgItem(hDlg,IDC_MISSIONTYPE),MissStr[(uchar)mission]);
        		EndPaint(hDlg,&ps);
				}
        	}
			break;
		case WM_COMMAND:                		/* message: received a command */
      		switch (LOWORD(wParam))
				...
				case IDC_MT_MISSIONCOMBO:
					if (HIWORD(wParam) == CBN_SELENDOK)
						mission = (MissionTypeEnum)SendMessage(GetDlgItem(hDlg,IDC_MT_MISSIONCOMBO),CB_GETCURSEL,0,0);
					break;
...
BOOL WINAPI WeatherEditProc(HWND hDlg,UINT message,WPARAM wParam,LPARAM lParam)   
	...
	switch (message)
		...
		case WM_INITDIALOG:             		/* message: initialize dialog box */
			...
			sprintf(buffer,"%5.1f",((WeatherClass*)realWeather)->temperature);
...
//				SetWindowText(GetDlgItem(hDlg,IDC_OFFENSELOSS),buffer);
				...
				for (i=0; i< (uchar)MissionTypeEnum::AMIS_OTHER; i++)
					{
					sprintf(temp,"%d  ",tempTeam->GetMissionPriority((MissionTypeEnum)i));
					strcat(buffer,temp);
					}
				...
				for (i=0; i< (uchar)MissionTypeEnum::AMIS_OTHER; i++)
					{
					sprintf(buffer,"%s:   %d",MissStr[i],tempTeam->GetMissionPriority((MissionTypeEnum)i));
				...
				sprintf(buffer,"%d",tempTeam->GetMissionPriority((MissionTypeEnum)sm));
...
//						tempTeam->offensiveLoss = j;
				...
				case IDC_MISSPRIEDIT:
					...
					j = atoi(buffer);
					if (HIWORD(wParam) == EN_KILLFOCUS)
						tempTeam->SetMissionPriority((MissionTypeEnum)sm, j);
					break;
...
BOOL WINAPI FistOfGod(HWND hDlg,UINT message,WPARAM wParam,LPARAM lParam)   
	...
	switch (message)
		{
		case WM_INITDIALOG:             		/* message: initialize dialog box */
			ReadIndexedString(300+(uchar)TheCampaign.MissionEvaluator->player_element->mission, tbuffer1, 512);
			ReadIndexedString(300+(uchar)TheCampaign.MissionEvaluator->package_mission,tbuffer2,512);
