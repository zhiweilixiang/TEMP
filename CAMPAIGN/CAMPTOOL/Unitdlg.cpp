...
//			SetWindowText(GetDlgItem(hDlg,IDC_UNIT_OPT5EDIT),buffer);
			...
			SendMessage(GetDlgItem(hDlg,IDC_UNIT_OPT1COMBO),CB_SETCURSEL,(uchar)u->GetUnitMission(),0);
	...
	//		SetWindowText(GetDlgItem(hDlg,IDC_UNIT_OPT3VAL),GetLoadoutName(u->GetUnitLoadout()));
			// Now the rest
			sprintf(buffer,"%d",(int)(u->GetUnitMissionTarget()));
			SetWindowText(GetDlgItem(hDlg,IDC_UNIT_OPT3EDIT),buffer);
			sprintf(buffer,"%d",u->GetSquadronFuel());
			SetWindowText(GetDlgItem(hDlg,IDC_UNIT_OPT4EDIT),buffer);
		...
		case IDC_UNIT_OPT1COMBO:
			if (message == CBN_SELENDOK)
				...
				if (u->IsFlight())
					...
					((Package)u)->SetPackageType((MissionTypeEnum)i);
				else
					{
					GridIndex		x,y;
					Objective		o;
				...
				for (j=0; j<6; j++)
					...
					if (inButton(&ebuttons[i][j],xPos,yPos))
						...
						if (message == WM_LBUTTONDBLCLK && E)
							{
         	   				GlobUnit = E;	aggmode = asAgg; asAgg = 1;
            				DialogBox(hInst,MAKEINTRESOURCE(IDD_UNITDIALOG1),hDlg,(DLGPROC)EditUnit);
							GlobUnit = U;	asAgg = aggmode;
       ...
       case WM_PAINT:
			...
			if (GetUpdateRect(hDlg, &rect, FALSE) && flights)
				...
				while (u && i < 16)
					...
					sprintf(buffer,"%s",MissStr[(uchar)u->GetUnitMission()]);
