...
{
	if (load_log) {
		fprintf (load_log, "%08x PackageClass ", (int)(*stream) - start_load_stream);
		fflush (load_log);
	}

	if (share_.id_.creator_ == vuLocalSession.creator_){
		PackageNS.UseId(share_.id_.num_);
		// && share_.id_.num_ > lastPackageId){
...
int PackageClass::Save (VU_BYTE **stream)
{
	...
	if (save_log)
	{
		fprintf (save_log, "%08x PackageClass ", (int)(*stream) - start_save_stream);
		fflush (save_log);
	}
...
int PackageClass::CheckNeedRequests (void)
{
	...
	if (!wait_cycles || Camp_GetCurrentTime() > takeoff + (AMIS_TAKEOFF_DELAY-1) * CampaignMinutes)
	{
		...
		if ((wait_for & (uchar)MissionTypeEnum::AMIS_BARCAP) || (wait_for & (uchar)MissionTypeEnum::AMIS_SWEEP))
		{
			// if we're still waiting on these, they arn't planned yet and probably won't be. 
			wait_for |= (uchar)MissionTypeEnum::AMIS_BARCAP | (uchar)MissionTypeEnum::AMIS_SWEEP;
			wait_for ^= (uchar)MissionTypeEnum::AMIS_BARCAP | (uchar)MissionTypeEnum::AMIS_SWEEP;
		}
		if ((package_flags & AMIS_ADDBDA) && (mis_request.priority > MINIMUM_BDA_PRIORITY))
		{
			...
			if (target)
			{
				mis.mission = MissionTypeEnum::AMIS_BDA;
				mis.who = GetTeam();
				mis.vs = target->GetTeam();
				mis.targetID = target->Id();
				mis.target_num = 255;
				mis.aircraft = MissionData[(uchar)mis.mission].str;
				mis.tot = mis_request.tot + (CampaignTime)(MissionData[(uchar)mis.mission].separation)*CampaignSeconds;
				mis.tot_type = TYPE_EQ;
				mis.roe_check = ROE_AIR_ENGAGE;
				mis.caps = caps | MissionData[(uchar)mis.mission].caps;
		...
		if (g_bTankerWaypoints)
		{
			if ((wait_for & AMIS_ADDTANKER) && (package_flags & AMIS_NEEDTANKER))
			{
				...
				while (e)
				{
					...
					if (e->GetUnitMission() != MissionTypeEnum::AMIS_TANKER && f->refuel != 0)
					{
...
int PackageClass::BuildPackage (MissionRequest mis, F4PFList assemblyList)
{
	...
	mis_request = *mis;
	interceptor = FalconNullId;
	package_flags = MissionData[(uchar)mis->mission].flags;
	escort_type = MissionData[(uchar)mis->mission].escorttype;					// 2001-09-19 M.N.
	...
	if (mis->mission == MissionTypeEnum::AMIS_AIRLIFT && mis->aircraft > 2) {
	...
	mis->aircraft = min(mis->aircraft, MissionData[(uchar)mis->mission].str);
	...
	if (target && target->IsObjective())
	{
		...
		else
		{
			...
			while (stat > 0 && tar < 5 && targets < 4 && count < 10)
			{
				...
				if (MissionData[(uchar)mis->mission].str > 2)
	...
	else if (target && target->IsUnit())
	{
		targets = 1;
		targetf[tar] = 255;
		if (mis->mission == MissionTypeEnum::AMIS_SEADSTRIKE)
		{
		...
		if (mis->mission == MissionTypeEnum::AMIS_ONCALLCAS)
		{
	...
	ls = ScoreThreatFast (mis->tx, mis->ty, GetAltitudeLevel(MissionData[(uchar)mis->mission].minalt*100), mis->who);
	hs = ScoreThreatFast (mis->tx, mis->ty, GetAltitudeLevel(MissionData[(uchar)mis->mission].maxalt*100), mis->who);
	...
	while (targets)
	{
		...
		mis->caps |= MissionData[(uchar)mis->mission].caps;
		if (!mis->aircraft)
			mis->aircraft = MissionData[(uchar)mis->mission].str;
		...
		if (flight)
		{
			...
			if (MissionData[(uchar)mis->mission].loitertime)
				mis->tot = mis_request.tot + MissionData[(uchar)mis->mission].loitertime*flights*CampaignMinutes;
			else
		...
		// This is kinda annoying, because we have to mess with the flights counter, etc..
		...
		if (flights == 1 && (package_flags & AMIS_ADDFAC))
		{
			// Add FAC directly
			newmis = *mis;
			newmis.mission = MissionTypeEnum::AMIS_FAC;
			newmis.target_num = 255;
			newmis.targetID = FalconNullId;
			newmis.tot = mis_request.tot - 5 * CampaignMinutes;
			newmis.caps |= MissionData[(uchar)newmis.mission].caps;
			newmis.aircraft = MissionData[(uchar)newmis.mission].str;
			...
			else 
			{
				...
				mis->tx = mis_request.tx;
				mis->ty = mis_request.ty;
				mis_request.mission = mis->mission = MissionTypeEnum::AMIS_PRPLANCAS;
	...
	if ((package_flags & AMIS_ADDSEAD) && (targetd & NEED_SEAD))
	{
		// Add SEAD directly
		newmis = *mis;
		newmis.mission = MissionTypeEnum::AMIS_SEADESCORT;
		...
		newmis.aircraft = min(MissionData[(uchar)newmis.mission].str, mis->aircraft);
		newmis.priority = 0;
		newmis.priority = GetPriority(&newmis);
		//		newmis.flags = REQF_USERESERVES;
		...
		if (flight)
		{
			newmis.tot = mis_request.tot + (CampaignTime)(MissionData[(uchar)newmis.mission].separation)*CampaignSeconds;
	...
	if ((package_flags & AMIS_ADDESCORT) ) // && mis->vs && TeamInfo[mis->vs]->atm->averageCAStrength > aa_strength)
	{
		...
		if (escort_type != (uchar)MissionTypeEnum::AMIS_NONE)
		newmis.mission = (MissionTypeEnum)escort_type;	// 2001-11-10 Modified by M.N. use requested escort type
		else {
			if (rand()&1) {
				newmis.mission = MissionTypeEnum::AMIS_SEADESCORT;
			}
			else {
				newmis.mission = MissionTypeEnum::AMIS_ESCORT;
			}
		}
	...
	if (package_flags & AMIS_ADDBARCAP)
	{
		...
		newmis.mission = MissionTypeEnum::AMIS_BARCAP;
		newmis.roe_check = ROE_AIR_ENGAGE;
		newmis.priority = mis->priority / 5;
		if (MissionData[(uchar)mis_request.mission].skill == ARO_GA)
	...
	if (package_flags & AMIS_ADDSWEEP)
	{
		...
		newmis.ty = mis->ty;
		newmis.match_strength = aa_strength;
		newmis.mission = MissionTypeEnum::AMIS_SWEEP;
		newmis.priority = 0;
		newmis.roe_check = ROE_AIR_ENGAGE;
		if (MissionData[(uchar)mis_request.mission].skill == ARO_GA || MissionData[(uchar)mis_request.mission].skill == ARO_REC)
			newmis.context = enemyCASAircraftPresent;
		else if (MissionData[(uchar)mis_request.mission].skill == ARO_ASW)
...
#ifdef DEBUG
	if (mis->mission == MissionTypeEnum::AMIS_BARCAP)
		gBARCAPgot++;
	if (mis->mission == MissionTypeEnum::AMIS_SWEEP)
		gSWEEPgot++;
#endif
	return PRET_SUCCESS;
}
...
int PackageClass::RecordFlightAddition (Flight flight, MissionRequest mis, int targetd)
{
	...
	if (flights == 1)
	{
		...
		if (!(MissionData[(uchar)mis->mission].flags & AMIS_TARGET_ONLY))
		{
			...
			if (!bw && MissionData[(uchar)mis->mission].flags & AMIS_NO_BREAKPT)
				bw = tw;
...
void PackageClass::FindSupportFlights (MissionRequest mis, int targetd)
{
	...
	if (TeamInfo[GetTeam()] && TeamInfo[GetTeam()]->atm && TeamInfo[GetTeam()]->atm->packageList)
	{
		...
		endTime = mis->tot + (MissionData[(uchar)mis->mission].loitertime+10)*CampaignMinutes;
		...
		while (pack)
		{
			...
			{
				if ((package_flags & AMIS_ADDAWACS) && flight->GetUnitMission() == MissionTypeEnum::AMIS_AWACS)
				{
					pack->GetUnitDestination(&px,&py);
					dist = (float)DistSqu(mis->tx,mis->ty,px,py);
					if (pmis->tot + MissionData[(uchar)MissionTypeEnum::AMIS_AWACS].loitertime*CampaignMinutes > endTime && dist < bestAWACSDist)
					{
						awacs = flight->Id();
						bestAWACSDist = dist;
					}
				}
				if ((package_flags & AMIS_ADDJSTAR) && flight->GetUnitMission() == MissionTypeEnum::AMIS_JSTAR)
				{
					pack->GetUnitDestination(&px,&py);
					dist = (float)DistSqu(mis->tx,mis->ty,px,py);
					if (pmis->tot + MissionData[(uchar)MissionTypeEnum::AMIS_JSTAR].loitertime*CampaignMinutes > endTime && dist < bestJSTARDist)
					{
						jstar = flight->Id();
						bestJSTARDist = dist;
					}
				}
				if ((package_flags & AMIS_ADDECM) && (targetd & NEED_ECM) && flight->GetUnitMission() == MissionTypeEnum::AMIS_ECM)
				{
					pack->GetUnitDestination(&px,&py);
					dist = (float)DistSqu(mis->tx,mis->ty,px,py);
					if (pmis->tot + MissionData[(uchar)MissionTypeEnum::AMIS_ECM].loitertime*CampaignMinutes > endTime && dist < bestECMDist)
					{
						ecm = flight->Id();
						bestECMDist = dist;
					}
				}
				if ((package_flags & AMIS_ADDTANKER) && flight->GetUnitMission() == MissionTypeEnum::AMIS_TANKER)
				{
					GridIndex	x,y;
					pack->GetUnitDestination(&px,&py);
					...
					if (x && y)
						...
						dist = (float)DistSqu(mis->tx,mis->ty,px,py);
					if (pmis->tot + MissionData[(uchar)MissionTypeEnum::AMIS_TANKER].loitertime*CampaignMinutes > endTime && dist < bestTANKDist)
					{
						tanker = flight->Id();
						bestTANKDist = dist;
					}
	...
	if ((package_flags & AMIS_ADDAWACS) && awacs == FalconNullId)
	{
		...
		newmis.tx = mis->tx;
		newmis.ty = mis->ty;
		newmis.mission = MissionTypeEnum::AMIS_AWACS;
	...
	if ((package_flags & AMIS_ADDJSTAR) && jstar == FalconNullId)
	{
		...
		newmis.tx = mis->tx;
		newmis.ty = mis->ty;
		newmis.mission = MissionTypeEnum::AMIS_JSTAR;
		newmis.roe_check = ROE_AIR_OVERFLY;
	...
	if ((package_flags & AMIS_ADDECM) && (targetd & NEED_ECM) && ecm == FalconNullId)
	{
		...
		newmis.tx = mis->tx;
		newmis.ty = mis->ty;
		newmis.mission = MissionTypeEnum::AMIS_ECM;
		newmis.roe_check = ROE_AIR_OVERFLY;
	...
	if ((package_flags & AMIS_ADDTANKER) && tanker == FalconNullId)
	{
		...
		newmis.mission = MissionTypeEnum::AMIS_TANKER;
		newmis.roe_check = ROE_AIR_OVERFLY;
...
void PackageClass::HandleRequestReceipt(int type, int them, VU_ID triggered_flight)
{
	...
	switch (type)
	{
			case (uchar)MissionTypeEnum::AMIS_BARCAP:
			case (uchar)MissionTypeEnum::AMIS_SWEEP:
					wait_for |= (uchar)MissionTypeEnum::AMIS_BARCAP | (uchar)MissionTypeEnum::AMIS_SWEEP;
					wait_for ^= (uchar)MissionTypeEnum::AMIS_BARCAP | (uchar)MissionTypeEnum::AMIS_SWEEP;
					...
					if (MissionData[(uchar)mis_request.mission].escorttype == (uchar)MissionTypeEnum::AMIS_NONE)
						return;
					else
					mis.mission = (MissionTypeEnum)MissionData[(uchar)mis_request.mission].escorttype;
					mis.who = GetTeam();
					mis.vs = them;
					mis.target_num = 255;
					mis.aircraft = MissionData[(uchar)mis.mission].str;
					...
					mis.tot = mis_request.tot + MissionData[(uchar)mis.mission].separation*CampaignSeconds;
					mis.tot_type = TYPE_EQ;
					mis.roe_check = ROE_AIR_ENGAGE;
					...
					if (iax > 0 && iay > 0)
					{
						...
						GetUnitDestination(&mis.tx,&mis.ty);
					if (mis.mission == MissionTypeEnum::AMIS_ESCORT || mis.mission == MissionTypeEnum::AMIS_HAVCAP)
					{
					...
					else if (mis.mission != MissionTypeEnum::AMIS_NONE)
					{
						// Request a new package (Sweep or RESCAP mostly)
						if (mis.mission == MissionTypeEnum::AMIS_SWEEP && mis_request.mission >= MissionTypeEnum::AMIS_FAC && mis_request.mission <= MissionTypeEnum::AMIS_CAS)
							mis.context = friendlyCASExpected;
						else if (mis.mission == MissionTypeEnum::AMIS_RESCAP)
					...
					break;
			case (uchar)MissionTypeEnum::AMIS_TANKER:
					responses |= PRESPONSE_TANKER;
					tanker = triggered_flight;
					break;
			case (uchar)MissionTypeEnum::AMIS_AWACS:
					responses |= PRESPONSE_AWACS;
					awacs = triggered_flight;
					break;
			case (uchar)MissionTypeEnum::AMIS_JSTAR:
					responses |= PRESPONSE_JSTAR;
					jstar = triggered_flight;
					break;
			case (uchar)MissionTypeEnum::AMIS_ECM:
...
Flight AttachFlight (MissionRequest mis, Package pack)
{
	...
	else
	{
		...
		if (!pack->GetFlights() &&
			MissionData[(uchar)mis->mission].skill == ARO_S &&
			squadron->GetRating(ARO_SB) > squadron->GetRating(ARO_S) &&
			// RV - Biker - Don't convert all to STRATBOMB
			rand()%100 > 25)
		{
			mis->mission = MissionTypeEnum::AMIS_STRATBOMB;
			pack->GetMissionRequest()->mission = MissionTypeEnum::AMIS_STRATBOMB;
		}
	}
...
void FinalizeFlight(Unit flight, int flights)
{
	...
	if (flights > 1 && !(MissionData[(uchar)flight->GetUnitMission()].flags & AMIS_DONT_COORD) && !(MissionData[(uchar)flight->GetUnitMission()].flags & AMIS_TARGET_ONLY))
		assem = 1;
	else
		assem = 0;
	w = flight->GetFirstUnitWP();
	while (w)
	{
		...
		if (w->GetWPFlags() & WPF_ASSEMBLE && !assem)
		{
			...
			w->SetWPRouteAction(MissionData[(uchar)flight->GetUnitMission()].routewp);
...
int CallInOCAStrikes (MissionRequest mis)
{
	...
	if (mis->mission == MissionTypeEnum::AMIS_SEADSTRIKE)			// Don't generate recursive SEAD missions
		return 0;
...
Flight PackageClass::GetFACFlight (void)
{
	...
	while (fac)
	{
		if (fac->GetUnitMission() == MissionTypeEnum::AMIS_FAC)
			return fac;
