...
//#ifdef KEV_ADEBUG
extern char MissStr[(uchar)MissionTypeEnum::AMIS_OTHER][16];
//#endif
...
int AirTaskingManagerClass::Task (void)
{
	...
	while (lp)
	{
		...
		if (missionsFilled >= missionsToFill && !mis->action_type && !(MissionData[(uchar)mis->mission].flags & AMIS_FLYALWAYS))
			continue;
		...
		switch (res)
		{
				case PRET_SUCCESS:
						...
						if (MissionData[(uchar)mis->mission].skill == ARO_CA)
						{
							if (pc)
						...
						if (!mis->action_type && !(MissionData[(uchar)mis->mission].flags & AMIS_FLYALWAYS))
							missionsFilled++;
						requestList->Remove(pp);
						pp = NULL;
					break;
		...
		else
		{
			...
			if (sortie_rate)
			{
				if (airbase->IsObjective())
				{
					...
					mis.target_num = 255;
					mis.mission = MissionTypeEnum::AMIS_OCASTRIKE;
				...
				else if (airbase->GetDomain() == DOMAIN_SEA)
				{
					...
					mis.targetID = airbase->Id();
					mis.target_num = 255;
					mis.mission = MissionTypeEnum::AMIS_ASHIP;
			...
			if (supplyBase == total_airbases && airbase->IsObjective() && airbase->GetType() == TYPE_AIRBASE)
			{
				...
				mis.targetID = airbase->Id();
				mis.target_num = 0;
				mis.mission = MissionTypeEnum::AMIS_AIRLIFT;
...
int AirTaskingManagerClass::BuildPackage (Package *pc, MissionRequest mis)
{
	...
	if (timeleft < MissionData[(uchar)mis->mission].min_time)
	{
...
#endif
			if (e && e->IsUnit())
				((Unit)e)->SendUnitMessage((*pc)->GetMainFlightID(),FalconUnitMessage::unitRequestMet, (uchar)mis->mission,mis->who,PRET_SUCCESS);
		}
...
{
	int				time,ls,hs,tr;
	long			misflags = MissionData[(uchar)mis->mission].flags;
	Flight			flight;
	//	Package			pack;
	...
	if (time > MissionData[(uchar)mis->mission].max_time)
		return PRET_TIMEOUT;
	...
	if (mis->mission == MissionTypeEnum::AMIS_INTERCEPT)
	{
...
int AirTaskingManagerClass::BuildSpecificDivert(Flight flight)
{
	...
	while (lp)
	{
		...
		// Check vs roll
		if (flight->GetUnitCurrentRole() != MissionData[(uchar)mis->mission].skill)
			continue;
		...
		if (hs > ls)
			hs = ls;
		if (MissionData[(uchar)mis->mission].flags & AMIS_HIGHTHREAT)
			tr = MAX_FLYMISSION_HIGHTHREAT;
		else if (MissionData[(uchar)mis->mission].flags & AMIS_NOTHREAT)
			tr = MAX_FLYMISSION_NOTHREAT;
		else
			tr = MAX_FLYMISSION_THREAT;
		if (hs > tr)
...
void AirTaskingManagerClass::ProcessRequest(MissionRequest request)
{
	...
	if (request->mission == MissionTypeEnum::AMIS_AWACS)
		FindBestLocation(request,awacsList);
	else if (request->mission == MissionTypeEnum::AMIS_JSTAR)
		FindBestLocation(request,awacsList);
	else if (request->mission == MissionTypeEnum::AMIS_ECM)
		FindBestLocation(request,ecmList);
	else if (request->mission == MissionTypeEnum::AMIS_TANKER)
	...
	else if (e->IsObjective())
	{
		...
		if (o && !o->GetSpotted(request->who)) // && o->GetObjectiveStatus() < 100)
		{
			...
			{
				request->mission = MissionTypeEnum::AMIS_RECON;
	...
	else if (e->IsUnit() && (e->IsBattalion() || e->IsBrigade()))
	{
		...
		if (u && !u->GetSpotted(request->who) && !FriendlyTerritory (request->tx, request->ty, request->who))
		{
			...
			if (request->mission != MissionTypeEnum::AMIS_SEADSTRIKE && !(rand()%3))
			{
				request->mission = MissionTypeEnum::AMIS_SAD;
	...
	else if (e->IsUnit() && e->IsTaskForce())
	{
		...
		if (u && !u->GetSpotted(request->who))
		{
			...
			if ((rand()%3))
			{
				request->mission = MissionTypeEnum::AMIS_PATROL;
				request->targetID = FalconNullId;
		...
		while (pack)
		{
							...
							pmis->mission == request->mission && !pack->Aborted()) // KCK: Probably want '&& !mission finished' too
			{
				...
				else
				{
					...
					if (abs((int)pmis->tot - (int)request->tot) < MissionData[(uchar)request->mission].mintime * CampaignMinutes
									&& Distance(request->tx,request->ty,px,py) < MissionData[(uchar)request->mission].mindistance)
					{
						...
						if (request->flags & REQF_NEEDRESPONSE)
						{
							...
							if (e->IsUnit())
								((Unit)e)->SendUnitMessage(pack->GetMainFlightID(),FalconUnitMessage::unitRequestMet, (uchar)request->mission,request->who,0);
						}
						return;
					}
				}
	...
	while (lp)
	{
		...
		else
		{
			// Check if this planned/in-progress mission is similar to the one requested
			if (abs((int)pmis->tot - (int)request->tot) < MissionData[(uchar)request->mission].mintime * CampaignMinutes
							&& Distance(request->tx,request->ty,pmis->tx,pmis->ty) < MissionData[(uchar)request->mission].mindistance)
			{
	...
	*mis = *request;
	...
	if (!mis->action_type && !(MissionData[(uchar)mis->mission].flags & AMIS_FLYALWAYS) && rand()%100 < TeamInfo[mis->who]->GetGroundAction()->actionTempo)
		missionsToFill++;
...
Squadron AirTaskingManagerClass::FindBestAir(MissionRequest mis, GridIndex bx, GridIndex by)
{
	...
	role = MissionData[(uchar)mis->mission].skill;
...

	{
		...
		while (ns)
		{
			...
			if ((MissionData[(uchar)mis->mission].flags & AMIS_NPC_ONLY) && TheCampaign.IsValidAircraftType(sq))
				continue;
			...
			sq->GetLocation(&X,&Y);
			d = Distance(X,Y,mis->tx,mis->ty);
			if (!(MissionData[(uchar)mis->mission].flags & AMIS_FUDGE_RANGE))
			{
				float		db = 0.0F;
				// Add loiter time to the estimated distance.
				if (MissionData[(uchar)mis->mission].loitertime > 0)
					db = (MissionData[(uchar)mis->mission].loitertime * speed) / 60.0F;
				
				// RV - Biker - Reduce loiter time for short range AC
				if (d+db > sq->GetUnitRange()) {
					MissionData[(uchar)mis->mission].loitertime = min(45, MissionData[(uchar)mis->mission].loitertime);
					db = (MissionData[(uchar)mis->mission].loitertime * speed) / 60.0F;
					if (d+db > sq->GetUnitRange())
						continue;
				}
				// Airlift missions must come from another airbase
				if (mis->mission == MissionTypeEnum::AMIS_AIRLIFT && d < 40)
					continue;
			}

			// Check speed vs required
			if (mis->speed && (mis->flags & AMIS_MATCHSPEED || MissionData[(uchar)mis->mission].flags & AMIS_MATCHSPEED))
			{
			...
			memset(mis->slots,255,4);
			if (MissionData[(uchar)mis->mission].flags & AMIS_DONT_USE_AC)
				av = mis->aircraft;
			else
				av = sq->FindAvailableAircraft(mis);
...
Flight AirTaskingManagerClass::FindBestAirFlight(MissionRequest mis)
{
	...
	while (nu)
	{
		...
		if (MissionData[(uchar)cf->GetUnitMission()].skill != role)
			continue;
		...
		if (cf->GetUnitMission() != MissionTypeEnum::AMIS_ALERT && (!cf->GetCurrentUnitWP() || cf->GetCurrentUnitWP()->GetWPAction() == WP_TAKEOFF))
			continue;
		...
		//continue;
		if (cf->GetUnitMission() == MissionTypeEnum::AMIS_ALERT && d > 250.0f/*MAX_SCRAMBLE_DISTANCE*/)
			continue;
		...
		if (role == ARO_CA)
		{
			...
			if (cf->GetUnitMission() == MissionTypeEnum::AMIS_BARCAP2)
...
void AirTaskingManagerClass::ScheduleAircraft(VU_ID abid, WayPoint w, int aircraft)
{
	...
	if (abe->IsObjective())
	{
		...
		ShiAssert (sortie_rate <= 2);//RERUN observed to be 100 or more sometimes... 
	}
	else if (abe->IsUnit())
		sortie_rate = 1;
	if (sortie_rate < 2)
...
int AirTaskingManagerClass::FindNearestActiveTanker(GridIndex *x, GridIndex *y, CampaignTime *time)
{
	...
	if (packageList)
	{
		...
		while (p)
		{
			if (p->GetMissionRequest()->mission == MissionTypeEnum::AMIS_TANKER && p->GetMissionRequest()->tot < *time && p->GetMissionRequest()->tot+(MissionData[(uchar)MissionTypeEnum::AMIS_TANKER].loitertime*CampaignMinutes) > *time)
			{
				...
				if (d < bd)
				{
					...
					bt = p->GetMissionRequest()->tot + (MissionData[(uchar)MissionTypeEnum::AMIS_TANKER].loitertime*CampaignMinutes);
				}
			}
...
int RequestSARMission (FlightClass* flight)
{
	...
	mis.tot_type = TYPE_NE;
	mis.mission = MissionTypeEnum::AMIS_SAR;
...
void RequestIntercept (FlightClass* enemy, int who, RequIntHint hint)
{
	...
	if (hint != RI_HELP)
	{
		MissionTypeEnum test = enemy->GetUnitMission();
		if (test != MissionTypeEnum::AMIS_OCASTRIKE &&
						test != MissionTypeEnum::AMIS_INTSTRIKE &&
						test != MissionTypeEnum::AMIS_STRIKE &&
						test != MissionTypeEnum::AMIS_DEEPSTRIKE &&
						test != MissionTypeEnum::AMIS_STSTRIKE &&
						test != MissionTypeEnum::AMIS_STRATBOMB &&
						test != MissionTypeEnum::AMIS_SEADSTRIKE)
			return;
	}
	...
	mis.tot = Camp_GetCurrentTime();
	mis.tot_type = TYPE_NE;
	mis.mission = MissionTypeEnum::AMIS_INTERCEPT;
	mis.roe_check = ROE_AIR_ATTACK;
...
int TargetAllSites (Objective po, int action, int team, CampaignTime startTime)
{
			...
			if (otarget->GetObjectiveStatus() > 30 && GetRoE(team,otarget->GetTeam(),ROE_AIR_ATTACK) == ROE_ALLOWED){
				...
				if (o == po){
					...
					else if (otarget->GetType() == TYPE_COM_CONTROL)
					{
						...
						mis.tot = startTime + 15*CampaignMinutes;
						mis.mission = MissionTypeEnum::AMIS_STRIKE;
						if (action == AACTION_OCA)
							mis.context = AirActionOCA;
						else
							mis.context = AirActionPrepAB;
									...
									otarget->GetType() == TYPE_POWERPLANT)
					{
						...
						mis.mission = MissionTypeEnum::AMIS_INTSTRIKE;
		...
		while (utarget){
			if (
				...
				if (o == po){
					...
					mis.mission = MissionTypeEnum::AMIS_SEADSTRIKE;
					mis.priority = 0;
	...
	mis.vs = po->GetTeam();
	mis.mission = MissionTypeEnum::AMIS_SWEEP;
