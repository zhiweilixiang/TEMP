...
BrigadeClass::BrigadeClass(VU_BYTE **stream, long *rem) : GroundUnitClass(stream, rem)
{
	if (load_log)
	{
		fprintf (load_log, "%08x BrigadeClass ", (int)(*stream) - start_load_stream);
		fflush (load_log);
	}
...
int BrigadeClass::Save (VU_BYTE **stream)
	...
	if (save_log)
	{
		fprintf (save_log, "%08x BrigadeClass ", (int)(*stream) - start_save_stream);
		fflush (save_log);
	}
...
void BrigadeClass::SetUnitOrders (int neworders, VU_ID oid)
	...
	if (o->IsNearfront())
		...
		mis.targetID = Id();
		mis.mission = MissionTypeEnum::AMIS_RECONPATROL;
