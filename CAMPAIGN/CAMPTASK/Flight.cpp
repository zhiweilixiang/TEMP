...
FlightClass::FlightClass(ushort type, Unit parent, Unit squad) : AirUnitClass(type, GetIdFromNamespace(FlightNS))
{
	...
	loadout = NULL;
	loadouts = 0;
	mission = old_mission = MissionTypeEnum::AMIS_NONE;
	last_direction = Here;					
	priority = 0;						
	mission_id = 0;
	eval_flags = 0;
...
FlightClass::FlightClass(VU_BYTE **stream, long *rem) : AirUnitClass(stream, rem)
{
	...
	if (load_log)
	{
		fprintf (load_log, "%08x FlightClass ", (int)(*stream) - start_load_stream);
		fflush (load_log);
	}
	...
	if (mission == MissionTypeEnum::AMIS_TANKER)
	{
...
int FlightClass::Save (VU_BYTE **stream)
{
	...
	if (save_log)
	{
		fprintf (save_log, "%08x FlightClass ", (int)(*stream) - start_save_stream);
		fflush (save_log);
	}
...
int FlightClass::MoveUnit(CampaignTime time){
	...
	if (GetUnitMission() > MissionTypeEnum::AMIS_SEADESCORT && GetUnitMission() < MissionTypeEnum::AMIS_FAC){
...
int FlightClass::GetUnitCurrentRole() const {
	return MissionData[(uchar)mission].skill;
}
...
int FlightClass::Reaction (CampEntity e, int knowledge, float range)
{
	...
	if (GetUnitMission() == MissionTypeEnum::AMIS_NONE)
		return 0;
	...
	if (score && e->IsFlight() && ((Flight)e)->GetUnitMission() == MissionTypeEnum::AMIS_FAC)
		return 1;
...
int FlightClass::ChooseTactic (void)
{
	...
	if (GetUnitTactic() != ATACTIC_ABORT && tid == ATACTIC_ABORT)
	{
		...
		if (MissionData[(uchar)mission].flags & AMIS_EXPECT_DIVERT)
		{
...
int FlightClass::CheckTactic (int tid)
{
	...
	if (!haveWeaps && tid >= ATACTIC_ENGAGE_DEF && MissionData[(uchar)mission].flags & AMIS_NO_TARGETABORT)
		haveWeaps++;
	...
	if (!CheckEngaged(tid,Engaged()))
		return 0;
	if (!CheckAction(tid,(uchar)ourMission))
		return 0;
...
void FlightClass::SetUnitMission (MissionTypeEnum mis)
{
	if (mission != mis)
	{
		mission = mis;
		...
		if (IsTacan() && mis != MissionTypeEnum::AMIS_TANKER)
		{
			// Cancel tacan in case of abort
			SetTacan(0);
		}
		else if(!IsTacan() && mis == MissionTypeEnum::AMIS_TANKER)
		{
			SetTacan(1);
		}
	}
...
int FlightClass::ChooseTarget()
{
			...
			if ( !e->IsDead() && roeg[e->GetTeam()] == ROE_ALLOWED){						
				...
				if (((BattalionClass *)e)->class_data->RadarVehicle < 255 && e->IsEmitting()){
					...
					// If the radar vehicle index is more than 15, it's an AAA.
						...
						if (GetUnitMission() == MissionTypeEnum::AMIS_SEADESCORT){
			...
			if (spot || d <70.0f && LastawackWarning + 20000.0f < SimLibElapsedTime){
				...
				if (!mAWACSavail){
					...
					while (nu && !mAWACSavail){
						...
						if (cf->GetUnitMission() == MissionTypeEnum::AMIS_AWACS && cf->GetTeam() == GetTeam()){
							mAWACSavail = true;
						}
					}
				}

				if (GetUnitMission() == MissionTypeEnum::AMIS_AWACS){
	...
	if (
		(GetUnitMission() == MissionTypeEnum::AMIS_JSTAR || GetUnitCurrentRole() == ARO_GA || GetUnitCurrentRole() == ARO_FAC)
		/*&& !react_against*/ 
	){
		ground_search_dist = GetDetectionRange(Tracked);
		if (ground_search_dist > MAX_GROUND_SEARCH && GetUnitMission() != MissionTypeEnum::AMIS_JSTAR)
...
int FlightClass::BuildMission (MissionRequestClass *mis)
{
	...
	else
	{
		...
		if (!length && mis->mission != MissionTypeEnum::AMIS_ALERT)
			return PRET_ABORTED;
	...
	if ((MissionData[(uchar)mis->mission].flags & AMIS_EXPECT_DIVERT) && !Diverted() && mis->mission != MissionTypeEnum::AMIS_ONCALLCAS){
		SetUnitPriority(0);
	}
	...
	if (mission != MissionTypeEnum::AMIS_ALERT)
	{
		int		timedelta;
	...
	// RV - Biker - Check if we've set some messed up WP altitude
	float x, y, z;
	float groundAlt;
	int adjustedAlt;
	...
	if (fuelNeeded > class_data->Fuel)
	{
		...
		if (fuelNeeded > fuelAvail && !(MissionData[(uchar)mis->mission].flags & AMIS_FUDGE_RANGE))
		{
	...
	//	if (target)
	if (target && mission != MissionTypeEnum::AMIS_SEADESCORT)
	{
	...
	switch (GetUnitMission())
	{
		...
		case MissionTypeEnum::AMIS_ASW:
			...
			needweaps = 1;
			SetUnitMission(MissionTypeEnum::AMIS_SEADSTRIKE);
			break;
		case MissionTypeEnum::AMIS_ASHIP:
	...
	if (GetUnitMission() != MissionTypeEnum::AMIS_SAR) {
	...
	if (GetUnitMission() == MissionTypeEnum::AMIS_ONCALLCAS ||
		GetUnitMission() == MissionTypeEnum::AMIS_PRPLANCAS ||
		GetUnitMission() == MissionTypeEnum::AMIS_SAD ||
		GetUnitMission() == MissionTypeEnum::AMIS_INT ||
		GetUnitMission() == MissionTypeEnum::AMIS_BAI ||
		GetUnitMission() == MissionTypeEnum::AMIS_ASW)
	{
		for (int i = 0; i < loadouts; i++) {
			// Don't count gun (HP 0)
			for (int hp = 1; hp <= lastHP; hp++) {
	...
	if (MissionData[(uchar)mis->mission].skill == ARO_S && !mis->action_type)
	{
		if (squad->class_data->Flags & VEH_STEALTH)
			SetUnitMission(MissionTypeEnum::AMIS_STSTRIKE);
		else if (DistanceToFront(mis->tx,mis->ty) > 100.0F)
			SetUnitMission(MissionTypeEnum::AMIS_DEEPSTRIKE);
	}
...
int FlightClass::DetectVs (CampEntity e, float *d, int *combat, int *spot, int *estr)
{
	...
	*spot = 0;
	...
	if (
		...
		GetUnitMission() >= MissionTypeEnum::AMIS_SEADSTRIKE &&
		GetUnitMission() <= MissionTypeEnum::AMIS_ECM &&
...
int FlightClass::HasWeapons (void)
{
	...
	MissionTypeEnum oldmission = ourMission;
	int oldrole = role;
	...
	if (e)
	{
		...
		if (role != ARO_CA && theirDomain == DOMAIN_AIR)
		{
			if (ourRange < MIN_IGNORE_RANGE)
			{
				...
				role = ARO_CA;
				ourMission = MissionTypeEnum::AMIS_SWEEP;
			}
		}
	}
...
{
	if (mission == MissionTypeEnum::AMIS_ECM)
		return TRUE;
	return FALSE;
}
#include "atcbrain.h"
...
void FlightClass::ClearAssignedTarget (void)
{
	if (assigned_target != FalconNullId)
	{
		...
		PlayDivertRadioCalls (NULL, MissionTypeEnum::AMIS_NONE, this, 1, AWACSMsgEnum::DIVERT_CANCELED);
	}
	assigned_target = FalconNullId;
	SetOverrideWP(NULL);
	priority = 0;
...
void FlightClass::WriteDirty (unsigned char **stream)
{
	...
	*(unsigned short *)ptr = dirty_flight;
	...
	{
		*(MissionTypeEnum*)ptr = mission;
		ptr += sizeof (MissionTypeEnum);
	}
