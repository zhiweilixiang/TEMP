...
extern int HDelta[7];
extern MissionDataType MissionData [(uchar)MissionTypeEnum::AMIS_OTHER];
...
GroundUnitClass::GroundUnitClass(VU_BYTE **stream, long *rem) : UnitClass(stream, rem)
{
	...
	if (load_log) {
		fprintf (load_log, "%08x GroundUnitClass ", (int)(*stream) - start_load_stream);
		fflush (load_log);
	}
...
int GroundUnitClass::Save (VU_BYTE **stream)
{
	...
	if (save_log)
	{
		fprintf (save_log, "%08x GroundUnitClass ", (int)(*stream) - start_save_stream);
		fflush (save_log);
	}
...
int BuildGroundWP (Unit u)
{
	...
	for (;i<path.GetLength(); i++){
		...
		time += TimeToArrive(cost,(float)speed);
		if (!didcas && (time - Camp_GetCurrentTime())/CampaignMinutes < MissionData[(uchar)MissionTypeEnum::AMIS_BAI].max_time){
		...
		//	}
			if (n->GetType() == TYPE_ROAD || n->GetType() == TYPE_INTERSECT)
			{
				...
				//				else
				mis.targetID = u->Id();
				mis.mission = MissionTypeEnum::AMIS_INT;
				mis.roe_check = ROE_AIR_ATTACK;
				if (u->GetUnitOrders() == GORD_CAPTURE)
...
int RequestCAS (int team, Unit target)
{
	...
	while (u)
	{
		if (u->GetDomain() == DOMAIN_AIR && u->GetTeam() == team && u->GetUnitCurrentRole() == ARO_GA && u->GetUnitPriority() == 0)
		{
			...
			mis.mission = MissionTypeEnum::AMIS_CAS;
			if (target->Engaged())
				bonus += 20;
			if (target->Combat())
				bonus += 20;
...
void RequestOCCAS (Unit u, GridIndex x, GridIndex y, CampaignTime time)
{
	...
	if (timeleft < MissionData[(uchar)MissionTypeEnum::AMIS_ONCALLCAS].min_time || timeleft > MissionData[(uchar)MissionTypeEnum::AMIS_ONCALLCAS].max_time)
	...
	mis.targetID = u->Id();
	mis.mission = MissionTypeEnum::AMIS_ONCALLCAS;
...
void RequestBAI (Unit u, GridIndex x, GridIndex y, CampaignTime time)
{
	...
	timeleft = (int) ((time - Camp_GetCurrentTime())/CampaignMinutes);
	if (timeleft < MissionData[(uchar)MissionTypeEnum::AMIS_BAI].min_time || timeleft > MissionData[(uchar)MissionTypeEnum::AMIS_BAI].max_time)
		return;		// Not in required time parameters
	mis.tot = time;
	mis.vs = u->GetTeam();
	mis.tot_type = TYPE_GE;
	...
	mis.targetID = u->Id();
	mis.mission = MissionTypeEnum::AMIS_BAI;
	if (u->GetUnitCurrentRole() == GRO_ATTACK)
		mis.context = enemyUnitAdvance;
	else
		mis.context = enemyUnitMove;
...
int RequestAirborneTransport (Unit u)
{
	...
	mis.requesterID = u->Id();
	mis.mission = MissionTypeEnum::AMIS_AIRCAV;
