...
{
	if (load_log)	{
		fprintf (load_log, "%08x SquadronClass ", (int)(*stream) - start_load_stream);
		fflush (load_log);
	}
	...
	else
	{
		...
		{
			squadron_patch = AssignUISquadronID(GetUnitNameID()); //RERUN test
		}
	}
	last_resupply_time = 0;	
	last_resupply = 0;		
...
int SquadronClass::Save (VU_BYTE **stream)
{
	...
	if (save_log)
	{
		fprintf (save_log, "%08x SquadronClass ", (int)(*stream) - start_save_stream);
		fflush (save_log);
	}
	...
	if (g_bScramble)
	{
		...
		if (rating[ARO_CA] > 4)
		{
			...
			if (want_alert)
			{
				...
				mis.mission = MissionTypeEnum::AMIS_ALERT;
