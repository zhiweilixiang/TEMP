...
int Detected(Unit u, FalconEntity *e, float *range)
	...
	// KCK: Someday, I should make this range squared - but it'd affect a lot of stuff
...
//noprint			MonoPrint("Unit %d taking off at time %d / %d.\n", u->GetCampID(), Camp_GetCurrentTime(), w->GetWPArrivalTime());
		...
		case WP_LAND:		
			...
			if (u->GetUnitMission() == MissionTypeEnum::AMIS_AIRLIFT)
				{
				int		team = u->GetTeam();
		...
		case WP_PICKUP:
			// Pick up a unit
			if (u->GetUnitMission() == MissionTypeEnum::AMIS_AIRCAV && !u->Cargo())
				u->LoadUnit(NULL);
			break;
		default:
            break;
	...
	if ((w->GetWPFlags() & WPF_REPEAT) || (w->GetWPFlags() & WPF_REPEAT_CONTINUOUS))
		...
		if (Camp_GetCurrentTime() > w->GetWPDepartureTime() && !(w->GetWPFlags() & WPF_REPEAT_CONTINUOUS))
			...
			u->AdjustWayPoints();
			if (u->GetUnitMission() == MissionTypeEnum::AMIS_AWACS)
				{
				// AWACS off station
	...
	if (w->GetWPFlags() & WPF_CP)
		{
		if (u->GetUnitPriority() > 0 && u->GetUnitMission() == MissionTypeEnum::AMIS_AWACS)
			{
			// AWACs on station
			msg = new FalconRadioChatterMessage(FalconNullId, FalconLocalGame);
			msg->dataBlock.from = u->Id();
		...
		else if (u->GetUnitMission() == MissionTypeEnum::AMIS_TANKER && !u->IsTacan())
			{
			// Tanker on station
			u->SetTacan(1);
			}
