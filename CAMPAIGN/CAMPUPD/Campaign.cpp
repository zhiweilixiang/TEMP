...
int	VisualDetectionRange[(int)DamType::OtherDam] = { 12, 4, 5, 5, 8, 16, 10, 5 };
...
uchar			DefaultDamageMods[(int)DamType::OtherDam+1] = { 0, 100, 100, 0, 0, 100, 100, 0, 0, 0, 0 };
							...
							if (airbase && airbase->IsObjective() && airbase->brain){
									...
									if(info){
										...
										if(TheCampaign.CurrentTime >= minDeagTime){
											...
											for(int i = 0; i < numVeh; i++){
												airbase->brain->AddTraffic(u->Id(), AtcStatusEnum::noATC, rwindex, nextTOTime);
								...
								if (u->IsSetFalcFlag(FEC_PLAYER_ENTERING|FEC_HASPLAYERS) && u->IsFlight())
								{
									...
									if (((Flight)u)->GetEvalFlags() & FEVAL_START_COLD){
										...
										minDeagTime -= CampaignMinutes * PlayerOptionsClass::TAXI_MINUTES;
									}
	...
	else if (!gCompressTillTime || lastchoice != PlayerOptions.GetStartFlag())
	{
		...
		switch (PlayerOptions.GetStartFlag()) {
				...
				case PlayerOptionsClass::START_TAXI:
						// Booster 2004/10/12 Taxi takeoff time option
						gCompressTillTime = sTakeoffTime - PlayerOptionsClass::TAXI_MINUTES * CampaignMinutes;
						break;
	...
	if (w && w->GetWPAction() == WP_TAKEOFF)
	{
		...
		switch (PlayerOptions.GetStartFlag()) {
				case PlayerOptionsClass::START_RAMP:
						...
						break;
				case PlayerOptionsClass::START_TAXI:
						gLaunchTime -= PlayerOptionsClass::TAXI_MINUTES * CampaignMinutes;	// Booster 2004/10/12 Taxi takeoff time option	
						break;
				case PlayerOptionsClass::START_RUNWAY:
						if (gCommsMgr && gCommsMgr->Online())
...
unsigned int __stdcall HandleCampaignThread (void)
{
	...
	TheCampaign.Flags |= (ulong)CampStatusEnum::CAMP_RUNNING;
	while (ThreadManager::campaign_active()){
		...
		// 20 ms at most
		//RERUN test so let's remove it, CTDs detected here anyway...
		...
		if (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_SUSPEND_REQUEST){
			// Someone's asked us to suspend
			TheCampaign.Flags |= (ulong)CampStatusEnum::CAMP_SUSPENDED;
			TheCampaign.Flags ^= (ulong)CampStatusEnum::CAMP_SUSPEND_REQUEST;
		}
...
#define NEW_UPDATE_HANDLE_CAMPAIGN 1
		...
		if ((TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_SUSPENDED) || (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_PAUSE)){
		...
		if (deltatime > 0 && (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_LOADED)){
			...
			if (!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_LIGHT) && TheCampaign.IsMaster()){
...
void DoCampaignLoop (int startup){
	...
	CheckDivertStatus (AWACSMsgEnum::DIVERT_NO_DIVERT);
...
{
	TheCampaign.Flags |= (ulong)CampStatusEnum::CAMP_TACTICAL_PAUSE;
}
