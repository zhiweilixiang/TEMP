...
void WeatherClass::ReceiveWeather(FalconWeatherMessage* message)
{
	...
	if(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_NEED_WEATHER)
	{
		GenerateClouds(FALSE);
		TheCampaign.Flags &= ~(ulong)CampStatusEnum::CAMP_NEED_WEATHER;
	}
	...
	if(type != game_Campaign && type != game_PlayerPool ){
		...
		//			UpdateCondition((int)utemp);
			...
			else
			{
				...
				data_ptr); data_ptr += sizeof (CampaignTime);
				temperature = *((float *) data_ptr); 
				data_ptr += sizeof (float);
