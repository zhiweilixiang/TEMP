...
void instant_action::create_player_flight (void)
{
	...
	player_flight->SetUnitMission (MissionTypeEnum::AMIS_SWEEP);
...
//	new_flight->SetInPackage(1);
	...
	{
		new_flight->SetUnitMission (MissionTypeEnum::AMIS_NONE);
	}
	else if (data.ground)
	{
		new_flight->SetUnitMission (MissionTypeEnum::AMIS_SAD);
	}
	else
	{
		new_flight->SetUnitMission (MissionTypeEnum::AMIS_SWEEP);
	}
	...
	if ((data.type == ia_kc10) && (data.side == 1)) // allied kc10
	{
		if (!player_flight->GetUnitPackage())
		{
			...
			new_package->GetMissionRequest()->mission = MissionTypeEnum::AMIS_NONE;
			...
			new_flight->SetUnitMission (MissionTypeEnum::AMIS_TANKER);
		}
		else {
#if VU_ALL_FILTERED
