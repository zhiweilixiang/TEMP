...
// sfr: ok we go crazy now, no more mutexes
...
F4THREADHANDLE CampaignClass::InitCampaign (FalconGameType gametype, FalconGameEntity *joingame)
{
	...
	Flags |= (ulong)CampStatusEnum::CAMP_THEATER_LOADED;
...
#ifdef CAMPTOOL
	// KCK: Windows is so messed up... I don't even want to start explaining
	// why I have to do this..  sigh.
	CampLeaveCriticalSection();
	if (!(Flags & (ulong)CampStatusEnum::CAMP_LIGHT)){
...
int CampaignClass::NewCampaign (FalconGameType gametype, char *savefile)
{
	...
	Flags |= (ulong)CampStatusEnum::CAMP_LOADED;
	CampLeaveCriticalSection();
	TheCampaign.Resume();
	return 1;
}
...
int CampaignClass::LoadCampaign (FalconGameType gametype, char *savefile)
{
	...
	switch (gametype)
	{
		...
		case game_Dogfight:
		{
			Flags |= (ulong)CampStatusEnum::CAMP_LIGHT;
			DisposeEventLists();
			break;
		}
		case game_TacticalEngagement:
		{
			Flags |= (ulong)CampStatusEnum::CAMP_TACTICAL;
			DisposeEventLists();
			break;
		}
	}
	...
	if (!(Flags & (ulong)CampStatusEnum::CAMP_LIGHT) && !(Flags & (ulong)CampStatusEnum::CAMP_TACTICAL)){
...
int CampaignClass::JoinCampaign (FalconGameType gametype, FalconGameEntity *game)
{
	...
	if (Flags & (ulong)CampStatusEnum::CAMP_NEED_MASK)
	{
		return 0;
		// We're resuming a previous request.
		ShiAssert (0);		// KCK: I don't want to do this anymore. These are sent reliably
		need_from_master = Flags & ((ulong)CampStatusEnum::CAMP_NEED_MASK & ~(ulong)CampStatusEnum::CAMP_NEED_ENTITIES);
		need_from_all = Flags & ((ulong)CampStatusEnum::CAMP_NEED_MASK & (ulong)CampStatusEnum::CAMP_NEED_ENTITIES);
		...
		if (need_from_master)
		{
			...
			if (need_from_master & (ulong)CampStatusEnum::CAMP_NEED_OBJ_DELTAS)
				camprequest->dataBlock.size += FS_MAXBLK/8;
			if (need_from_master & (ulong)CampStatusEnum::CAMP_NEED_UNIT_DATA)
				camprequest->dataBlock.size += FS_MAXBLK/8;
	...
	else
	{
		...
		Flags &= ~(ulong)CampStatusEnum::CAMP_NEED_MASK;

		if (Flags & (ulong)CampStatusEnum::CAMP_LIGHT)
		{
			need_from_master = (ulong)CampStatusEnum::CAMP_NEED_PERSIST | (ulong)CampStatusEnum::CAMP_NEED_OBJ_DELTAS | (ulong)CampStatusEnum::CAMP_NEED_UNIT_DATA;
			need_from_all = 0;
		...
		{
			need_from_master = (ulong)CampStatusEnum::CAMP_NEED_WEATHER | (ulong)CampStatusEnum::CAMP_NEED_PERSIST | (ulong)CampStatusEnum::CAMP_NEED_PRIORITIES | 
				(ulong)CampStatusEnum::CAMP_NEED_OBJ_DELTAS | (ulong)CampStatusEnum::CAMP_NEED_TEAM_DATA | (ulong)CampStatusEnum::CAMP_NEED_UNIT_DATA | 
				(ulong)CampStatusEnum::CAMP_NEED_VC;
			need_from_all = 0;
			//			need_from_all = CAMP_NEED_ENTITIES;
			Flags |= need_from_master | need_from_all;
			if (!LoadPilotInfo(TheCampaign.Scenario))
		...
		Flags |= (ulong)CampStatusEnum::CAMP_SLAVE;
		Flags |= (ulong)CampStatusEnum::CAMP_ONLINE;
	...
	Flags &= ~(ulong)CampStatusEnum::CAMP_NEED_ENTITIES;

	MonoPrint ("CampaignClass::JoinCampaign exit\n");
...
{
	if (!IsLoaded() || (Flags & (ulong)CampStatusEnum::CAMP_NEED_MASK))
		return 0;
...
{
	ulong still_needed = Flags & (ulong)CampStatusEnum::CAMP_NEED_MASK;
...
int CampaignClass::SaveCampaign (FalconGameType gametype, char *savefile, int save_mode)
{
	...
	if (!IsLoaded() || (Flags & (ulong)CampStatusEnum::CAMP_LIGHT && save_mode != CAMP_SAVE_LIGHT))
		return 0;
...
void CampaignClass::EndCampaign (void){
	...
	TheCampaign.Flags |= (ulong)CampStatusEnum::CAMP_SHUTDOWN_REQUEST;
...
#define JOIN_AT_END 1
	...
	if (Flags & (ulong)CampStatusEnum::CAMP_LOADED){
		if (!(Flags & (ulong)CampStatusEnum::CAMP_LIGHT)){
			FreeTheaterTerrain();
		...
		if (!(Flags & (ulong)CampStatusEnum::CAMP_ONLINE)){
	...
	gNavigationSys = NULL;
	gTacanList = NULL;
	Flags = (ulong)CampStatusEnum::CAMP_RUNNING;
	InMainUI = true;
...
int CampaignClass::Decode (VU_BYTE **stream, long *rem) {
	...
	// Now we're preloaded
	Flags |= (ulong)CampStatusEnum::CAMP_PRELOADED;
...
int CampaignClass::Encode (VU_BYTE **stream)
{
	...
	// Encode the data
	memcpy(buffer, &CurrentTime, sizeof(CampaignTime));			buffer += sizeof(CampaignTime);
	memcpy(buffer, &TE_StartTime, sizeof(CampaignTime));		buffer += sizeof(CampaignTime);
	memcpy(buffer, &TE_TimeLimit, sizeof(CampaignTime));		buffer += sizeof(CampaignTime);
	memcpy(buffer, &TE_VictoryPoints, sizeof(long));			buffer += sizeof(long);

	memcpy(buffer, &TE_type, sizeof(long));						buffer += sizeof(long);
	memcpy(buffer, &TE_number_teams, sizeof(long));				buffer += sizeof(long);
	memcpy(buffer, TE_number_aircraft, sizeof(long) * 8);		buffer += sizeof(long) * 8;
	memcpy(buffer, TE_number_f16s, sizeof(long) * 8);			buffer += sizeof(long) * 8;
	memcpy(buffer, &TE_team, sizeof(long));						buffer += sizeof(long);
	memcpy(buffer, TE_team_pts, sizeof(long) * 8);				buffer += sizeof(long) * 8;
	memcpy(buffer, &TE_flags, sizeof(long));					buffer += sizeof(long);

	for (loop = 0; loop < 8; loop ++)
	{
		if (TeamInfo[loop])
		{
			memcpy(buffer, &TeamInfo[loop]->teamFlag, 1);		buffer += 1;
			memcpy(buffer, &TeamInfo[loop]->teamColor, 1);		buffer += 1;
			memcpy(buffer, &TeamInfo[loop]->name, 20);			buffer += 20;
			memcpy(buffer, &TeamInfo[loop]->teamMotto, 200);		buffer += 200;
		}
		else
		{
			memset (buffer, 0, 1 + 1 + 20 + 200);				buffer += 1 + 1 + 20 + 200;
		}
	}

	// Don't bother with tasking times, they'll have to rebuild anyway...
	memcpy(buffer, &lastMajorEvent, sizeof(CampaignTime));		buffer += sizeof(CampaignTime);
	memcpy(buffer, &lastResupply, sizeof(CampaignTime));		buffer += sizeof(CampaignTime);
	memcpy(buffer, &lastRepair, sizeof(CampaignTime));			buffer += sizeof(CampaignTime);
	memcpy(buffer, &lastReinforcement, sizeof(CampaignTime));	buffer += sizeof(CampaignTime);
	memcpy(buffer, &TimeStamp, sizeof(short));					buffer += sizeof(short);
	memcpy(buffer, &Group, sizeof(short));						buffer += sizeof(short);
	// Dont read flags
	memcpy(buffer, &GroundRatio, sizeof(short));				buffer += sizeof(short);
	memcpy(buffer, &AirRatio, sizeof(short));					buffer += sizeof(short);
	memcpy(buffer, &AirDefenseRatio, sizeof(short));			buffer += sizeof(short);
	memcpy(buffer, &NavalRatio, sizeof(short));					buffer += sizeof(short);
	memcpy(buffer, &Brief, sizeof(short));						buffer += sizeof(short);
	// Dont read Processor
	memcpy(buffer, &TheaterSizeX, sizeof(short));				buffer += sizeof(short);
	memcpy(buffer, &TheaterSizeY, sizeof(short));				buffer += sizeof(short);
	memcpy(buffer, &CurrentDay, sizeof(uchar));					buffer += sizeof(uchar);
	memcpy(buffer, &ActiveTeams, sizeof(uchar));				buffer += sizeof(uchar);
	memcpy(buffer, &DayZero, sizeof(uchar));					buffer += sizeof(uchar);
	memcpy(buffer, &EndgameResult, sizeof(uchar));				buffer += sizeof(uchar);
	...
	memcpy(buffer, &Situation, sizeof(uchar));					buffer += sizeof(uchar);
	memcpy(buffer, &EnemyAirExp, sizeof(uchar));				buffer += sizeof(uchar);
	memcpy(buffer, &EnemyADExp, sizeof(uchar));					buffer += sizeof(uchar);
	memcpy(buffer, &BullseyeName, sizeof(uchar));				buffer += sizeof(uchar);
	memcpy(buffer, &BullseyeX, sizeof(GridIndex));				buffer += sizeof(GridIndex);
	memcpy(buffer, &BullseyeY, sizeof(GridIndex));				buffer += sizeof(GridIndex);
	memcpy(buffer, TheaterName, sizeof(char)*CAMP_NAME_SIZE);	buffer += sizeof(char)*CAMP_NAME_SIZE;
	memcpy(buffer, Scenario, sizeof(char)*CAMP_NAME_SIZE);		buffer += sizeof(char)*CAMP_NAME_SIZE;
	memcpy(buffer, SaveFile, sizeof(char)*CAMP_NAME_SIZE);		buffer += sizeof(char)*CAMP_NAME_SIZE;
	memcpy(buffer, UIName, sizeof(char)*CAMP_NAME_SIZE);		buffer += sizeof(char)*CAMP_NAME_SIZE;
...
#endif
	memcpy(buffer, &PlayerSquadronID, sizeof(VU_ID));			buffer += sizeof(VU_ID);
	...
	memcpy(buffer, &entries, sizeof(short));					buffer += sizeof(short);
	event = StandardEventQueue;
	while (event)
	{
		memcpy(buffer, event, sizeof(uieventnode));				buffer += sizeof(uieventnode);
		size = _tcslen(event->eventText);
		memcpy(buffer, &size, sizeof(short));					buffer += sizeof(short);
		memcpy(buffer, event->eventText, sizeof(_TCHAR)*size);	buffer += sizeof(_TCHAR)*size;
		event = event->next;
	}
	// Write priority list
	entries = 0;
	...
	// Write the map data
	memcpy(buffer, &CampMapSize, sizeof(short));				buffer += sizeof(short);
	if (CampMapSize)
		memcpy(buffer, CampMapData, CampMapSize);				buffer += CampMapSize;
	// Write the squadron data
	memcpy(buffer, &LastIndexNum, sizeof(short));				buffer += sizeof(short);
	memcpy(buffer, &NumAvailSquadrons, sizeof(short));			buffer += sizeof(short);
	if (NumAvailSquadrons > 0)
	{
		ShiAssert(CampaignSquadronData);
		memcpy(buffer, CampaignSquadronData, sizeof(SquadUIInfoClass)*NumAvailSquadrons);		buffer += sizeof(SquadUIInfoClass)*NumAvailSquadrons;
	}
	memcpy(buffer, &Tempo, sizeof(uchar));						buffer += sizeof(uchar);
	memcpy(buffer, &CreatorIP,sizeof(long));					buffer += sizeof(long);
	memcpy(buffer, &CreationTime,sizeof(long));					buffer += sizeof(long);
	memcpy(buffer, &CreationRand,sizeof(long));					buffer += sizeof(long);
	ShiAssert ( (int)(buffer - bufhead) == datasize );

	// Compress it and return
	*stream = new VU_BYTE[datasize + sizeof(long) + MAX_POSSIBLE_OVERWRITE];
	sptr = *stream;
	memcpy(sptr, &datasize, sizeof(long));						sptr += sizeof(long);
...
int CampaignClass::LoadScenarioStats (FalconGameType type, char *savefile)
{
	...
	PostMessage(FalconDisplay.appWin,FM_GOT_CAMPAIGN_DATA, (ulong)CampStatusEnum::CAMP_NEED_PRELOAD,0);
	return 1;
}
...
int CampaignClass::RequestScenarioStats (FalconGameEntity *game)
{
	...
	Flags |= (ulong)CampStatusEnum::CAMP_NEED_PRELOAD;
	camprequest = new FalconRequestCampaignData(masterSession->Id(), masterSession);
	camprequest->dataBlock.who = vuLocalSessionEntity->Id();
	camprequest->dataBlock.dataNeeded = (ulong)CampStatusEnum::CAMP_NEED_PRELOAD;
	FalconSendMessage (camprequest,TRUE);
...
{
	if (Flags & (ulong)CampStatusEnum::CAMP_PRELOADED)
	{
...
void CampaignClass::Suspend (void)
{
	...
	Flags |= (ulong)CampStatusEnum::CAMP_SUSPEND_REQUEST;
	while (!IsSuspended() && (Flags & (ulong)CampStatusEnum::CAMP_SUSPEND_REQUEST)){
...
void CampaignClass::Resume(void)
{
	...
	Flags ^= (ulong)CampStatusEnum::CAMP_SUSPENDED;
}
...
// =========================================================
// Support methods to figure out which events to show the UI
// =========================================================
...
void CampaignClass::ReadValidAircraftTypes (char *typefile)
{
	...
	sscanf(data_ptr,"%d%n", (int*)(&NumberOfValidTypes), &size);
...
void Camp_MakeInstantAction (void)
{
	TheCampaign.Flags |= (ulong)CampStatusEnum::CAMP_LIGHT;
...
void NukeHistoryFiles (void)
{
	...
	sprintf(filename, "%s\\tmp.his", FalconCampUserSaveDirectory); //RERUN don't need 2nd param , TheCampaign.SaveFile);
	unlink(filename);
	sprintf(filename,"%s\\tmp.frc",FalconCampUserSaveDirectory); //RERUN don't need 2nd param ,TheCampaign.SaveFile);
	unlink(filename);
#ifdef DEBUG
