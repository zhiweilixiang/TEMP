...
/*****************************************************************************/
//	Bla.. messy function that should fudge it so that throttles are NOT
...
//			else if(i == 1)
	...
	for (i = 0; i < NumberOfPOVs; i++)	// Retro 26Dec2003
	{
		if(IO.povHatAngle[i] != -1)
		{
			...
			{
				theFunc2 = UserFunctionTable.GetPOVFunction(i,LastDirPOV[i],&ID);
				if(theFunc2)
					CallFunc(theFunc2,1, 0, NULL); //Wombat778 03-06-04 Use callfunc instead of directly calling funcs, so they can be captured
			...
			{
				theFunc2 = UserFunctionTable.GetPOVFunction(i,Direction,&ID);
				if(theFunc2)
				{
					if( ID  < 0) {
						CallFunc(theFunc2,1, KEY_DOWN, NULL);		//Wombat778 03-06-04 Use callfunc instead of directly calling funcs, so they can be captured
					}
					else {
						CallFunc(theFunc2,1, KEY_DOWN, OTWDriver.pCockpitManager->GetButtonPointer(ID));		//Wombat778 03-06-04 Use callfunc instead of directly calling funcs, so they can be captured
						OTWDriver.pCockpitManager->Dispatch(ID, 0);//the 0 should be mousside but I don't have anywhere
					}												//to store it and all functions currently use 0. ;)
				}
				else
...
int CheckForForceFeedback(const int theJoyIndex)
{
	...
	{
		//	FFB only for the primary device ! if nothing is yet mapped 
