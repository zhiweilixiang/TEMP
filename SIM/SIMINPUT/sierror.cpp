...
#define Err(x) { x, #x }
	...
	Err((int)DIERR_DEVICEFULL ),
	Err(DIERR_DEVICENOTREG ),
	Err((int)DIERR_EFFECTPLAYING ),
	Err(DIERR_GENERIC ),
	Err(DIERR_HANDLEEXISTS ),
	Err((int)DIERR_HASEFFECTS ),
	Err((int)DIERR_INCOMPLETEEFFECT ),
	Err(DIERR_INPUTLOST ),
	Err(DIERR_INVALIDPARAM ),
#ifdef DIERR_MAPFILEFAIL
	Err((int)DIERR_MAPFILEFAIL ),
#endif
	Err((int)DIERR_MOREDATA ),
	Err(DIERR_NOAGGREGATION ),
	Err(DIERR_NOINTERFACE ),
	Err(DIERR_NOTACQUIRED ),
	Err((int)DIERR_NOTBUFFERED ),
	Err((int)DIERR_NOTDOWNLOADED ),
	Err((int)DIERR_NOTEXCLUSIVEACQUIRED ),
	Err(DIERR_NOTFOUND ),
	...
	Err(DIERR_READONLY ),
	Err((int)DIERR_REPORTFULL ),
	Err((int)DIERR_UNPLUGGED ),
