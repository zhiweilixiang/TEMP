...
//    int i = DROGUE;
		if (boom[DROGUE].drawPointer) 
		{
		  ...
		  boom[DROGUE].drawPointer->SetDOFangle(0, (float)fabs(boom[DROGUE].ext/-DrogueExt));	// 04DEC03 - FRB - drogue extension uses Translation DOFs(TDOF)
...
void TankerBrain::TurnTo (float newHeading)
{
	...
	dx = (float)(self->XPos() + dist * sin (newHeading));
	dy = (float)(self->YPos() + dist * cos (newHeading));
...
*/
				...
				dx = (float)(TrackPoints[0].x + dist * cos (heading));
				dy = (float)(TrackPoints[0].y + dist * sin (heading));
				...
				dx = (float)(TrackPoints[1].x + dist * cos (heading));
				dy = (float)(TrackPoints[1].y + dist * sin (heading));
				...
				dx = (float)(TrackPoints[2].x + dist * cos (heading));
				dy = (float)(TrackPoints[2].y + dist * sin (heading));
