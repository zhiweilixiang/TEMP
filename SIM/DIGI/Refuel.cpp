...
// in position, if not, try to be faster 
		...
		if (g_nShowDebugLabels & 0x800)
		{
			char tmpchr[32];
			if (decelerating)
				sprintf(tmpchr,"D %3.2f %5.1f %6.1f",af->vcas, oldrx, (float)(af->Fuel() + af->ExternalFuel()));
			else
				sprintf(tmpchr,"%3.2f %5.1f %6.1f",af->vcas, oldrx, (float)(af->Fuel() + af->ExternalFuel()));
			if (self->drawPointer)
				((DrawableBSP*)self->drawPointer)->SetLabel (tmpchr, ((DrawableBSP*)self->drawPointer)->LabelColor());
		}
