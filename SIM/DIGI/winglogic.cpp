...
void DigitalBrain::AiCheckLandTakeoff(void)
{
	...
	if( atcstatus >= AtcStatusEnum::tReqTaxi && atcstatus <= AtcStatusEnum::tTaxiBack )
	{
		mpActionFlags[AI_FOLLOW_FORMATION] = FALSE;
		AddMode(TakeoffMode);
	}
	else if(	mpActionFlags[AI_LANDING] 
		|| (atcstatus >= AtcStatusEnum::lReqClearance && atcstatus <= AtcStatusEnum::lCrashed))
	{		
		if(atcstatus > AtcStatusEnum::lIngressing)
			mpActionFlags[AI_FOLLOW_FORMATION] = FALSE;
		AddMode(LandingMode);					
	}
...
void DigitalBrain::AiCheckForUnauthLand(VU_ID lead)
{
	...
	if(!self->OnGround() && leader && mpActionFlags[AI_FOLLOW_FORMATION] == TRUE && 
		leader->DBrain()->ATCStatus() < AtcStatusEnum::tReqTaxi && leader->OnGround() && atcstatus == AtcStatusEnum::noATC) {
