...
void DigitalBrain::ResetATC(){
	SetATCStatus(AtcStatusEnum::noATC);
...
void DigitalBrain::ResetTaxiState(void)
{
	...
	if(!self->OnGround() || !Airbase){
		if(atcstatus >= AtcStatusEnum::tReqTaxi){
			ResetATC();
		}
		return;
	}
	...
	if( (atcstatus == AtcStatusEnum::tTakeRunway || atcstatus == AtcStatusEnum::tTakeoff) &&
		 (PtDataTable[curTaxiPoint].type == TakeoffPt || PtDataTable[curTaxiPoint].type == RunwayPt) )
	{
		...
		atcstatus = AtcStatusEnum::tTakeRunway;
	}
	else	
	{
	...
	switch(atcstatus)
	{
		case AtcStatusEnum::noATC:
			...
			if(dx*dx + dy*dy < APPROACH_RANGE * NM_TO_FT * NM_TO_FT * 0.95F){
				atcstatus = AtcStatusEnum::lReqClearance;
				if( !isWing )
					SendATCMsg(AtcStatusEnum::lReqClearance);
			}
			else {
				atcstatus = AtcStatusEnum::lIngressing;
				SendATCMsg(AtcStatusEnum::lIngressing);
			}
		...
		case AtcStatusEnum::lTakingPosition:
			...
			if(((Unit)self->GetCampaignObject())->GetTotalVehicles() > 1)
			{
				...
				if(leader && !mpActionFlags[AI_RTB]) // JB 010527 (from MN)
				{
					...
					float tx, ty, tz;
					abObj->brain->FindFinalPt(self, rwIndex, &tx, &ty);
					tz = abObj->brain->GetAltitude(self, AtcStatusEnum::lTakingPosition);
					SetTrackPoint(tx, ty, tz);
				}
			}
			if ( self->ZPos() - gAvoidZ > minZ ){
		...
		case AtcStatusEnum::lAborted:
			...
			if(dx*dx + dy*dy < 0.25F*NM_TO_FT*NM_TO_FT){
				...
				rwIndex = abObj->brain->FindBestLandingRunway(self, TRUE);
				abObj->brain->FindFinalPt(self, rwIndex, &tx, &ty);
				tz = abObj->brain->GetAltitude(self, AtcStatusEnum::lReqClearance);
		...
		case AtcStatusEnum::lEmerHold:
			...
			if(waittimer < SimLibElapsedTime){
				atcstatus = AtcStatusEnum::lEmerHold;
				SendATCMsg(AtcStatusEnum::lEmerHold);
				waittimer = SimLibElapsedTime + LAND_TIME_DELTA;
			}
			Loiter();
			af->gearHandle = -1.0F;
		...
		case AtcStatusEnum::lHolding:
			...
			if(rwtime < SimLibElapsedTime + maxi - CampaignSeconds * 5 ){
				...
				if(cosAngle < 0.0F)
				{
					...
					tz = abObj->brain->GetAltitude(self, atcstatus);
					SetTrackPoint(tx, ty, tz);
					if(atcstatus != AtcStatusEnum::lHolding){
		...
		case AtcStatusEnum::lFirstLeg:
			...
			if(fabs(relx) < turnDist && fabs(rely) < turnDist*3.0F){
				abObj->brain->FindFinalPt(self, rwIndex, &finalX, &finalY);
				if(cosAngle < 0.0F) {
					atcstatus = AtcStatusEnum::lToBase;
					abObj->brain->FindBasePt(self, rwIndex, finalX, finalY, &baseX, &baseY);
					...
					atcstatus = AtcStatusEnum::lToFinal;
		...
		case AtcStatusEnum::lEmergencyToBase:
			...
			if (fabs(relx) < turnDist && fabs(rely) < turnDist*3.0F){
				abObj->brain->FindFinalPt(self, rwIndex, &finalX, &finalY);
				if(atcstatus == AtcStatusEnum::lEmergencyToBase){
					atcstatus = AtcStatusEnum::lEmergencyToFinal;
				}
				else{
					atcstatus = AtcStatusEnum::lToFinal;
				}
		...
		case AtcStatusEnum::lEmergencyToFinal:
			...
			if(relx < 3.0F*NM_TO_FT && relx > -1.0F*NM_TO_FT && fabs(rely) < turnDist){
				...
				atcstatus = AtcStatusEnum::lOnFinal;
				if(atcstatus == AtcStatusEnum::lEmergencyToFinal){
					atcstatus = AtcStatusEnum::lEmergencyOnFinal;
				}
				else{
					atcstatus = AtcStatusEnum::lOnFinal;
				}
		...
		case AtcStatusEnum::lOnFinal:
			...
			if ( self->IsSetFlag( ON_GROUND ) ){
				atcstatus = AtcStatusEnum::lLanded;
				SendATCMsg(atcstatus);
			...
			if(cosAngle > 0.0F && af->groundZ - self->ZPos() > 50.0F){
				...
				finalZ = abObj->brain->GetAltitude(self, AtcStatusEnum::lOnFinal);
		...
		case AtcStatusEnum::lLanded:
			// edg: If we're not on the ground, we're in trouble (which has
			...
			if ( !self->OnGround() )
			{
				...
				{
					atcstatus = AtcStatusEnum::lAborted;
					SendATCMsg(atcstatus);
					float tx, ty, tz;
			...
			if (CloseToTrackPoint() )
			{
				...
				case TaxiPt:
					...
					atcstatus = AtcStatusEnum::lTaxiOff;
					waittimer = 0;
					SendATCMsg(atcstatus);
					break;
			...
			{
				char label [96];
				sprintf(label,"Speed: %d kts, PointDist: %d ft",FloatToInt32(self->GetVt()/KNOTS_TO_FTPSEC),FloatToInt32((float)sqrt(pow(dx, 2) + pow(dy, 2))));
				if (self->drawPointer)
...
void DigitalBrain::TakeOff()
{
	...
	if (!IsSetATC(DonePreflight) && curTaxiPoint) 
	{
		...
		if (SimLibElapsedTime > t2t - max(PlayerOptionsClass::TAXI_MINUTES * CampaignMinutes, 3 * CampaignMinutes)){
...
//			ShiAssert(!"Show this to Dave P. (not on ground)");
		...
		switch(PtDataTable[curTaxiPoint].type)
		{
			...
			case LargeParkPt:
			{
				atcstatus = AtcStatusEnum::tReqTakeoff;
				waittimer = SimLibElapsedTime + TAKEOFF_TIME_DELTA;  
				float tx, ty;
				TranslatePointData(Airbase, curTaxiPoint, &tx, &ty);
				SetTrackPoint(tx, ty);
				if(!isWing){
					SendATCMsg(AtcStatusEnum::tReqTakeoff);
				}
				break;
			}
			case TakeoffPt:	
...
//				CalculateTaxiSpeed(MoveAlong);
				...
				if (cosAngle >  0.99619F){
					if(ReadyToGo()){
						waittimer = 0;
						atcstatus = AtcStatusEnum::tTakeoff;
						SendATCMsg(atcstatus);
		...
		// edg: test for bad states -- I've seen planes taking the runway
...
//			CalculateTaxiSpeed(MoveAlong);
		...
		if (af->z - af->groundZ < -50.0F)
		{
			if(af->gearHandle > -1.0F)
				af->gearHandle = -1.0F;
			atcstatus = AtcStatusEnum::tFlyOut;
			SendATCMsg(atcstatus);
		}		
		break;
...
// a failed assertion		tmpWaypoint = tmpWaypoint->GetPrevWP();
					...
					dx = (float)(Airbase->XPos() + dist * cos (legHeading));
					dy = (float)(Airbase->YPos() + dist * sin (legHeading));
...
void DigitalBrain::DealWithBlocker(SimBaseClass *inTheWay, ObjectiveClass *Airbase)
{
	...
	if(atcstatus != AtcStatusEnum::tTakeRunway)
	{
...
void DigitalBrain::ChooseNextPoint(ObjectiveClass	*Airbase)	 // to Runway	 Takeoff() & ResetTaxiState()
{
	...
	if (isWing){
		...
		else if (leader && leader->OnGround() && leader->DBrain()->ATCStatus() != AtcStatusEnum::tTaxiBack){
...
//		return;
	...
	//			waittimer = 0;
				...
				CalculateTaxiSpeed(HurryUp);    // 17JAN04 - FRB - expedite takeoff
				if(atcstatus != AtcStatusEnum::tTakeRunway){
					SendATCMsg(AtcStatusEnum::tPrepToTakeRunway);
					atcstatus = AtcStatusEnum::tTaxi;
				}
				//waittimer = CalcWaitTime(Airbase->brain);
				waittimer = 0;
			}
				...
				if(atcstatus != AtcStatusEnum::tTakeRunway && !self->IsSetFalcFlag(FEC_HOLDSHORT)){
					atcstatus = AtcStatusEnum::tHoldShort;
					SendATCMsg(atcstatus);
				}
			}
		break;
...
{
	if(atcstatus != AtcStatusEnum::lTaxiOff){
		atcstatus = AtcStatusEnum::lTaxiOff;
		SendATCMsg(AtcStatusEnum::noATC);
	}
...
void DigitalBrain::SendATCMsg(AtcStatusEnum msg)
{
	...
	ATCMessage->dataBlock.status	= msg;
...
//	point = GetNextPt(curTaxiPoint);
	...
	if(point && time)
	{
		...
		desiredSpeed = (float)sqrt(pow(dx,2) + pow(dy,2))/time;
...
//			int taxiPoint = GetPrevPtLoop(curTaxiPoint);
	...
	if ( atcstatus == AtcStatusEnum::tTakeoff )
	{
...
//	if((!self->OnGround()) || (atcstatus == tTakeRunway))	 // 07FEB04 - FRB - added atcstatus check
	...
	if((!self->OnGround()) || !self->IsPlayer() && (atcstatus == AtcStatusEnum::tTakeRunway))
		return;
...
//		if(closest == GetNextPtLoop(curTaxiPoint) && self->AutopilotType() == AircraftClass::APOff)
		...
		{
			if( IsSetATC(CheckTaxiBack) && atcstatus != AtcStatusEnum::tTaxiBack)
			{
				atcstatus = AtcStatusEnum::tTaxiBack;
				SendATCMsg(atcstatus);
			}
			else
...
//		else if(closest == GetPrevPtLoop(curTaxiPoint) && atcstatus == tTaxiBack)
		else if(closest == GetPrevTaxiPt(curTaxiPoint) && atcstatus == AtcStatusEnum::tTaxiBack)
		{
