...
float	DigitalBrain::SimpleTrackElevation(float zft, float scale)
{
		...
		altErr > 0.0f && self->Pitch() > 0.0f)
	{
		...
		float curPitch = self->Pitch(); // RERUN Current pitch == current pitch
		float scale;
		...
		float maxVcas = aeroDataset[self->af->VehicleIndex()].inputData[AeroDataSet::MaxVcas];
		float curKias = self->GetKias(); // Current speed

		// scale the pitch based on on the difference of pitch from max
		if (curPitch > maxPitch) {
			//scale = 0.0f;
			scale = 1-(Abs(((maxVcas - curKias) / maxVcas))); //RERUN scale = 0.0f created bouncing effects.
			scale *= 0.5f; //RERUN tone down steep pitch
		} else
