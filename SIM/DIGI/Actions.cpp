...
extern int g_nAirbaseCheck; //RERUN
#if defined (_DEBUG)
...
//RERUN extern int g_nAirbaseCheck;
...
char label[100];//cobra change from 40
   ...
   if ( threatPtr && curMode != TakeoffMode)
   {
	   ...
	   if(atcstatus != AtcStatusEnum::noATC)
	   {
		   ShiAssert(atcstatus < AtcStatusEnum::tReqTaxi);
...
// If not, continue to follow waypoints.
		   ...
		   case WaypointMode:
			...
			sprintf(label,"Landing %s",
				(int)atcstatus >= 0 && (int)atcstatus < MAXATCSTATUS ? ATCModes[(int)atcstatus] : "");
#endif
				break;
...
if (g_nShowDebugLabels & 0x100000)
{
	...
	if (yaw < 0.0f)
		yaw += 360.0F * DTR;
	yaw *= RTD;
	sprintf(label,"%4.0f %3.0f %6.2f %3.3f", self->af->vcas, yaw, -self->ZPos(), (float)(self->pctStrength * 100.0F));
	if ( self->drawPointer )
			((DrawableBSP*)self->drawPointer)->SetLabel (label, ((DrawableBSP*)self->drawPointer)->LabelColor());
}
if (g_nShowDebugLabels & 0x800000)
{
	sprintf(label,"0x%p leader: 0x%p", (void*)self, (void*)flightLead);
	if ( self->drawPointer )
			((DrawableBSP*)self->drawPointer)->SetLabel (label, ((DrawableBSP*)self->drawPointer)->LabelColor());
}
#endif
