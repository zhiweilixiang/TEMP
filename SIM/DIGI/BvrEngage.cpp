...
#include "Fcc.h"//me123
#include "sms.h"//cobra
//RERUN #define MANEUVER_DEBUG // MNLOOK
...
void DigitalBrain::ChoiceProfile(void)
{
	...
	if (targetPtr)
		...
		if (missionType > MissionTypeEnum::AMIS_ESCORT && !g_bUseAggresiveIncompleteA2G && (IsSetATC(HasAGWeapon)|| !missionComplete))
			{
...
//		DesiredSpeed = ((AircraftClass*)self)->af->MinVcas();
	...
	else
	{
		...
		trackZ = max (trackZ,-45000.0f); //RERUN this was disabled. testing enabling to avoid the trackZ being positive assert.
		}
		else 
		{
