...
//RERUN #define MANEUVER_DATA_FILE   "sim\\acdata\\brain\\mnvrdata.dat"
#define MANEUVER_DATA_FILE   "Zips\\sim\\acdata\\brain\\mnvrdata.dat"
	...
	if(flight){
		...
		if(package){
			...
			for (UnitClass *un = package->GetFirstUnitElement(); un; un = package->GetNextUnitElement()) {
			   if (un->IsFlight()) {
				   if (((FlightClass *)un)->GetUnitMission() == MissionTypeEnum::AMIS_ESCORT)
 ...
 SimlibFileClass *mnvrFile;
    ...
    if (fileType == '#')
    {
        for (int i = 0; i < NumMnvrClasses; ++i)
        {
            ...
            //RERUN sscanf(mnvrFile->GetNext(), "%x", &maneuverClassData[i]);
			sscanf(mnvrFile->GetNext(), "%x", (unsigned int*)&maneuverClassData[i]); 
