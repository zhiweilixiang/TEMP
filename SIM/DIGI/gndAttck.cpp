...
int DigitalBrain::IsNotMainTargetSEAD()
{
	...
	if ( missionType == MissionTypeEnum::AMIS_SEADESCORT )
		return TRUE;

	// Only for SEADS missions
	if ( missionType != MissionTypeEnum::AMIS_SEADSTRIKE )
		return FALSE;
...
//================================================
	...
	if (!groundTargetPtr) 
	{
		...
		if (groundTargetPtr)
		{
			if ((missionType == MissionTypeEnum::AMIS_SEADSTRIKE) && !hasHARM)
			{
				SetGroundTarget( NULL );
				agDoctrine = AGD_NONE;
				missionComplete = TRUE;
...
DownwindBypass:
	...
	switch (onStation)
	{
		...
		case NotThereYet:
			...
			else if (missionType == MissionTypeEnum::AMIS_SEADESCORT || missionType == MissionTypeEnum::AMIS_SEADSTRIKE)
			{
				// see if we're too close to set up the ground run
				// if so, we're going to head to a new point perpendicular to
				// our current direction and make a run from there
			...
			if ((agDoctrine == AGD_LOOK_SHOOT_LOOK) && 
					(missionType == MissionTypeEnum::AMIS_SEADESCORT || missionType == MissionTypeEnum::AMIS_SEADSTRIKE))
			{
...
//======= End Downwind ===========================================
		...
		case Final:
			...
			if ((missionType == MissionTypeEnum::AMIS_BDA || missionType == MissionTypeEnum::AMIS_RECON) && hasCamera )
			{
    ...
    case Final1:
		 {
				...
				if ( missionType == MissionTypeEnum::AMIS_BDA || missionType == MissionTypeEnum::AMIS_RECON)
				{
...
void DigitalBrain::SelectGroundWeapon( void )
{
	...
	if (missionType == MissionTypeEnum::AMIS_AIRCAV) {
		runAway = false;
	}
...
*/
		...
		agDoctrine = AGD_LOOK_SHOOT_LOOK;
		if (missionType == MissionTypeEnum::AMIS_SEADESCORT || missionType == MissionTypeEnum::AMIS_SEADSTRIKE)
		{
		...
		else
		{
			...
			if ( missionType == MissionTypeEnum::AMIS_SAD || missionType == MissionTypeEnum::AMIS_INT || missionType == MissionTypeEnum::AMIS_BAI || 
				 hasAGMissile || hasHARM || IsNotMainTargetSEAD())
				agDoctrine = AGD_LOOK_SHOOT_LOOK;
