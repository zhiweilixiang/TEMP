...
void SimulationLoopControl::StartLoop(void) {
	...
	while (1) {
		...
		if (player && !player->IsDead()){
			...
			//RERUN check first for player entity
			if ((SimBaseClass*)FalconLocalSession->GetPlayerEntity() == NULL){
				FalconLocalSession->SetPlayerEntity(player); //RERUN fix this on-the-fly...
			}
