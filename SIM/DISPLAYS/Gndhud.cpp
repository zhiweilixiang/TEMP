...
char tmpStr[32];
  ...
  display->AdjustOriginInViewport (betaHudUnits, dy);
    ...
    display->Line (pipperAz - diam, pipperEl, pipperAz - diam - tic, pipperEl);
		...
		float sx = (float)sin(45*DTR) * diam;
		float sy = (float)cos(45*DTR) * diam;
		float sticx = (float)(sin(45*DTR) * (diam+stic));
		float sticy = (float)(cos(45*DTR) * (diam+stic));
    ...
    display->Line (-diam, yOffset, -diam - tic, yOffset);
		...
		float sx = (float)sin(45*DTR)*diam;
		float sy = (float)cos(45*DTR)*diam+yOffset;
		float sticx = (float)sin(45*DTR)*(diam+stic);
		float sticy = (float)cos(45*DTR)*(diam+stic)+yOffset;
