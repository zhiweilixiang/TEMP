...
void AdvancedHarmTargetingPod::HASDisplay (VirtualDisplay* activeDisplay)
{
	...
	for (tmpElement = FCC->GetFirstGroundElement(); tmpElement; tmpElement = tmpElement->GetNext())
	{
		...
		x2 = x * x;
		y2 = y * y;
		range = (float)sqrt( x2 + y2 ); // Get range
		alpha = (float)atan( alt / range ); // Get the elevation angle
		...
		phi = (float)atan( NormDisplayX / NormDisplayYNoOffset );
		ex = (float)(elevation * sin(phi));
		ey = (float)(elevation * cos(phi));
...
void AdvancedHarmTargetingPod::HandoffDisplay (VirtualDisplay* activeDisplay)
{
	...
	if ( preHandoffMode == Has )
	{
		for (tmpElement = FCC->GetFirstGroundElement(); tmpElement; tmpElement = tmpElement->GetNext())
		{
			...
			x2 = x * x;
			y2 = y * y;
			range = (float)sqrt( x2 + y2 );
			alpha = (float)atan( alt / range );
			elevation = (1.0f - (alpha / HALF_PI)) * HAS_DISPLAY_RADIUS;
		...
		if ( POSTargets[i] )
		{
				 ...
				 tmpElement->BaseObject() == lockedTarget->BaseData() ) 
			{
				...
				if ( curMissile && curTarget && curTarget->BaseObject() )
				{
					...
					dx = (float)(fabs(playerAC->XPos() - curTarget->BaseObject()->XPos()));
					dx *= dx;
					dy = (float)(fabs(playerAC->YPos() - curTarget->BaseObject()->YPos()));
					dy *= dy;
					theRange = (float)sqrt(dx + dy);
				...
				// RV - I-Hawk - Get ETA - BTW I noticed the HUD ETA for waypoints is messed up
