      ...
      else
      {
		...
		if ( ctlroll )
		{
			...
			if (g_bSimpleFMUpdates && gearPos < 0.7F && platform->DBrain()->GetCurrentMode() != DigitalBrain::LandingMode && platform->DBrain()->GetCurrentMode() != DigitalBrain::RefuelingMode)
				maxBank = (float)min(maxBank, acos (1/(max(min(curMaxGs,gsAvail), 1.5f) )));
...
// track point. 
			if ( (g_bTankerFMFix && platform->DBrain() && platform->DBrain()->IsTanker() && 
				...
				r = (float)((360.0 * GRAVITY * tan (turnangle)/ (2* M_PI *vt)) * DTR);
			else
	...
	else
	{
		...
		rotate = 17.16f * (float)sqrt((weight/area)/fabs(cltakeoff));//cobra
		ctlpitch += 0.1f;
		AtcStatusEnum tstatusf = platform->DBrain()->ATCStatus();
		if ( vcas > rotate && tstatusf > AtcStatusEnum::tTaxi )//cobra
		{
			// pitch where we want to be
			tmp = ctlpitch * MAX_AF_PITCH;
			q = (tmp - gmma) * kPitch;
   ...
   if (!IsSet(InAir))
   {
	   ...
	   else if (qsom*cnalpha > 0.55F)
	   {
		   ...
		   AtcStatusEnum tstatus = platform->DBrain()->ATCStatus();
		   if ( vcas > rotate && tstatus > AtcStatusEnum::tTaxi)//Cobra
		   {
			   #ifdef DEBUG
			   ShiAssert(platform->DBrain()->ATCStatus() != AtcStatusEnum::lLanded);
			   #endif
			   SetFlag(InAir);
