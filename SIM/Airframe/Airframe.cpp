		...
		case None:
					...
					( fabs(pshape) > 0.6F	&& fabs(rshape) > 0.6F && IsSet(LowSpdHorn) ) ) )
			{
				...
				if(	(!platform->OnGround () && vcas < 60.0f ||
					...
					alpha > 31.0F  - 9*((double)loadingFraction - 1.3F) //RERUN removed extra '-' - //me123 addet 9           
								- fabs(assymetry/weight)*10.0F		||// 10 from 5 me123
					alpha < -14.0F + ((double)loadingFraction - 1.3F) +
								IsSet(LowSpdHorn)*3.0 + fabs(assymetry/weight)*5.0F )
				{
...
//	MonoPrint("RemoveWeapon W%f DI%f, O%f\n",Weight,DragIndex,offset);
