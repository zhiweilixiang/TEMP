		...
		if ( platform->LandingCheck( noseAngle, impactAngle, groundType) )
		{
			...
			if( IsSet(GearBroken) || gearPos <= 0.1F )
			{
				...
				if ( platform->DBrain() && !platform->IsSetFalcFlag(FEC_INVULNERABLE))
				{
					...
					platform->DBrain()->SetATCStatus(AtcStatusEnum::lCrashed);
				...
				if(airbase ){
					...
					if( Runway ){
						platform->DBrain()->SendATCMsg(AtcStatusEnum::lCrashed);
					}
				}
			}
...
float AirframeClass::CheckHeight(void) const {
	...
	if (deltz < deltzWing){ deltz = deltzWing; }
	if (deltz < deltzNose){ deltz = deltzNose; }
	if (deltz < deltzGear){ deltz = deltzGear; }
	if (deltz < deltzBody){ deltz = deltzBody; }
...
void AirframeClass::DragBodypart(void)
{
	...
	else if(vt <= 0.0F && platform->DBrain()->ATCStatus() != AtcStatusEnum::lCrashed){
		platform->DBrain()->SetATCStatus(AtcStatusEnum::lCrashed);
		if(platform != SimDriver.GetPlayerEntity()){
			platform->DBrain()->SetWaitTimer(SimLibElapsedTime + 1 * CampaignMinutes);
		}
