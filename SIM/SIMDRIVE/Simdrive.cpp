...
void SimulationDriver::Cycle(){
	...
	if (vuLocalSessionEntity && vuLocalGame && vuLocalGame->IsLocal()){
		...
		while (sess && !flying ){
			// sfr: why get from DB???
			// this is a hack. They get from DB because UI can be closing while this is runnig
			// definitely NOT SAFE
...
#define NO_CAMP_CRITICAL 1
	...
	if (runGraphics){
		...
		if (!OTWDriver.DisplayInCockpit()){
			...
			float v = (float)sqrt(pow(x, 2) + pow(y, 2) + pow(z, 2));
			float vol = ( v - 10) * 1000;
			if (vol > 0){
				vol = 0;
			}
...
SimBaseClass* SimulationDriver::FindNearestTraffic (AircraftClass* aircraft, ObjectiveClass *self, float* altitude)
{
	...
	{	   
				...
				GetTTRelations((Team)theObject->GetTeam(), myTeam) <= Neutral)  
		{
			...
			{	
				...
				if (SimToGrid((float)sqrt(trafficRange)) <= trafficCheckRange)	//SimToGrid and sqrt convert to NM
				{
					...
					{				
						...
						if (self->brain->trafficCheck == TrafficStatusEnum::conflictTraffic)	// Traffic is a conflict
						{
							retval = theObject;							// Set retval to current traffic
							self->brain->trafficCheck = TrafficStatusEnum::newTraffic;		// This is new traffic	
						}
					}
				}
			}
			...
			else
			{
				...
				if (abs(tmpTrafficAlt - myAlt) < abs(trafficAlt - myAlt) && SimToGrid((float)sqrt(tmpRange))
						<= priTrafficDist)
				{
					FindTrafficConflict(theObject, aircraft, self);			// Check for conflict
					// a conflict was found and set in FindTrafficConflict function
					if (self->brain->trafficCheck == TrafficStatusEnum::conflictTraffic)
					{
						// this return value to this traffic because it is more of a threat
						trafficRange = tmpRange;						// setup traffic range
						retval = theObject;
						self->brain->trafficCheck = TrafficStatusEnum::priorityTraffic;
					}
				}
				else
				{
					if (tmpRange < trafficRange)
					{
						...
						if (self->brain->trafficCheck == TrafficStatusEnum::conflictTraffic)
						{
							...
							retval = theObject;
							self->brain->trafficCheck = TrafficStatusEnum::newTraffic;
						}
					}
				}
			}
	...
	if (retval)
	{
		*altitude = -retval->ZPos();
		...
		{
			self->brain->trafficCheck = TrafficStatusEnum::oldTraffic;
		}
...
void SimulationDriver::FindTrafficConflict(SimBaseClass *traffic, AircraftClass *myAircraft, ObjectiveClass *self)
{
	...
	if (relativeBearing >= 5 && relativeBearing <= 90)   
	{
		...
		{
			self->brain->trafficCheck = TrafficStatusEnum::conflictTraffic;	//possible conflict
		}
		else
			self->brain->trafficCheck = TrafficStatusEnum::noTraffic;			//no conflict
		return;
	}
