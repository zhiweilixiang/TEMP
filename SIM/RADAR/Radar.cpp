...
// END OF ADDED SECTION
	...
	if (target->BaseData()->OnGround() && !target->BaseData()->IsMover() )  {S = 0.0f;}//me123 don't show ground target's that don't move // 2001-09-07 S.G. ARE THEY SHOWING UP ON THE GM RADAR? IF NOT, WE HAVE A PROBLEM SINCE THEY WON'T SHOW ANYWHERE...
...
// 2000-11-24 QUESTION BY S.G. me123, DON'T YOU THINK A MAX OF 450 knots IS A BIT FAST FOR GROUND VEHICLE?
	if (target->BaseData()->OnGround() && target->BaseData()->IsMover()) { Vr = g_fMoverVrValue *(float)rand()/BIGGEST_RANDOM_NUMBER;}  //me123 let's make some bogus Vt for the moving ground target
...
// 2001-09-07 ADDED BY S.G. RP5 DATA ALREADY TAKES THIS INTO ACCOUNT. HAVING IT DONE IN THE EXE DOUBLES THE DESIRED EFFECT
	if (0)// me123 agreed with jjb to test this g_bRP5Comp) 
	{
		...
		float lookdown = (float)(asin (dz / target->localData->range)- radarData->BeamHalfAngle);
		float mainbeamclutterrange = (float)(-platform->ZPos()*FT_TO_NM / sin (lookdown));
...
//RERUN static const char RADAR_DIR[] = "sim\\radar";
static const char RADAR_DIR[] = "Zips\\sim\\radar";
