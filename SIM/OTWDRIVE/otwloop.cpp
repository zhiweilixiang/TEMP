	...
	if ( endFlightTimer != 0 ){
		...
		if (SimDriver.GetPlayerAircraft() && !SimDriver.GetPlayerAircraft()->IsEject()){
			...
			if(playerAircraft->OnGround()){
				if (playerAircraft->DBrain()->ATCStatus() < AtcStatusEnum::tReqTaxi){
...
void OTWDriverClass::DisplayInfoBar(void)
{
	...
	)
	{
		...
		if ((!otwPlatform->IsGroundVehicle())&&(!otwPlatform->IsBomb()))
		{
			...
			// height
			sprintf(tmp,"%5.i",(int)((DrawableBSP*)otwPlatform->GetAltitude()));
			strcat( tmpo,tmp);
...
void OTWDriverClass::RenderFrame(){
	...
	//JAM 18Nov03
	weatherCondition = realWeather->weatherCondition; //RERUN
