...
SfxClass::Draw (void)
{
	...
	if ( (!(flags & SFX_F16CRASHLANDING) && !(flags & SFX_MOVES)) || (flags & SFX_TIMER_FLAG) )
	{
		...
		{
            //RERUN 
			DrawableBSP* baseObjDrawPtr = (DrawableBSP*)baseObj->drawPointer;
			if (baseObjDrawPtr)
				baseObjDrawPtr->SetScale(scaleOTW);
	...
	{
		OTWDriver.ObjectSetData(baseObj.get(), &pos, &rot);
		DrawableObject* bspDrawableObject = (DrawableBSP*)baseObj->drawPointer;
		if (bspDrawableObject) //RERUN added CTD check
		{
			((DrawableBSP*)(bspDrawableObject))->Update(&pos, &rot);
			baseObj->drawPointer->SetScale(scaleOTW);
		}
	}
