...
BOOL OTWDriverClass::Padlock_ConsiderThisObject(SimBaseClass* pObj, BOOL isPainted, float range, float az, float el)
{
	...
	if (pObj && pObj->IsSim() && pObj->IsWeapon()) {
		...
		ushort objType = ((MissileClass*)pObj)->Type();

		if ((objType - VU_LAST_ENTITY_TYPE) < 0)
			return FALSE; //RERUN

		classPtr = &(Falcon4ClassTable[objType - VU_LAST_ENTITY_TYPE]);
		if ((!classPtr) || (classPtr->vehicleDataIndex < 0))
			return FALSE; //RERUN
		wpnDefinition = &SimWeaponDataTable[classPtr->vehicleDataIndex];
		if (!wpnDefinition)
			return FALSE;
		if ((wpnDefinition->weaponClass != (int)wcAimWpn) && (wpnDefinition->weaponClass != (int)wcSamWpn))
			return FALSE;
	}
	...
	if ((pObj) && (range < g_fPadlockBreakDistance * NM_TO_FT || isPainted))
	{
