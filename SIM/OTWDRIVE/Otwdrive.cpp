...
void OTWDriverClass::RunActionCamera( void )
{
	...
	if ( otwPlatform.get() == NULL ){
		viewStep = 1;
		FindNewOwnship();
		if ( otwPlatform.get() == NULL ) { return; }
	}
	...
	*/
	...
	if ( (otwPlatform.get() != NULL) && otwPlatform->IsFiring() )
	{
		// check back in several secs
		actionCameraTimer = vuxRealTime + 8000;
		return;
	...
	if ( (otwPlatform.get() != NULL) && otwPlatform->OnGround() && mOTWDisplayMode == ModeOrbit )
	{
		mOTWDisplayMode = ModeChase;
		SelectExternal();
	}
...
void OTWDriverClass::RunHybridPitMode(float pan, float tilt)
{
	...
	if (mOTWDisplayMode==Mode3DCockpit)							
	{
		...
		{			
			...
			if (abs((long)vuxRealTime-lasttime) > g_nHybridPitModeDelay)					
			{
...
void OTWDriverClass::SetGraphicsOwnship (SimBaseClass* obj)
{
	...
	if (otwPlatform.get() != NULL)
	{
		...
		if (otwPlatform.get() != FalconLocalSession->GetPlayerEntity()){
...
void OTWDriverClass::FindNewOwnship (void)
{
	...
	if (viewStep > 0){
		...
		while (newObject){
			if (otwPlatform.get() == theObject){ break; }
			theObject = newObject;
			newObject = (SimBaseClass*)updateWalker.GetNext();
		}
	...
	if (newObject)
	{
		...
		if ( newObject != SimDriver.GetPlayerAircraft() && DisplayInCockpit() )
		{
