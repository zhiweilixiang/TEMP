...
#else
// RERUN hardcoded SH leftover stuff. Check this.
...
SimMoverClass::~SimMoverClass(void){
	...
	if ( DOFData ){ MemFreePtr (DOFData); }
	if ( DOFType ){ MemFreePtr (DOFType); }
	if ( switchData ){ MemFreePtr (switchData); }
	if ( switchChange ){ MemFreePtr (switchChange); }
	if (VertexData){ MemFreePtr (VertexData); }
...
void SimMoverClass::CleanupLocalData(){
	// sfr: @TODO remove this (for now im just calling base class cleanup)
...
{
	// LEON TODO: Need to convert from a remote to a local entity.
				      ...
				      fabs (pos.z - fpos.z) < radius + p3.z )
			      {
				      ...
				      if ( testFeature->drawPointer->GetRayHit( &pos, &vec, &collide, 1.0f ) )
				      {
						  if ( IsWeapon() )
						  {
							  ...
							  if(fc->DamageMod[(int)wc->DamageType])
								  return testFeature;
