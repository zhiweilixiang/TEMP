...
inline double localGetDT(VU_TIME timestamp, VU_TIME last_timestamp) {
	...
	if (!((SimBaseClass*)entity_)->IsAwake()){
		...
		// predict
			...
			if (event->updateTime_ < vuxGameTime + PREDICTION_PERIOD){
				...
				double dt_ev = localGetDT(predictedTime_, event->updateTime_);
			...
			double dt_inv = 1.0f / localGetDT(predictedTime_, vuxGameTime);
			SM_SCALAR dx, dy, dz;
			...
			if (drsp > evsp){
				SM_SCALAR adjFactor = (SM_SCALAR)sqrt(evsp / drsp);
			...
			for (unsigned int i=0;i<3;++i){
				// always below 2PI
				et[i] = (SM_SCALAR)fmod(et[i], VU_TWOPI);
				ct[i] = (SM_SCALAR)fmod(ct[i], VU_TWOPI);
