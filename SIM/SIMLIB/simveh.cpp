...
void SimVehicleClass::ApplyDamage (FalconDamageMessage* damageMessage)
{
	...
	switch (damageMessage->dataBlock.damageType)
		...
		case FalconDamageType::ProximityDamage:
			...
			hitPoints = (float)damageMessage->dataBlock.damageStrength * ((float)vc->DamageMod[(int)DamType::HighExplosiveDam])/100.0f;
			break;
		...
		default:
			...
			hitPoints = (float)wc->Strength * ((float)vc->DamageMod[ (int)wc->DamageType ])/100.0f;
			break;
		}
