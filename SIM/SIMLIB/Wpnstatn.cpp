...
//	else
	...
	while(weapPtr != NULL)
	{
		if(weapPtr->drawPointer != NULL) // basics only load the 1st BSP
		{
			DetachWeaponBSP(weapPtr);
...
void AdvancedWeaponStation::DetachWeaponBSP(SimWeaponClass *weapPtr)
{
	...
	if (weapBSP->instance.SlotChildren != NULL) //RERUN added check, testing
	{
		if (*weapBSP->instance.SlotChildren != NULL)
		{

			if (theRack)
			{
				theRack->DetachChild(weapBSP, weapPtr->GetRackSlot());//RERUN fix these. slotnumber == numslots
			}
			else
			{
				if (thePylon)
