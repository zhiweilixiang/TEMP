...
void MissileClass::Start(SimObjectType *tgt)
{
   ...
   // edg NOTE: messing with the order we set launch state and targeting
...
// 2000-11-17 MODIFIED BY S.G. az CAN'T BE USED HERE BECAUSE IT CAN BE NEGATIVE AND THE DATA FILE DO NOT ACCOUNT FOR THAT. PLUS HEAD AND TAIL HAVE THE SAME VALUE (BAD!)
	float rmax = 10.0f;
	if ((rangeData) && (rangeData->numAltBreakpoints > 0 && rangeData->numAltBreakpoints < 100)) //RERUN added check numAltBreakpoint to make sense, else it can CTD.
	{
		// FRB - CTD's
...
*/
	...
	float rng = (float)(fabs((static_cast<SimVehicleClass*>(parent.get())->ZPos() - OTWDriver.GetGroundLevel(x, y)) 
											/ (tan(static_cast<SimVehicleClass*>(parent.get())->Pitch() - 0.01))));
	float dx = (float)(sin(static_cast<SimVehicleClass*>(parent.get())->Yaw()) * rng);
	float dy = (float)(cos(static_cast<SimVehicleClass*>(parent.get())->Yaw()) * rng);
