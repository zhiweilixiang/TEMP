...
#include "classtbl.h" //RERUN for RDR_NO_RADAR
...
void MissileClass::RunSeeker(){	
	...
	//if ((sensorArray == NULL) || (F4IsBadReadPtr(sensorArray[0], sizeof(sensorArray[0]))))
	if ((sensorArray == NULL) || (*sensorArray == NULL))
		return; //RERUN
	if (launchState == PreLaunch && parent && parent->IsAirplane() &&
...
bool MissileClass::GoActive(void)
{ 
	if (GetRadarType() == RDR_NO_RADAR)
		return false; //RERUN Type 0 is RDR_NO_RADAR -- we'd better not be going active without a radar...
	...
	//RERUN ShiAssert( GetRadarType() );	// Type 0 is RDR_NO_RADAR -- we'd better not be going active without a radar...
