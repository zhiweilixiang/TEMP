...
int BombClass::Exec (void)
{
	...
	else if (SimDriver.MotionOn())
	{
		...
		else if (((flags & IsGPS) || (flags & IsJSOW)) && (SimLibElapsedTime - timeOfDeath) > (2.0f * SEC_TO_MSEC))  
		{
			...
			static float maxenergy = (float)sqrt(pow(XDelta(), 2) + pow(YDelta(), 2));
			...
			float newzdelta= ZDelta() - (float)(auxData->JDAMLift * SimLibMajorFrameTime * cos(Pitch()));		// Cobra - Use Bombdata JDAMLift			 
			float newenergy = (float)sqrt(pow(newxdelta, 2) + pow(newydelta, 2));
	  ...
	  if ( bombType == None &&  z - terrainHeight > -800.0f)
	  {
		  ...
		  if (hitObj)
		  {
			  ...
			  {
				  //me123 addet arming check, for now digi's dont have arming delay, because they will mess up the delivery
...
//#else
		//RERUN Re-enabled the following line that sets targetPtr again. It was commented out as well and caused targetPtr->Reference to crash.
...
void BombClass::ApplyProximityDamage ( float groundZ, float detonateHeight )
{
	...
	float modifier = 1.0F;
	if (wc && wc->DamageType == DamType::NuclearDam)
	...
	if (/*parentReferenced && */SimDriver.objectList){		
		...
		switch (wc->DamageType){
			case DamType::PenetrationDam:
			case DamType::HeaveDam:
			case DamType::KineticDam:
			case DamType::IncendairyDam:
			case DamType::ChemicalDam:
				damageMod = 1.0F;  // Cobra - no penalties
				//damageMod = 0.25F;
				break;
			case DamType::HighExplosiveDam:
			case DamType::ProximityDam:
			case DamType::HydrostaticDam:
			case DamType::OtherDam:
			case DamType::NoDamage:
			default:
				damageMod = 1.0F;
				break;
		}
		...
		while (testObject){
			...
			if ( wc->DamageType != DamType::NuclearDam && (testObject == parent &&
			...
			if (testObject != this)
			{
				...
				if(testObject && testObject->IsAirplane() && wc && wc->DamageType == DamType::NuclearDam)
				{
				...
				else if (wc && wc->DamageType == DamType::NuclearDam)
				{
