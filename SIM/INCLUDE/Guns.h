...
public:
	void SetTrailID(int ID) {trailID = ID;} // MLR 12/13/2003 - DrawableTrailID, has to be set before Exec() is called;
	void InitTracers();
	void CleanupTracers();
	GunClass(int type);
	virtual ~GunClass();
...
public:
	...
	void SetTracerCollisionMode( TracerCollisionMode mode ){ tracerMode = mode; }
