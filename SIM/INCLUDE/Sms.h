...
public:
	...
	bool GetHARMPowerState(void) { return HARMPowered; }
	float GetHARMInitTimer(void) { return HARMInitTimer; }
	void ToggleHARMPower(void) { HARMPowered = !HARMPowered; }

	int GetCurrentHardpoint(void) {return curHardpoint;};
...
public:
	int IsFiringRockets( void ) { return runRockets; }
	...
	virtual void AddWeaponGraphics (void);
	virtual void FreeWeaponGraphics (void);
	virtual SimWeaponClass* GetCurrentWeapon (void) {return curWeapon.get();};
	...
	void NextAGBProfile(void) { curProfile = (curProfile + 1) & 1; }
	void SetAGBProfile(int i) { curProfile = i; }

	int GetAGBRippleCount(void)    { return agbProfile[curProfile].rippleCount;     }
	int GetAGBRippleInterval(void) { return agbProfile[curProfile].rippleInterval;  }
	int GetAGBFuze(void)           { return agbProfile[curProfile].fuzeNoseTail;    }
	int GetAGBBurstAlt(void)       { return agbProfile[curProfile].burstAltitude;   }
	int GetAGBReleaseAngle(void)   { return agbProfile[curProfile].releaseAngle;    }
	float GetAGBC1ArmDelay1(void)  { return agbProfile[curProfile].C1ArmDelay1;     }
	float GetAGBC1ArmDelay2(void)  { return agbProfile[curProfile].C1ArmDelay2;     }
	float GetAGBC2ArmDelay(void)   { return agbProfile[curProfile].C2ArmDelay;      }
	bool GetAGBPair(void)          { return agbProfile[curProfile].releasePair;     }
	FireControlComputer::FCCSubMode  GetAGBSubMode(void)        { return agbProfile[curProfile].subMode;         }

	void SetAGBRippleCount(int x)     { agbProfile[curProfile].rippleCount    = x; }
	void SetAGBRippleInterval(int x) { agbProfile[curProfile].rippleInterval = x; }
	void SetAGBFuze(int x)            { agbProfile[curProfile].fuzeNoseTail   = x; }
	void SetAGBBurstAlt(int x)        { agbProfile[curProfile].burstAltitude  = x; }
	void SetAGBReleaseAngle(int x)    { agbProfile[curProfile].releaseAngle   = x; }
	void SetAGBC1ArmDelay1(float x)   { agbProfile[curProfile].C1ArmDelay1    = x; }
	void SetAGBC1ArmDelay2(float x)   { agbProfile[curProfile].C1ArmDelay2    = x; }
	void SetAGBC2ArmDelay(float x)    { agbProfile[curProfile].C2ArmDelay     = x; }
	void SetAGBPair(bool x)           { agbProfile[curProfile].releasePair    = x; }
