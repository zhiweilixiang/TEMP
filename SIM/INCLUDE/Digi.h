...
public:
	...
	void SetBvrCurrProfile(BVRProfileType newProfile) { bvrCurrProfile = newProfile; };
	float PIDLoop(
	...
	float GetHoldAltitude ()							{ return holdAlt; };
	void SetHoldHeading (float newPsi)					{holdPsi = newPsi;};
	float GetHoldHeading ()								{return holdPsi; };
	...
	MissionClassEnum MissionClass(void)					{return missionClass;}
	...
	void	SetTrackPoint(float x, float y){ trackX = x; trackY=y; }
	void	SetTrackPoint(float x, float y, float z){ SetTrackPoint(x, y); trackZ = z; }
...
public:
	...
	AG_DOCTRINE GetAGDoctrine(void) {return agDoctrine; } // 2002-03-08 MN so leader can check if wingmen are still engaging a ground target
