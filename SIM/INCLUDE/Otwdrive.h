...
{
  public:
	  drawPtrList() : drawPointer(), next(), prev()	{value = 0.0f; };
	~drawPtrList()	{};
...
public:
		...
		// I put this little state machine (SM) here.. on pressing the 'takePrettyScreenPicture' key, I set the
		...
		SimBaseClass* GraphicsOwnship () const { return otwPlatform.get(); }
		SimBaseClass* GetTrackPlatform() const { return otwTrackPlatform.get(); }
		void SetTrackPlatform(SimBaseClass *sb){ otwTrackPlatform.reset(sb); }
...
public:
	  ...
	  bool IsProfilerDisplaying() { return DisplayProfiler; }	// Retro 21Dec2003
	  ...
	  void GetCameraPanTilt(float *pan, float *tilt) {*pan = eyePan, *tilt = eyeTilt;};	//Wombat778 3-19-04
	  ...
	  // Retro 23Dec2003 Start
		struct CamDisplacement {
			...
			CamDisplacement(float dir, float max, float scal)
				{ direction = dir; maxDispl = max; scale = scal; Displ = 0.F; }
		};
...
public:
	...
	float GetFPS(){return (currentFPS);};//Cobra
...
public:
	DrawableBSP *GetVirtualCockpit(){ return vrCockpit; }
