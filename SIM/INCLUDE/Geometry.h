...
public:
      ...
      void *operator new(size_t size) { ShiAssert( size == sizeof(ObjectGeometry) ); return MemAllocFS(pool);	};
      void operator delete(void *mem) { if (mem) MemFreeFS(mem); };
      static void InitializeStorage()	{ pool = MemPoolInitFS( sizeof(ObjectGeometry), 100, 0 ); };
...
public:
	ObjectGeometry()
	{
		sinalp = cosalp = tanalp =
		sinbet = cosbet = tanbet =
		singam = cosgam =
		sinsig = cossig =
		sinmu = cosmu =
		sinthe = costhe =
