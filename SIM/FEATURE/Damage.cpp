...
//RV - I-Hawk - Added a 0 vector for RV new PS calls
	...
	switch (damageMessage->dataBlock.damageType)
		...
		case FalconDamageType::BombDamage:
			...
			hitPoints = (float)wc->Strength * ((float)fc->DamageMod[ (int)wc->DamageType ])/100.0f;
		...
		case FalconDamageType::ProximityDamage:
			hitPoints = (float)damageMessage->dataBlock.damageStrength * ((float)fc->DamageMod[(int)DamType::HighExplosiveDam])/100.0f;
			break;
	...
	if (pctStrength <= 0.0F)
		...
		if ((Status() & VIS_TYPE_MASK) != VIS_DESTROYED)
	   		...
	   		if ( gACMIRec.IsRecording() )
			{
				...
				featStat.data.prevStatus = (Status() & VIS_TYPE_MASK);
	...
	if ( pctDamage > 0.0f && IsAwake() && drawPointer )
	{
		...
		if ( pctStrength > 0.0f )
		{
			...
			if ( pctDamage * PRANDFloatPos() > 0.02f )
			{
				...
				else if ( (fc->Flags & FEAT_CAN_SMOKE) )
				{
		...
		if ( pctStrength <= 0.0f && !IsSetFlag( SHOW_EXPLOSION ) )
		{
			...
			// if ( fc->Flags & FEAT_CAN_EXPLODE )
			if ( fc->Flags & (FEAT_CAN_BURN | FEAT_CAN_EXPLODE ) )
			{
				ppos = pos;
				ppos.z += minB.z * 0.15f;
