...
void AircraftClass::MakeNonPlayerVehicle(){
	...
	if (
		!HasPilot() || 
		((DBrain()->ATCStatus() < AtcStatusEnum::tReqTaxi) && OnGround() && !af->IsSet(AirframeClass::OnObject))
	){
		...
		if ((DBrain()->ATCStatus() != AtcStatusEnum::lLanded) && (DBrain()->ATCStatus() != AtcStatusEnum::lTaxiOff)){
