...
int ClosestApproachLinePoint(Tpoint &A, Tpoint &B, Tpoint &C, float &PercentOfAB) // AB is the line, C is the point
{
	...
	AC.y = C.y-A.y;
	AC.z = C.z-A.z;
	//RERUN float lengthAC = sqrt(AC.x*AC.x + AC.y*AC.y + AC.z*AC.z);
	float lengthAC = (float)(sqrt(pow(AC.x, 2) + pow(AC.y, 2) + pow(AC.z, 2))); //RERUN avoid possible loss of data
	Tpoint ACnorm;
	...
	AB.y = B.y-A.y;
	AB.z = B.z-A.z;
	//RERUN float lengthAB = sqrt(AB.x*AB.x + AB.y*AB.y + AB.z*AB.z);
	float lengthAB = (float)(sqrt(pow(AB.x, 2) + pow(AB.y, 2) + pow(AB.z, 2))); //RERUN avoid possible loss of data
	Tpoint ABnorm;
...
float TurbRecordNode::RetieveTurbulence(RetrieveTurbulanceParams &rtp)
{
	...
	if(ClosestApproachLinePoint(Start.position,End.position,rtp.pos,fraction))
	{
		...
		//RERUN float distance = sqrt(delta.x*delta.x + delta.y*delta.y + delta.z*delta.z) + 1;
		float distance = (float)(sqrt(pow(delta.x, 2) + pow(delta.y, 2) + pow(delta.z, 2))) + 1; //RERUN avoid loss of data warning
