	...
	for(int i = 0; i < nTrails; i++)
	{
		...
		//trail is contrail... use 10% of ContrailLow value as margin for fade-in/out
			...
			if ( -ZPos() > (contrailHighValue) && -ZPos() < contrailHigh110Percent ){
				engineTrailMargin *= (float)( fabs( -ZPos() - (double)(contrailHigh110Percent)) ) / contrailLow10Percent;
			}
	   ...
	   if(dotrails ||  doturb)
	   {
		   ...
		   if(orientation)
		   {
			   ...
			   if(af->auxaeroData->hasSwingWing)
			   {
				   ...
				   tp.x = (float)(x * cos((double)swingWingAngle) - y * sin((double)swingWingAngle) + af->auxaeroData->swingWingHinge.x);
				   tp.y = (float)(x * sin((double)swingWingAngle) + y * cos((double)swingWingAngle) + af->auxaeroData->swingWingHinge.y);
			   }
			...
			if ((ZPos() <= minB.z * 0.96f && ZPos() > minB.z - 0.5f) || (ZPos() > -g_fCarrierStartTolerance && af->vcas < 10.0f)) {
				...
				if(af->IsSet(af->GearBroken) || af->gearPos <= 0.1F) {
					if (af->platform->DBrain() && !af->platform->IsSetFalcFlag(FEC_INVULNERABLE)) {
						...
						af->platform->DBrain()->SetATCStatus(AtcStatusEnum::lCrashed);
