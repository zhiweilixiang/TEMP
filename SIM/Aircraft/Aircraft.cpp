...
#else
	memset(acmiDOFValue, 0, sizeof(float) * COMP_MAX_DOF);
	memset(acmiSwitchValue, 0, sizeof(int) * COMP_MAX_SWITCH);
#endif
...
#else
	memset(smokeTrail, 0, sizeof(DWORD) * TRAIL_MAX);
	memset(conTrails, 0, sizeof(DWORD) * MAXENGINES);
	memset(engineTrails, 0, sizeof(DWORD) * MAXENGINES);
#endif
...
#else
	PulseTurbulence = { 0.0f, 0.0f, 0.0f };
	TotalTurbulence = { 0.0f, 0.0f, 0.0f };
	StaticTurbulence = { 0.0f, 0.0f, 0.0f };
#endif
...
int AircraftClass::Exec(void)
{
	...
	if (IsExploding()){
			...
			&& !OTWDriver.IsShutdown()) // JB - come on, practice safer viewpoints ;-)
		{
			...
			{
				//in multiplay this sometimes gets messed up for the player flight
		...
		if (flight && (flight->GetUnitMission() > MissionTypeEnum::AMIS_SEADESCORT && flight->GetUnitMission() < MissionTypeEnum::AMIS_FAC) )
		{
...
//   		if (FCC->GetMasterMode() != FireControlComputer::AirGroundBomb)
   ...
   if ( !IsDead() )
	...
	// sfr: TODO must go to initialization. HACK
	if(Sms)
	{
		if ((this == SimDriver.GetPlayerEntity()) && (autopilotType == CombatAP) && (Sms->MasterArm() == SMSBaseClass::Safe) && !OnGround())
		{
		...
		ConvertSimToGrid(&pos, &gx, &gy);
 		...
 		if (nearest){
			...
			BIG_SCALAR yd = nearest->YPos() - af->y;
 			dist = (float)(sqrt(pow(xd, 2) + pow(yd, 2))*FT_TO_NM);
