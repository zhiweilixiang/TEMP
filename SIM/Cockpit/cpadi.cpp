...
void RenderADIPoly(tagRECT *srcrect, tagRECT *srcloc, tagRECT *destrect, GLint alpha, TextureHandle *pTex, float angle)		//Wombat778 3-22-04 helper function to keep the displayblit3d tidy. 
{
	...
	float x1, y1;
	const float angsin = (float)sin(angle);
	const float angcos = (float)cos(angle);
...
void CPAdi::DisplayBlit3D()
{
	...
	if(mDoBackRect)
	{
		...
		TwoDVertex pVtx[4];
		ZeroMemory(pVtx, sizeof(pVtx));
		pVtx[0].x = (float)mBackDest.left; pVtx[0].y = (float)mBackDest.top; pVtx[0].u = fStartU; pVtx[0].v = fStartV;
		pVtx[0].r = pVtx[0].g = pVtx[0].b = pVtx[0].a = 1.0f;
		pVtx[1].x = (float)mBackDest.right; pVtx[1].y = (float)mBackDest.top; pVtx[1].u = fMaxU; pVtx[1].v = fStartV;
		pVtx[1].r = pVtx[1].g = pVtx[1].b = pVtx[1].a = 1.0f;
		pVtx[2].x = (float)mBackDest.right; pVtx[2].y = (float)mBackDest.bottom; pVtx[2].u = fMaxU; pVtx[2].v = fMaxV;
		pVtx[2].r = pVtx[2].g = pVtx[2].b = pVtx[2].a = 1.0f;
		pVtx[3].x = (float)mBackDest.left; pVtx[3].y = (float)mBackDest.bottom; pVtx[3].u = fStartU; pVtx[3].v = fMaxV;
