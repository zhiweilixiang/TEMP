  ...
  Flight jstar = theFlight->GetJSTARFlight();
	...
	if (!jstar) {
		...
		while (nu && !jstar) {
			...
			if (cf->GetUnitMission() == MissionTypeEnum::AMIS_JSTAR && cf->GetTeam() == us && cf->GetUnitTOT()+5*CampaignMinutes < Camp_GetCurrentTime() && cf->GetUnitTOT()+95*CampaignMinutes > Camp_GetCurrentTime()) {
