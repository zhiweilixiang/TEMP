...
void FireControlComputer::CalculateRocketImpactPoint(void)
{
	...
	if ( fccWeaponPtr )
	{
		...
		{
			noSolution = !((MissileClass *)fccWeaponPtr.get())->FindRocketGroundImpact(
				&ImpactX,
				&ImpactY,
				&ImpactZ,
				&ImpactTime );
		...
		else
		{
			...
			{   
				...
				*/
				...
				if(rocketPointer)
				{
					noSolution = !rocketPointer->FindRocketGroundImpact(
																					&ImpactX,
																					&ImpactY,
																					&ImpactZ,
																					&ImpactTime );
   ...
   if ((Sms->CurHardpoint() >= 0) && (Sms->hardPoint[Sms->CurHardpoint()]->GetWeaponData()->flags & SMSClass::Loftable))
   {
	  ...
	  airGroundMaxRange += radius * trig.cos;
		...
		if (isJSOW) {
			...
			float angleFactor = (float)cos(min(abs(groundDesignateAz), 90.0*DTR)*2.0);
