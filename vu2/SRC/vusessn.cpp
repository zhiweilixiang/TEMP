...
int VuTargetEntity::GetMessages() {
	...
	if (reliableComms_.handle_ && reliableComms_.status_ == VU_CONN_ACTIVE){
		...
		if (ping == -1) {
			...
			if (
				session && 
				((FalconSessionEntity*)vuDatabase->Find(this->Id()))->GetFlyState () !=  FLYSTATE_FLYING
			){
				session = NULL;//session is not in cockpit
			}

			if (session && 
				((FalconSessionEntity*)vuDatabase->Find(vuLocalSessionEntity->Game()->OwnerId()))->GetFlyState() !=  FLYSTATE_FLYING
			){
...
int VuSessionEntity::Save(VU_BYTE** stream){
	...
	//**stream = len;
	memcpy(*stream, &len, sizeof(VU_BYTE));             *stream += sizeof(VU_BYTE);
	memcpy(*stream, callsign_, len);      *stream += len;

	// camera
	unsigned char cameraCount = static_cast<unsigned char>(cameras_.size());
	memcpy(*stream, &cameraCount, sizeof(cameraCount)); *stream += sizeof(cameraCount);
	for (unsigned char i=0;i<cameraCount;++i){
		VU_ID id = cameras_[i]->Id();
		memcpy(*stream, &id, sizeof(id)); *stream += sizeof(id);
	}
...
int VuSessionEntity::RemoveCamera(VuEntity *e, bool silent){
	...
	for (VuEntityVectorIterator it=cameras_.begin();it!=cameras_.end();++it){
		VuEntity *ce = it->get();
		if (ce->Id() == e->Id()){
			cameras_.erase(it);
			if (!silent){
...
void VuSessionEntity::SendBestEnqueuedPositionUpdatesAndClear(unsigned int qty, VU_TIME timestamp){
	...
	for (
		...
		it != oobPositionUpdateQ_.end();
		++it
	){
		VuEntity *e = *it;
		// avoid duplicate sends
		if (!e->EnqueuedForPositionUpdate()){
...
VU_ERRCODE VuGroupEntity::AddSession(VuSessionEntity *session) {
	...
	if (!sessionCollection_->Find(session)) {
