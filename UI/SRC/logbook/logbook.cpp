...
int LogBookData::Load(void)
{
	if (strlen(g_strLgbk) != 0){
		...
		fp = CreateFile("logbook.bin", GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		...
		ReadFile(fp, (LPBYTE)Pilot.Name, _NAME_LEN_ + 1, &br, NULL);
		ReadFile(fp, (LPBYTE)Pilot.Callsign, _CALLSIGN_LEN_ + 1, &br, NULL);
		CloseHandle(fp);
#endif
	}
...
int LogBookData::SaveData(void)
{
	...
	fileHandle = CreateFile("logbook.bin", GENERIC_WRITE, FILE_SHARE_WRITE, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	if (fileHandle == INVALID_HANDLE_VALUE)
		return FALSE;

	WriteFile(fileHandle, (LPBYTE)Name(), _NAME_LEN_ + 1, &br, NULL);
	WriteFile(fileHandle, (LPBYTE)Callsign(), _CALLSIGN_LEN_ + 1, &br, NULL);
