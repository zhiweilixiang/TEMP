...
void LBSetupControls(IMAGE_RSC *Picture, IMAGE_RSC *Patch)
{
	...
	if(win)
	{
		...
		if(button)
		{
			...
			{
				_TCHAR buf[MAX_PATH];
				//RERUN _stprintf(buf,_T("%s\\patches\\%s.tga"),FalconDataDirectory,Patch);
				_stprintf(buf, _T("%s\\patches\\%s.tga"), FalconDataDirectory, Patch->GetImage()); //RERUN, just a guess...
