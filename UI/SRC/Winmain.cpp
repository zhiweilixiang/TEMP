...
int shiAssertsOn= FALSE, 
	shiWarningsOn = FALSE, // JB 010325
...
extern JoinStatusEnum gCampJoinStatus;
...
//	  		}
		...
		case FM_JOIN_CAMPAIGN:
			if (lTestVar){
			   ...
			   if (gCommsMgr)
				   ...
				   switch (wParam)
					   {
					   case (int)JoinStatusEnum::JOIN_PRELOAD_ONLY:			// Preload only
						   MonoPrint("Requesting campaign preload.\n");
						   retval = TheCampaign.RequestScenarioStats(game);
						   break;
					   case (int)JoinStatusEnum::JOIN_REQUEST_ALL_DATA:		// Request all game data
						   MonoPrint("Requesting all campaign data.\n");
						   retval = TheCampaign.RequestScenarioStats(game);
						   break;
					   case (int)JoinStatusEnum::JOIN_CAMP_DATA_ONLY:		// Request only non-preload data (Called by Campaign only)
		...
		case FM_GAME_FULL:
			...
			DisplayJoinStatusWindow ((ulong)CampStatusEnum::CAMP_GAME_FULL);
			CampaignJoinFail ();
		break;
...
*/
		...
		case FM_GOT_CAMPAIGN_DATA:
			switch(wParam)
				...
				// with it.
				case (ulong)CampStatusEnum::CAMP_NEED_PRELOAD:
					...
					SetCursor(gCursors[CRSR_F16]);
					break;
				case (ulong)CampStatusEnum::CAMP_NEED_ENTITIES:
					...
					break;
				case (ulong)CampStatusEnum::CAMP_NEED_WEATHER:
					if( !FalconLocalGame ||  vuPlayerPoolGroup == vuLocalGame)
						break;
					gCampJoinLastData=vuxRealTime;
				...
				case (ulong)CampStatusEnum::CAMP_NEED_PERSIST:
					if( !FalconLocalGame ||  vuPlayerPoolGroup == vuLocalGame)
						break;
					gCampJoinLastData=vuxRealTime;
					...
					DisplayJoinStatusWindow (wParam);
					break;
				case (ulong)CampStatusEnum::CAMP_NEED_OBJ_DELTAS:
					if( !FalconLocalGame || vuPlayerPoolGroup == vuLocalGame)
						break;
					gCampJoinLastData=vuxRealTime;
					gCampJoinTries = 0;
					...
					DisplayJoinStatusWindow (wParam);
					break;
				case (ulong)CampStatusEnum::CAMP_NEED_TEAM_DATA:
					if( !FalconLocalGame ||  vuPlayerPoolGroup == vuLocalGame)
						break;
					gCampJoinLastData=vuxRealTime;
					gCampJoinTries = 0;
					...
					DisplayJoinStatusWindow (wParam);
					break;
				case (ulong)CampStatusEnum::CAMP_NEED_UNIT_DATA:
					if( !FalconLocalGame ||  vuPlayerPoolGroup == vuLocalGame)
						break;
					gCampJoinLastData=vuxRealTime;
					...
					DisplayJoinStatusWindow (wParam);
					break;
				case (ulong)CampStatusEnum::CAMP_NEED_VC:
					if( !FalconLocalGame ||  vuPlayerPoolGroup == vuLocalGame)
						break;
					gCampJoinLastData=vuxRealTime;
					...
					DisplayJoinStatusWindow (wParam);
					break;
				case (ulong)CampStatusEnum::CAMP_NEED_PRIORITIES:
					if( !FalconLocalGame ||  vuPlayerPoolGroup == vuLocalGame)
						break;
					gCampJoinLastData=vuxRealTime;
...
void ShutdownCampaign (void)
{
	...
	SetTimeCompression(0);
	DisableCampaignMenus();
	gCampJoinStatus = JoinStatusEnum::JOIN_NOT_JOINING;
}
...
int i_am (char *with){
	...
	size = 63;
	retval = RegOpenKeyEx (HKEY_LOCAL_MACHINE, "Network\\Logon", 0, KEY_READ/* RERUN was KEY_ALL_ACCESS*/, &key);
