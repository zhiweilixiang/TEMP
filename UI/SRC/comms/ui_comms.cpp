...
if (g_nShowDebugLabels)
{
	...
	if ((arga && argb && !stricmp (arga, ".sfx"))||(arga && !stricmp (arga, "."))) 
	{
		...
		vc.x=(float)(ObserverPosition.x-Dist*cos(Pan)*cos(Tilt));
		vc.y=(float)(ObserverPosition.y-Dist*sin(Pan)*cos(Tilt));
		vc.z=(float)(ObserverPosition.z+Dist*sin(Tilt));
	...
	if (g_bServer) {
		if (arga && argb && !stricmp (arga, ".loadte")) {
			...
			TheCampaign.Flags |= (ulong)CampStatusEnum::CAMP_TACTICAL;
			if(!TACSelLoaded)
