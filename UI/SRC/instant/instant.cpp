...
#define _USE_REGISTRY_ 0 //RERUN 0=No,1=Yes
...
static void HookupIAControls(long ID)
{
	...
	C_EditBox *ebox;
	C_Clock *clk;
//RERUN	long hour; //THW 2004-02-18 Random daytime
...
#else
		SYSTEMTIME st;
		GetLocalTime(&st); //Time where you are. Or use GetSystemTime(&st) for UTC time.
		clk->SetHour(st.wHour); //Btw, UTC+9 is Korean standard time. 
		clk->SetMinute(st.wMinute);
		clk->SetSecond(st.wSecond);
#endif
		clk->Refresh();
	}
