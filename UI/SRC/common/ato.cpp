...
void ChangeFlightTypeCB (long, short hittype, C_Base *control)
{
	...
	type = static_cast<MissionTypeEnum>(listbox->GetTextID ());
...
C_ATO_Flight *BuildATOFlightInfo(Flight fl)
{
	...
	newinfo->SetCurrentTask((uchar)fl->GetUnitMission());
	...
	if (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT)
		...
		int	i,added;
		for (i=1,added=0;i< (uchar)MissionTypeEnum::AMIS_OTHER;i++)
			{
			if (IsValidMission(fl->Type()-VU_LAST_ENTITY_TYPE,(MissionTypeEnum)i))
				{
				lbox->AddItem(i,C_TYPE_ITEM,MissStr[i]);
				if (!added || i == (uchar)mission)
					{
					lbox->SetValue(i);
					added=1;
					}
		...
		lbox->AddItem((uchar)mission,C_TYPE_ITEM,MissStr[(uchar)mission]);
		lbox->SetValue((uchar)mission);
...
C_ATO_Package *BuildATOPackageInfo(Package pkg)
{
	...
	if(pkg->GetFirstUnitElement())
	{
		...
		_stprintf(buffer,"%s %1d - %s",gStringMgr->GetString(TXT_PACKAGE),pkg->GetCampID(),MissStr[(uchar)mistype]);
...
C_ATO_Package *AddPackagetoATO(Package FltPkg)
{
	...
	if(mistype == MissionTypeEnum::AMIS_ABORT || mistype == MissionTypeEnum::AMIS_ALERT)
		return(NULL);
	...
	atopkg=NULL;
	mistype=MainFlt->GetUnitMission();
	if(mistype == MissionTypeEnum::AMIS_BARCAP2)
		mistype = MissionTypeEnum::AMIS_BARCAP;
	...
	{
		txt=new C_Text;
		txt->Setup((uchar)mistype | 0x40000000,0);
...
C_ATO_Flight *AddFlighttoATO(Flight flt)
{
	...
	if(flt->GetUnitMission() == MissionTypeEnum::AMIS_ALERT || flt->GetUnitMission() == MissionTypeEnum::AMIS_ABORT)
		return(NULL);
