...
C_Entity *BuildUnitParent(Unit unit)
{
	...
	if (!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT) && unit->IsFlight() && gGps->GetTeamNo() >= 0 && unit->GetTeam() != gGps->GetTeamNo() && !unit->GetIdentified(static_cast<uchar>(gGps->GetTeamNo())))
...
C_Feature *BuildUnit(Unit un,long vehno,long vehid,Tpoint *)
{
	...
	if (classID)
	{
		...
		veh->InitEntity();
		// 2002-02-21 ADDED BY S.G. 'Fog of war code'. If an enemy flight and not identified and not editing a TE, change its label to 'Bandit'
		if (!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT) && un->IsFlight() && gGps->GetTeamNo() >= 0 && un->GetTeam() != gGps->GetTeamNo() && !un->GetIdentified(static_cast<uchar>(gGps->GetTeamNo())))
