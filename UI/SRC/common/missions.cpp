...
short GetFlightStatusID(Flight element)
{
	...
	if(wp)
	{
		...
		else
		{
			...
			while ((found == 0) && (wp))
			{
				...
				else if(wp->GetWPFlags() & WPF_TARGET)
				{
					if
					(
						element->GetUnitMission() == MissionTypeEnum::AMIS_BARCAP ||
						element->GetUnitMission() == MissionTypeEnum::AMIS_BARCAP2 ||
						element->GetUnitMission() == MissionTypeEnum::AMIS_HAVCAP ||
						element->GetUnitMission() == MissionTypeEnum::AMIS_RESCAP ||
						element->GetUnitMission() == MissionTypeEnum::AMIS_TARCAP ||
						element->GetUnitMission() == MissionTypeEnum::AMIS_AMBUSHCAP
					)
					{
						ID=_MIS_PATROL;
					}
		...
		if ((TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL) && (!found))
		{
			ID=_MIS_ENROUTE;
		}
	}
...
C_Mission *MakeMissionItem(C_TreeList *tree,Flight element)
{
	...
	if (ent_mission == MissionTypeEnum::AMIS_ALERT)
		ent_mission = MissionTypeEnum::AMIS_INTERCEPT;
	mission->SetMission(static_cast<short>(tree->GetUserNumber(C_STATE_1)),0,MissStr[(uchar)ent_mission]);
	...
	buffer[1]=0;
	// Get the priority from the package request
	...
	mission->SetUserNumber(C_STATE_3,(uchar)element->GetUnitMission());

	if (!element->Final() || element->GetUnitMission() == MissionTypeEnum::AMIS_ALERT)
	{
