...
void UI_Refresher::AddMission(CampEntity entity)
{
	...
	{
		if(((FlightClass*)entity)->GetUnitMission() != MissionTypeEnum::AMIS_ALERT)
		{
...
void UI_Refresher::UpdateMission(CampEntity entity)
{
	if(!entity->IsDead())
	{
		...
		if(((Flight)entity)->Final() && ((Flight)entity)->GetUnitMission() != MissionTypeEnum::AMIS_ALERT)
	...
	if(entity->IsUnit())
	{
		...
		if(entity->IsFlight())
		{
			if(!entity->IsDead())
			{
				...
				if (g_nUnidentifiedInUI && (!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT) && MapItem_ && entity->GetTeam() != Owner_->TeamNo_)) {
				...
				if (g_bAWACSSupport) {
				    ...
				    else if (g_nUnidentifiedInUI && (!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT) && MapItem_ && entity->IsFlight() && entity->GetTeam() != Owner_->TeamNo_ && !entity->GetIdentified(static_cast<uchar>(Owner_->TeamNo_))))
						strcpy(name, "Bandit");
					else {
						int vid = ((Unit)entity)->GetVehicleID(0);
						VehicleClassDataType *vc = GetVehicleClassData(vid);
