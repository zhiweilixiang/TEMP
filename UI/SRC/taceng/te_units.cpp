...
int MissionToATOMiss (MissionTypeEnum mistype);
...
void LoadTEUnits ()
{
	...
	for (int i = 0; table_of_equipment[i].side != 0; i++) 
	{
	    fprintf (fp, "%7d %7d %8d\n", 
...
void FillListBoxWithSquadrons(C_ListBox *lbox,long team,long aircraft_dindex)
{
	...
	if(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT)
	{
		// Add the new item
	...
	if(win)
	{
		...
		if (btn && btn->GetState() == 1 && !gPackageTOT)
			...
			if (clock)
				...
				if(!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT) && gTakeoffTime < TheCampaign.CurrentTime)
					{
...
void SetupPackageControls (C_Window	*win, C_Base *caller)
	...
	if(/*!(TheCampaign.Flags & (CAMP_TACTICAL|CAMP_TACTICAL_EDIT)) ||*/
	  (EdittingPackage && !(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT) && takeoff < TheCampaign.CurrentTime))
	{
	...
	if (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL)
	{
...
void tactical_add_package (VU_ID id,C_Base *caller)
{
	...
	if (!gLastAircraftType)
		gLastAircraftType = GetClassID(DOMAIN_AIR,CLASS_UNIT,TYPE_SQUADRON,STYPE_UNIT_FIGHTER_BOMBER,SPTYPE_F16C,VU_ANY,VU_ANY,VU_ANY) + VU_LAST_ENTITY_TYPE;
	gLastRole = (uchar)GetMissionFromTarget (gSelectedTeam, gLastAircraftType-VU_LAST_ENTITY_TYPE, ent);
...
void tactical_edit_package (VU_ID id,C_Base *caller)
{
	...
	if (flight)
		gLastRole = (uchar)flight->GetUnitMission();
...
void SetPackageTimes(Package new_package, CampaignTime takeoffTime, CampaignTime targetTime)
	...
	while (flight)
		...
		if (w && !flight->Moving())
			...
			if (count)
				delta += MissionData[(uchar)flight->GetUnitMission()].separation * CampaignSeconds;
			else if (targetTime)
				{
				while (w && !(w->GetWPFlags() & WPF_TARGET))
					w = w->GetNextWP();
				if (w)
					delta = (targetTime + MissionData[(uchar)flight->GetUnitMission()].separation*CampaignSeconds) - w->GetWPArrivalTime();
				}
			if (delta)
...
void tac_select_aircraft(long,short hittype,C_Base *control)
{
	...
	if(lbox)
		...
		// Clear old roles
		lbox->RemoveAllItems();
		for (i=1;i< (uchar)MissionTypeEnum::AMIS_OTHER;i++)
			{
			if (IsValidMission(ac_type,(MissionTypeEnum)i))
				{
...
void tac_select_role(long,short hittype,C_Base *control)
{
	...
	gLastRole = ((C_ListBox*)control)->GetTextID();
	mission = (MissionTypeEnum)gLastRole;
	...
	if(lbox)
		...
		if (ID > 1)
			...
			if(sqd) // Don't use sqd if NULL
			{
				...
				//if(airbase)
				if(airbase && !F4IsBadReadPtr(airbase, sizeof(CampBaseClass))) // JB 010326 CTD
				{
...
// M.N. Set the squadron's pilot experience
		...
		lbox=(C_ListBox*)win->FindControl(TAC_ROLE);
		if(lbox)
			mis.mission = static_cast<MissionTypeEnum>(lbox->GetTextID());
		else
			mis.mission = MissionTypeEnum::AMIS_TRAINING;
		if (!mis.tx || !mis.ty)
			{
...
// 2001-04-07 HACK ADDED By S.G. ALSO, WE MUST SET THE FLIGHT'S MISSION targetID TO WHATEVER TARGET WAS GIVEN IN 'target'
		if (mis.mission == MissionTypeEnum::AMIS_ECM) {
...
// END OF ADDED SECTION
		...
		if(!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT) && mis.tot < TheCampaign.CurrentTime)
...
*/
		if (flights)
			mis.tot += MissionData[(uchar)mis.mission].separation * CampaignSeconds;
...
void tactical_set_orders(Battalion bat, VU_ID obj, GridIndex tx, GridIndex ty)
{
	...
	if (!bat->IsLocal()) {
		...
		tmpId = o->Id();
		memcpy(dataptr, &tmpId, sizeof(tmpId));		dataptr += sizeof(tmpId);
		memcpy(dataptr, &x, sizeof(x));				dataptr += sizeof(x);	
		memcpy(dataptr, &y, sizeof(y));				dataptr += sizeof(y);	
