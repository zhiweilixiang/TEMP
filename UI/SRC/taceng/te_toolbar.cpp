...
void do_tactical_debrief (void)
{
	...
	if(current_tactical_mission->get_type() != tt_training)
	{
		...
		// discarded mission
		if (win && TheCampaign.MissionEvaluator && TheCampaign.MissionEvaluator->flight_data && TheCampaign.MissionEvaluator->flight_data->mission != MissionTypeEnum::AMIS_TRAINING)
		{
...
static void tactical_debriefing_print (long, short hittype, C_Base *ctrl)
{
	...
	TheCampaign.MissionEvaluator->flight_data->mission == MissionTypeEnum::AMIS_TRAINING)
	return;
    _TCHAR string[8192];
