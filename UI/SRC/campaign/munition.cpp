...
//				txt->Refresh();
//				txt->SetText(buf);
			if((takeoff / VU_TICS_PER_SECOND) > g_nLoadoutTimeLimit || (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT)) // JB 010729
			{
				txt->Refresh();
...
void SetupMunitionsWindow(VU_ID FlightID)
{
	...
	if(txt)
	{
		...
		if ((!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT)) && (!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_LIGHT)))
		{
