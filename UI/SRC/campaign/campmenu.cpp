...
void MenuToggleObjectiveCB(long ID,short,C_Base *control)
{
	...
	switch(ID)
	{
		case MID_INST_AF:
			...
			{
				gMapMgr->ShowObjectiveType(_OBTV_AIR_FIELDS);filterState[OBJ_AIRFIELDS] = true;
			}
			else
			{
				gMapMgr->HideObjectiveType(_OBTV_AIR_FIELDS);filterState[OBJ_AIRFIELDS] = false;
			}
			break;
		case MID_INST_AD:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_OBTV_AIR_DEFENSE);filterState[OBJ_AIRDEFENSE] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_OBTV_AIR_DEFENSE);filterState[OBJ_AIRDEFENSE] = false;
				}
			break;
		case MID_INST_ARMY:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_OBTV_ARMY);filterState[OBJ_ARMY] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_OBTV_ARMY);filterState[OBJ_ARMY] = false;
				}
			break;
		case MID_INST_CCC:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_OBTV_CCC);filterState[OBJ_CCC] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_OBTV_CCC);filterState[OBJ_CCC] = false;
				}
			break;
		case MID_INST_POLITICAL:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_OBTV_POLITICAL);filterState[OBJ_POLITICAL] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_OBTV_POLITICAL);filterState[OBJ_POLITICAL] = false;
				}
			break;
		case MID_INST_INFRA:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_OBTV_INFRASTRUCTURE);filterState[OBJ_INFRA] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_OBTV_INFRASTRUCTURE);filterState[OBJ_INFRA] = false;
				}
			break;
		case MID_INST_LOG:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_OBTV_LOGISTICS);filterState[OBJ_LOGISTICS] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_OBTV_LOGISTICS);filterState[OBJ_LOGISTICS] = false;
				}
			break;
		case MID_INST_WARPROD:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_OBTV_WAR_PRODUCTION);filterState[OBJ_WARPRODUCTION] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_OBTV_WAR_PRODUCTION);filterState[OBJ_WARPRODUCTION] = false;
				}
			break;
		case MID_INST_NAV:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_OBTV_NAVIGATION);filterState[OBJ_NAVIGATION] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_OBTV_NAVIGATION);filterState[OBJ_NAVIGATION] = false;
				}
			break;
		case MID_INST_OTHER:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_OBTV_OTHER);filterState[OBJ_OTHER] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_OBTV_OTHER);filterState[OBJ_OTHER] = false;
				}
			break;
		case MID_INST_NAVAL:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_OBTV_NAVAL);filterState[OBJ_NAVAL] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_OBTV_NAVAL);filterState[OBJ_NAVAL] = false;
				}
			break;
		case MID_SHOW_VC:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_VC_CONDITION_);filterState[OBJ_VICTORYCOND] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_VC_CONDITION_);filterState[OBJ_VICTORYCOND] = false;
				}
			break;
	}
	gMapMgr->DrawMap();
...
void MenuToggleUnitCB(long ID,short,C_Base *control)
{
	...
	{
		case MID_UNITS_DIV:
			gMapMgr->SetUnitLevel(0); filterState[UNITS_DIV] = true; filterState[UNITS_BRIG] = filterState[UNITS_BAT] = false;
			break;
		case MID_UNITS_BRIG:
			gMapMgr->SetUnitLevel(1); filterState[UNITS_BRIG] = true; filterState[UNITS_DIV] = filterState[UNITS_BAT] = false;
			break;
		case MID_UNITS_BAT:
			gMapMgr->SetUnitLevel(2); filterState[UNITS_BAT] = true; filterState[UNITS_DIV] = filterState[UNITS_BRIG] = false;
			break;
		case MID_UNITS_COMBAT:
			if(((C_PopupList *)control)->GetItemState(ID))
			{
				gMapMgr->ShowUnitType(_UNIT_COMBAT); filterState[UNITS_COMBAT] = true;
			}
			else
			{
				gMapMgr->HideUnitType(_UNIT_COMBAT); filterState[UNITS_COMBAT] = false;
			}
			break;
		case MID_UNITS_AD:
			if(((C_PopupList *)control)->GetItemState(ID))
			{
				gMapMgr->ShowUnitType(_UNIT_AIR_DEFENSE); filterState[UNITS_AIR_DEFENSE] = true;
			}
			else
			{
				gMapMgr->HideUnitType(_UNIT_AIR_DEFENSE); filterState[UNITS_AIR_DEFENSE] = false;
			}
			break;
		case MID_UNITS_SUPPORT:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowUnitType(_UNIT_SUPPORT); filterState[UNITS_SUPPORT] = true;
				}
			else
				{
				gMapMgr->HideUnitType(_UNIT_SUPPORT); filterState[UNITS_SUPPORT] = false;
				}
			break;
		case MID_UNITS_SQUAD_SQUADRON:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_UNIT_SQUADRON); filterState[UNITS_SQUAD_SQUADRON] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_UNIT_SQUADRON); filterState[UNITS_SQUAD_SQUADRON] = false;
				}
			break;
		case MID_UNITS_SQUAD_PACKAGE:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowObjectiveType(_UNIT_PACKAGE); filterState[UNITS_SQUAD_PACKAGE] = true;
				}
			else
				{
				gMapMgr->HideObjectiveType(_UNIT_PACKAGE); filterState[UNITS_SQUAD_PACKAGE] = false;
				}
			break;
		case MID_UNITS_SQUAD_FIGHTER:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowAirUnitType(_UNIT_FIGHTER); filterState[UNITS_SQUAD_FIGHTER] = true;
				}
			else
				{
				gMapMgr->HideAirUnitType(_UNIT_FIGHTER); filterState[UNITS_SQUAD_FIGHTER] = false;
				}
			break;
		case MID_UNITS_SQUAD_ATTACK:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowAirUnitType(_UNIT_ATTACK); filterState[UNITS_SQUAD_ATTACK] = true;
				}
			else
				{
				gMapMgr->HideAirUnitType(_UNIT_ATTACK); filterState[UNITS_SQUAD_ATTACK] = false;
				}
			break;
		case MID_UNITS_SQUAD_BOMBER:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowAirUnitType(_UNIT_BOMBER); filterState[UNITS_SQUAD_BOMBER] = true;
				}
			else
				{
				gMapMgr->HideAirUnitType(_UNIT_BOMBER); filterState[UNITS_SQUAD_BOMBER] = false;
				}
			break;
		case MID_UNITS_SQUAD_SUPPORT:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowAirUnitType(_UNIT_SUPPORT); filterState[UNITS_SQUAD_SUPPORT] = true;
				}
			else
				{
				gMapMgr->HideAirUnitType(_UNIT_SUPPORT); filterState[UNITS_SQUAD_SUPPORT] = false;
				}
			break;
		case MID_UNITS_SQUAD_HELI:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowAirUnitType(_UNIT_HELICOPTER); filterState[UNITS_HELICOPTER] = true;
				}
			else
				{
				gMapMgr->HideAirUnitType(_UNIT_HELICOPTER); filterState[UNITS_HELICOPTER] = false;
				}
			break;
		...
		case MID_UNITS_NAVY_COMBAT:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowNavalUnitType(_UNIT_COMBAT); filterState[UNITS_NAVY_COMBAT] = true;
				}
			else
				{
				gMapMgr->HideNavalUnitType(_UNIT_COMBAT); filterState[UNITS_NAVY_COMBAT] = false;
				}
			break;
		case MID_UNITS_NAVY_SUPPLY:
			if(((C_PopupList *)control)->GetItemState(ID))
				{
				gMapMgr->ShowNavalUnitType(_UNIT_SUPPORT); filterState[UNITS_NAVY_SUPPORT] = true;
				}
			else
				{
				gMapMgr->HideNavalUnitType(_UNIT_SUPPORT); filterState[UNITS_NAVY_SUPPORT] = false;
				}
			break;
	}
	gMapMgr->DrawMap();
...
void MenuToggleNamesCB(long ID,short,C_Base *control)
{
	...
	if(((C_PopupList *)control)->GetItemState(ID))
	{
		gMapMgr->TurnOnNames();filterState[LE_LABELS] = true;
	}
	else
	{
		gMapMgr->TurnOffNames();filterState[LE_LABELS] = false;
	}
	gMapMgr->DrawMap();
}
...
void MenuToggleBullseyeCB(long ID,short,C_Base *control)
{
	...
	if(((C_PopupList *)control)->GetItemState(ID))
	{
		gMapMgr->TurnOnBullseye();filterState[LE_BULLSEYE] = true;
	}
	else
	{
		gMapMgr->TurnOffBullseye();filterState[LE_BULLSEYE] = false;
	}
	gMapMgr->DrawMap();
}
void MenuSetCirclesCB(long,short,C_Base *)
{
	...
	if(menu)
	{
		...
		{
			filterState[CIRCLE_SAM_LOW] = true; filterState[CIRCLE_SAM_HIGH] = filterState[CIRCLE_RADAR_LOW] = filterState[CIRCLE_RADAR_HIGH] = false;
			gMapMgr->ShowThreatType(_THR_SAM_LOW);
		}
		else if(menu->GetItemState(MID_CIRCLE_SAM_HIGH))
		{
			filterState[CIRCLE_SAM_HIGH] = true; filterState[CIRCLE_SAM_LOW] = filterState[CIRCLE_RADAR_LOW] = filterState[CIRCLE_RADAR_HIGH] = false;
			gMapMgr->ShowThreatType(_THR_SAM_HIGH);
		}
		else if(menu->GetItemState(MID_CIRCLE_RADAR_LOW))
		{
			filterState[CIRCLE_RADAR_LOW] = true; filterState[CIRCLE_SAM_LOW] = filterState[CIRCLE_SAM_HIGH] = filterState[CIRCLE_RADAR_HIGH] = false;
			gMapMgr->ShowThreatType(_THR_RADAR_LOW);
		}
		else if(menu->GetItemState(MID_CIRCLE_RADAR_HIGH))
		{
			filterState[CIRCLE_RADAR_HIGH] = true; filterState[CIRCLE_SAM_LOW] = filterState[CIRCLE_SAM_HIGH] = filterState[CIRCLE_RADAR_LOW] = false;
	...
	if(gPopupMgr->GetCallingType() == C_TYPE_CONTROL)
	{
		...
		if(urec)
		{
			...
			if(ent)
			{
				...
				if (
					!(TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT) &&
					ent->IsFlight() && 
...
void MenuDeleteWPCB(long,short hittype,C_Base *)
{
	...
	if ((TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL_EDIT) && un->IsFlight())
	{
		fixup_unit (un);
		gGps->Update();
		gMapMgr->DrawMap();
