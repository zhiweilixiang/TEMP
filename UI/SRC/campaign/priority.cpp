...
//				POD->player_priority[i]=POD->air_priority[i];
	...
	for(i=0;i<NUM_TEAMS;i++)
	{
		...
		memcpy(TeamInfo[i]->SetAllMissionPriority(),DefaultMissionPriority[TAT_INTERDICT-1],sizeof(uchar)* (uchar)MissionTypeEnum::AMIS_OTHER);
	}
}
...
void LoadTargetPriorities()
{
	...
	if(win)
	{
		...
		if(sldr)
		{
			...
			value = TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_SWEEP);
...
void LoadMissionPriorities()
{
	...
	if(win)
	{
      ...
      // Cobra - 2/06 JG - Moved AMIS_ESCORT to Recon and changed to Escort.
		{
			sldr->SetCallback(PriSliderCB);
			value = TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_OCASTRIKE) ;
			PositionSlider(value,sldr); // Value is in range of 0 -> 100
		}
		// SAM Suppression
		...
		{
			sldr->SetCallback(PriSliderCB);
			value = TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_SEADSTRIKE);
			PositionSlider(value,sldr); // Value is in range of 0 -> 100
		}
		// Interdiction
		...
		{
			sldr->SetCallback(PriSliderCB);
			value = (	TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_SAD) +
								TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_INT) +
								TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_BAI)) / 3;
			PositionSlider(value,sldr); // Value is in range of 0 -> 100
		}
		// CAS
		...
		{
			sldr->SetCallback(PriSliderCB);
			value = (	TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_ONCALLCAS) +
								TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_PRPLANCAS) +
								TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_CAS)) / 3;
			PositionSlider(value,sldr); // Value is in range of 0 -> 100
		}
		// Strategic Strikes
		...
		{
			sldr->SetCallback(PriSliderCB);
			value = (	TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_INTSTRIKE) +
						TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_STRIKE) +
						TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_DEEPSTRIKE)) / 3;
			PositionSlider(value,sldr); // Value is in range of 0 -> 100
		}
		// Anti shipping
		...
		{
			sldr->SetCallback(PriSliderCB);
			value = (	TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_ASW) +
						TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_ASHIP)) / 2;
			PositionSlider(value,sldr); // Value is in range of 0 -> 100
		}
		// DCA
		...
		{
			sldr->SetCallback(PriSliderCB);
			value = (	TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_BARCAP) +
						TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_BARCAP2) +
						TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_HAVCAP) +
						TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_TARCAP) +
						TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_RESCAP) +
						TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_AMBUSHCAP) +
						TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_INTERCEPT)) / 7;
			PositionSlider(value,sldr); // Value is in range of 0 -> 100
		}
    ...
    // Cobra - 2/06 JG - Moved  AMIS_ESCORT & AMIS_SEADESCORT to Recon and changed to Recon to Escort
		...
		{
			sldr->SetCallback(PriSliderCB);
			value = (	TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_ESCORT) +
					      TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_SEADESCORT)) /2;
			PositionSlider(value,sldr); // Value is in range of 0 -> 100
		}
	}
}
...
void LoadDefaultTargetPriorities()
{
	...
	if(win)
	{
		...
		{
			sldr->SetCallback(PriSliderCB);
			value = TeamInfo[team]->GetMissionPriority(MissionTypeEnum::AMIS_SWEEP); // TJL 10/26/03 Changed from Escort to Sweep
			PositionSlider(value,sldr); // Value is in range of 0 -> 100
		}
		// Air fields
		sldr=(C_Slider*)win->FindControl(TARGET_2);
...
void SaveTargetPriorities()
{
	...
	if(win)
	{
		...
		if(sldr)
		{
			...
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_SWEEP, value); //TJL 10/26/03 Changed form Escort to Sweep
		}
		// Air fields
		sldr=(C_Slider*)win->FindControl(TARGET_2);
		if(sldr)
...
void SaveMissionPriorities()
{
	...
	if(win)
	{
      ...
      // Cobra - 2/06 JG - Moved AMIS_ESCORT to Recon and changed to Escort.
		{
			value=static_cast<uchar>(SliderValue(sldr));
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_OCASTRIKE, value);
		}
		// SAM Suppression
		sldr=(C_Slider*)win->FindControl(MISSION_2);
		if(sldr)
		{
			...
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_SEADSTRIKE, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_SEADESCORT, value);
		}
		// Interdiction
		sldr=(C_Slider*)win->FindControl(MISSION_3);
		if(sldr)
		{
			value=static_cast<uchar>(SliderValue(sldr));
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_SAD, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_INT, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_BAI, value);
		}
		// CAS
		sldr=(C_Slider*)win->FindControl(MISSION_4);
		if(sldr)
		{
			value=static_cast<uchar>(SliderValue(sldr));
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_ONCALLCAS, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_PRPLANCAS, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_CAS, value);
		}
		// Strategic Strikes
		sldr=(C_Slider*)win->FindControl(MISSION_5);
		if(sldr)
		{
			value=static_cast<uchar>(SliderValue(sldr));
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_INTSTRIKE, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_STRIKE, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_DEEPSTRIKE, value);
		}
		// Anti shipping
		sldr=(C_Slider*)win->FindControl(MISSION_6);
		if(sldr)
		{
			value=static_cast<uchar>(SliderValue(sldr));
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_ASW, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_ASHIP, value);
		}
		// DCA
		sldr=(C_Slider*)win->FindControl(MISSION_7);
		if(sldr)
		{
			value=static_cast<uchar>(SliderValue(sldr));
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_BARCAP, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_BARCAP2, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_HAVCAP, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_TARCAP, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_RESCAP, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_AMBUSHCAP, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_INTERCEPT, value);
		}
		// Recon   to become Escort    
    // Cobra - 2/06 JG - Moved  AMIS_ESCORT & AMIS_SEADESCORT to Recon and changed to Recon to Escort
		sldr=(C_Slider*)win->FindControl(MISSION_8);
		...
		{
			value=static_cast<uchar>(SliderValue(sldr));
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_ESCORT, value);
			TeamInfo[team]->SetMissionPriority(MissionTypeEnum::AMIS_SEADESCORT, value);
		}
	}
}
