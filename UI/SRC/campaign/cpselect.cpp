...
void RecieveScenarioInfo()
{
	...
	if (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_LIGHT)
		{
		// Do dogfight/IA stuff
		}
	else if (TheCampaign.Flags & (ulong)CampStatusEnum::CAMP_TACTICAL)
		{
			...
			if(tmpID)
			{
				...
				if (game)
				{
					if(game->GetGameType() == game_Campaign)
						SendMessage(gMainHandler->GetAppWnd(),FM_JOIN_CAMPAIGN, (int)JoinStatusEnum::JOIN_PRELOAD_ONLY,game_Campaign);
				}
			}
		}
		else
...
void DisplayJoinStatusWindow (int bits)
{
	...
	switch (bits)
	{
		case (ulong)CampStatusEnum::CAMP_GAME_FULL:
			id = TXT_GAME_IS_OVER;
			break;
		case 0:
