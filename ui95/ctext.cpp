...
void C_VersionText::Setup(long id, short type){ 
	...
	BYTE *lpVersionData;
	DWORD dwDataSize = GetFileVersionInfoSize("Freefalcon.exe", 0);
	if (dwDataSize == 0){
		DWORD dwDataSize = GetFileVersionInfoSize("Freefalcon.exe", 0);
		//get the version info
		lpVersionData = new BYTE[dwDataSize]; 
		GetFileVersionInfo("Freefalcon.exe", 0, dwDataSize, (void**)lpVersionData);
	}
	else {
		//get the version info
		lpVersionData = new BYTE[dwDataSize]; 
	...
	sprintf(g_sVersion,"Freefalcon : %s",(char *)lpData);
