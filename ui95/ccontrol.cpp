...
C_Control::C_Control(char **stream) : C_Base(stream)
{
	...
	memcpy(&Cursor_,*stream,sizeof(long));					*stream += sizeof(long);
	memcpy(&DragCursor_,*stream,sizeof(long));				*stream += sizeof(long);
	memcpy(&MenuID_,*stream,sizeof(long));					*stream += sizeof(long);
	memcpy(&HelpTextID_,*stream,sizeof(long));				*stream += sizeof(long);
	memcpy(&MouseOverColor_,*stream,sizeof(COLORREF));		*stream += sizeof(COLORREF);
	memcpy(&RelX_,*stream,sizeof(long));					*stream += sizeof(long);
	memcpy(&RelY_,*stream,sizeof(long));					*stream += sizeof(long);
	memcpy(&HotKey_,*stream,sizeof(short));					*stream += sizeof(short);
	memcpy(&MouseOver_,*stream,sizeof(short));				*stream += sizeof(short);
	memcpy(&MouseOverPercent_,*stream,sizeof(short));		*stream += sizeof(short);
...
void C_Control::Save(char **stream)
{
	...
	memcpy(*stream,&Cursor_,sizeof(long));					*stream += sizeof(long);
	memcpy(*stream,&DragCursor_,sizeof(long));				*stream += sizeof(long);
	memcpy(*stream,&MenuID_,sizeof(long));					*stream += sizeof(long);
	memcpy(*stream,&HelpTextID_,sizeof(long));				*stream += sizeof(long);
	memcpy(*stream,&MouseOverColor_,sizeof(COLORREF));		*stream += sizeof(COLORREF);
	memcpy(*stream,&RelX_,sizeof(long));					*stream += sizeof(long);
	memcpy(*stream,&RelY_,sizeof(long));					*stream += sizeof(long);
	memcpy(*stream,&HotKey_,sizeof(short));					*stream += sizeof(short);
	memcpy(*stream,&MouseOver_,sizeof(short));				*stream += sizeof(short);
	memcpy(*stream,&MouseOverPercent_,sizeof(short));		*stream += sizeof(short);
	...
	if(Sound_)
	{
		...
		if(count)
		{
			...
			{
				memcpy(*stream,&curidx,sizeof(long));		*stream += sizeof(long);
				memcpy(*stream,&rec,sizeof(long));			*stream += sizeof(long);
				rec=(long)Sound_->GetNext(&cur,&curidx);
			}
		}
	}
