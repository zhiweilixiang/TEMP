...
struct vector3
{
	...
	{
	    if (fabs((double)v.x - x) > tol)      return false;
	    else if (fabs((double)v.y - y) > tol) return false;
	    else if (fabs((double)v.z - z) > tol) return false;
	    return true;
	}

	int Compare(const vector3& v, float tol) const
	{
	    if (fabs((double)v.x - x) > tol)      return (v.x > x) ? +1 : -1; 
	    else if (fabs((double)v.y - y) > tol) return (v.y > y) ? +1 : -1;
	    else if (fabs((double)v.z - z) > tol) return (v.z > z) ? +1 : -1;
	    else                          return 0;
	}
