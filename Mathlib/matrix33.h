...
struct matrix33 
{
	...
	quaternion GetQuaternion() const
	{
	    ...
	    else 
	    {
	        ...
	        j = nxt[i];
	        k = nxt[j];
	        float s = (float)(sqrt(((double)(m[i][i]) - ((double)(m[j][j]) + (double)(m[k][k]))) + 1.0));
	        qa[i] = s * 0.5f;
	        s = 0.5f / s;
	        qa[3] = (m[j][k] - m[k][j])* s;
	        qa[j] = (m[i][j] + m[j][i]) * s;
	...
	{    
	    ...
	    if (s==EulRepYes) 
	    {
	        double sy = sqrt(pow(tmp.m12,2) + pow(tmp.m13,2));
	        if (sy > 16*(double)FLT_EPSILON) 
	        {
	    ...
	    {
	        double cy = sqrt(pow(tmp.m11,2) + pow(tmp.m21,2));
	        if (cy > 16*(double)FLT_EPSILON) 
	        {
	...
	bool Orthonorm(float limit) 
	{
	    if (((m11*m21+m12*m22+m13*m23)<limit) &&
	        ...
	        ((pow(m11,2)+pow(m12,2)+pow(m13,2))>(1.0-limit)) &&
	        ((pow(m11,2)+pow(m12,2)+pow(m13,2))<(1.0+limit)) &&
	        ((pow(m21,2)+pow(m22,2)+pow(m23,2))>(1.0-limit)) &&
	        ((pow(m21,2)+pow(m22,2)+pow(m23,2))<(1.0+limit)) &&
	        ((pow(m31,2)+pow(m32,2)+pow(m33,2))>(1.0-limit)) &&
	        ((pow(m31,2)+pow(m32,2)+pow(m33,2))<(1.0+limit)))
	        return true;
	    else
	        return false;
