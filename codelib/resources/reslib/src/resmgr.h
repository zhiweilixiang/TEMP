...
#   define RES_FSEEK   ResFSeek
    ...
    /* msvc has a bug in the linker that makes it impossible to just
	   ...
	   //NHV #define fopen       ResFOpen
	   //NHV #define fclose      ResFClose
	   //NHV #define ftell       ResFTell
	   //NHV #define fread       ResFRead
	   //NHV #define fseek       ResFSeek
#endif

//NHV    FILE * __cdecl     RES_FOPEN( const char * name, const char * mode );
//NHV    int    __cdecl     RES_FCLOSE( FILE * file );
//NHV    long   __cdecl     RES_FTELL( FILE * stream );  /* default is to use ftell !!! */
//NHV    size_t __cdecl     RES_FREAD( void *buffer, size_t size, size_t num, FILE *stream );
//NHV    int    __cdecl     RES_FSEEK( FILE * stream, long offset, int whence );
