...
#include "codelib\tools\lists\lists.h" //RERUN
...
enum State { Receive=0, Transmit, NotReady };
  ...
  if (fp == NULL) 
  	...
  	while(fgets(buffer,512,fp))
	{
		if(buffer[0]!='#')
		{
			...
			SFX_DEF[i].coneInsideAngle  = (float)cos( TokenF(0)   / 180 * PI );
			SFX_DEF[i].coneOutsideAngle = (float)cos( TokenF(180) / 180 * PI );
...
F4SoundFXSetDist( int sfxId, int override, float volume, float pscale)
{
	...
	if (SFX_DEF == NULL) return; //RERUN CTD
...
void F4SoundPos::PositionalData(void)
{
	...
	{   // platform is really a SimBaseClass object
		...
		if(velocity > 1100 && distance)
		{
			...
			float camang = (float)(180 * DTR - acos(velVec.x * rvx + velVec.y * rvy + velVec.z * rvz));
...
void F4SoundPos::Sfx(int SfxID, int SID, float PScale, float Vol, float X, float Y, float Z)
{
	...
	// make new node
    ShiAssert (SfxID < NumSFX );
	ShiAssert (SfxID >= 0 ); //RERUN 0 is just off
