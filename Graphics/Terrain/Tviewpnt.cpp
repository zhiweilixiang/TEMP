...
void TViewPoint::Update(const Tpoint *position )
{
	...
	if (vuxGameTime != prevvuxGameTime){
		float dist = 
			(float)sqrt(pow((pos.x - position->x), 2) + pow((pos.y - position->y), 2));
		dist = (float)sqrt(pow(dist, 2) + pow((pos.z - position->z), 2));
...
int TViewPoint::GetGroundType( float x, float y )
{
	...
	ShiAssert( (xPos >= -0.5f) && (xPos <= LEVEL_POST_TO_WORLD( 1, LOD )+0.5f) ); //RERUN observed xpos = -4 ! (after crash into ground)
	ShiAssert( (yPos >= -0.5f) && (yPos <= LEVEL_POST_TO_WORLD( 1, LOD )+0.5f) ); //RERUN observed yPos = -16 ! y was large negative. overflow issue?
