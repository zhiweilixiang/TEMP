...
char *Alloc(int size)
{
	...
	if ((unsigned int)(blk->free)>(unsigned int)(blk->end))
	{
		...
		else
		{
			...
			memset(blk->next, 0, sizeof(alloc_block_t));
			blk = blk->next;
			//RERUN blk->next = 0UL;
			blk->start = (char *)malloc(ALLOC_BLOCK_SIZE+ALIGN_BYTES-1);
			memset(blk->start, 0, sizeof(ALLOC_BLOCK_SIZE+ALIGN_BYTES-1));
