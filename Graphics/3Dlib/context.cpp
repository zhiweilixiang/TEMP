...
extern DisplayOptionsClass DisplayOptions; //RERUN
...
#if 0 //RERUN removed, depends on struct packing and other nastiness
void ContextMPR::DrawPrimitive(int nPrimType, WORD VtxInfo, WORD nVerts, MPRVtxTexClr_t **pData, bool terrain)
{
	...
	if(!bZBuffering)
	{
		...
		for(int i = 0; i < nVerts; i++)
		{
			...
			// if(!pData[i]) break; //RERUN not helping anyway
			
			// RERUN crash here with i =1 while pData only has pData[0]
...
#endif //RERUN
//RERUN added new version for mem corruption test
void ContextMPR::DrawPrimitive(int nPrimType, WORD VtxInfo, std::vector<MPRVtxTexClr_t>&pData, bool terrain)
{
	TLVERTEX* pVtx;
	TLVERTEX* sVertex;
	SPolygon* sPolygon;
	float	PolyZAvg = 0;
	WORD nVerts = (WORD)pData.size(); //RERUN
