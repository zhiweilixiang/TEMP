...
class DrawableBSP : public DrawableObject {
  ...
  protected:
	...
	DrawableBSP( float s, int ID ): DrawableObject( s ), instance(ID), label(), 
		                            labelColor(), orientation() { inhibitDraw = FALSE; labelLen = 0; id = ID; radius = instance.Radius(); RadarSign = instance.RadarSign(); };
