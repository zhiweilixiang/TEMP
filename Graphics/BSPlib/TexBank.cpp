...
short* TextureBankClass::CacheLoad;
short* TextureBankClass::CacheRelease;
...
void TextureBankClass::Setup(int nEntries)
{
    ...
    if (nEntries)
    {
        ...
        memset(CacheLoad, 0, sizeof(short) * (nTextures + CACHE_MARGIN));
        CacheRelease = (short*) malloc(sizeof(short) * (nTextures + CACHE_MARGIN));
        memset(CacheRelease, 0, sizeof(short) * (nTextures + CACHE_MARGIN));
    }
    else
    {
        TexturePool = NULL;
...
void TextureBankClass::Reference(DWORD id)
{
    int  isLoaded;
    ...
    else
    {
        ...
        if(TexturePool[id].tex.imageData != NULL)
            return;
        //RERUN ShiAssert(TexturePool[id].tex.TexHandle() == NULL);
        if (TexturePool[id].tex.TexHandle() != NULL)
            TexturePool[id].tex.FreeTexture(); //RERUN testing
        ...
        if (!TexFlags[id].OnOrder)
        {
            ...
            // put into load cache
            CacheLoad[LoadIn++] = (short)id; //RERUN why do we push a DWORD in a short??
...
void TextureBankClass::ReadImageDDS(DWORD id)
{
	...
	TexturePool[id].tex.flags = MPR_TI_DDS; //RERUN |= MPR_TI_DDS;
    TexturePool[id].tex.flags &= ~MPR_TI_PALETTE;
