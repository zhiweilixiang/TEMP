...
void ObjectLOD::SetupEmptyTable( int numEntries )
{
	...
	memset(LodBuffer, 0, sizeof(DEFAULT_BUFFER_SIZE));
	...
	memset(CacheLoad, 0, sizeof(short) * (numEntries + CACHE_MARGIN));
	CacheRelease = (short*) malloc(sizeof(short)*(numEntries + CACHE_MARGIN));
	memset(CacheRelease, 0, sizeof(short) * (numEntries + CACHE_MARGIN));
...
void ObjectLOD::SetupTable( int file, char *basename )
{
		...
		if(ObjectLodMap.ReadDataAt(Lod->fileoffset, &rt, sizeof(DxDbHeader))){
			...
			if((rt.Version & 0xffff) == (~rt.Version >>16)){
				...
				if(Lod->NrTextures){
					...
					if (Lod->TexBank)
					{
						memset(Lod->TexBank, 0, sizeof(DWORD) * Lod->NrTextures);
	...
	memset(CacheLoad, 0, sizeof(short) * (TheObjectLODsCount + CACHE_MARGIN));
	CacheRelease = (short*) malloc(sizeof(short)*(TheObjectLODsCount + CACHE_MARGIN));
	memset(CacheRelease, 0, sizeof(short) * (TheObjectLODsCount + CACHE_MARGIN));
	LoadIn = LoadOut = ReleaseIn = ReleaseOut = 0;
	LODsLoaded = 0;
}
...
////////////// COBRA - DX - The DX Load Procedure //////////////////////////////////////////
	...
	{
		if (LodBufferSize > 0) //RERUN freeing empty buffer caused CTD 
		{
			free(LodBuffer);
		}
		LodBufferSize = filesize;
		LodBuffer = (BYTE*)malloc(LodBufferSize);
		memset(LodBuffer, 0, LodBufferSize);
	}
