...
void ObjectParent::ReadParentList(int file)
{
    ...
    for (objParent = TheObjectList; objParent < end; objParent++)
    {
        ...
        {
            if (objParent->pLODs) //RERUN no LODS, no pointer to LODs
            {
                delete objParent->pLODs;
                objParent->pLODs = NULL;
            }
            continue;
        }
        ...
        for (i = 0; i < objParent->nLODs; i++)
        {
            ...
            if ((result < 0) || (F4IsBadReadPtr(&objParent->pLODs[i], sizeof(*objParent->pLODs)))) //RERUN bad mem check added
            {
                char message[256];
...
{
    // RERUN - Possible CTD Fix, if no LODs, exit
     if ((!nLODs)||(!pLODs)) 
         return;

    LODrecord *record = pLODs + nLODs - 1;

    if (F4IsBadReadPtr(pLODs, sizeof(LODrecord))) //RERUN deep checks
    {
        pLODs = NULL;
        nLODs = 0;
        return;
