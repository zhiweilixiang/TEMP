...
void RealWeather::RefreshWeather(RenderOTW *Renderer)
{
	...
	if(InsideOvercast()){
		...
		float DeltaCx = (float)(fabs(-viewerZ + stratusZ) / StratusHalf);
...
{
#if 1 // FRB - Disable rain for now.  It looks really bad.
	unsigned int i;
	Tpoint p0,p1,p2,p3;
	...
	if(otwPlatform)
	{
		...
		//RERUN vel = min(((SimMoverClass *)otwPlatform)->GetKias(),30.f);
		vel = min(((SimMoverClass*)otwPlatform)->GetKias(), 25.f);
		pitch = vel*(((SimMoverClass *)otwPlatform)->Pitch()-90.f*DTR)/30.f;
		if (((SimMoverClass*)otwPlatform)->GetKias() > 10)
			amount = (uint)(200 / (((SimMoverClass*)otwPlatform)->GetKias()/5));
		else
			amount = 200;
	}
	else
	...
	for(i = 0; i < amount; i++)
	{
		...
		&&   rainZ > -40 && rainZ < 40))
		{
			...
			{
				v0.r = v1.r = (1.f * .05f) + (litCloudColor.r * .8f);
				v2.r = v3.r = (.4f * .05f) + (litCloudColor.r * .8f);

				v0.g = v1.g = (1.f * .05f) + (litCloudColor.g * .8f);
				v2.g = v3.g = (.4f * .05f) + (litCloudColor.g * .8f);

				v0.b = v1.b = (1.f * .05f) + (litCloudColor.b * .8f);
				v2.b = v3.b = (.4f * .05f) + (litCloudColor.b * .8f);
			}

			v0.a = v1.a = v2.a = v3.a = 1.f;

			renderer->context.RestoreState(STATE_ALPHA_TEXTURE_NOFILTER_PERSPECTIVE); // STATE_ALPHA_TEXTURE_GOURAUD_PERSPECTIVE);
...
void RealWeather::UpdateWeatherQuality(void)
{
	...
	if(fabs(WeatherQualityRate) < (MIN_WEATHER_Q_STEP / 10.0f)) 
		realWeather->WeatherQualityRate = (float)(fabs(WeatherQualityRate)/WeatherQualityRate * MIN_WEATHER_Q_STEP);
}
