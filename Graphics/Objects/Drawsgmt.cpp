...
#define	NRANDPOS (double)( (double)rand()/(double)RAND_MAX )
#define NRAND	 ( 1.0 - 2.0 * NRANDPOS )
...
void DrawableTrail::AddPointAtHead( Tpoint *worldPos, DWORD )
{
	...
	if (!this) 
		return; //RERUN avoid this being nullpointer
	...
	if (Something > 500 && cn && !(Type->flags & TTF_LINE)){
		...
		ChunkNode* cn2 = new ChunkNode(Type);
		if(cn2){
	...
	if(n && n->connected)
	{
		...
		if(now<n->NowTime)
			return;
		float spacing=(float)(Type->spacing + NRANDPOS * Type->spcVariation);
			...
			else
			{
				...
				if(n)
				{
					...
					else
					{
						...
						dist=(float)sqrt(pow(xx, 2) + pow(yy, 2) + pow(zz, 2));
...
void TrailNode::Init(Tpoint *pos, TrailTypeEntry *trailType)
{
	...
	Radius=2;
	connected=1;
	radiusStart    = (float)(Type->radiusStart + NRANDPOS * Type->radiusVariationStart);
	posVariationEnd.x = (float)(NRANDPOS * Type->posVariationEnd);
	posVariationEnd.y = (float)(NRANDPOS * Type->posVariationEnd);
	posVariationEnd.z = (float)(NRANDPOS * Type->posVariationEnd);
	AlphaMult=(float)NRANDPOS;
}
