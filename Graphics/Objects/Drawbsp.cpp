...
//RERUN	ShiAssert( slotNumber < instance.ParentObject->nSlots );
//RERUN	ShiAssert( (instance.SlotChildren) && (instance.SlotChildren[slotNumber] == NULL) );
...
{
	// THIS IS A HACK TO TOLERATE OBJECTS WHICH DON'T YET HAVE DYNAMIC VERTS
	// THIS SHOULD BE REMOVED IN THE LATE BETA AND SHIPPING VERSIONS
...
// Besides no need to calculate radial distance is Z distance is already greater
		if (g_bLabelRadialFix)
				...
				labelPoint.csZ < limitcheck)			//Same condition as below!!!
			{
				...
				labelPoint.csZ = (float)sqrt(pow(dx,2) + pow(dy,2) + pow(dz,2));
			}
