...
{
public:
    ParticleParamNode() { id = 0, subid = 0, trailId = 0, sndId = 0; bspObj = nullptr; bspCTID = bspVisType = 0; }; //RERUN adding initializers to find crazy values

    char    name[PS_NAMESIZE];
    int     id;
    int     subid;

    int     particleType; // which particle class to use for particles????
...
{
    //RERUN Start
    Quad[0].tu = 0.0f, Quad[0].tv = 0.0f, Quad[0].dwColour = 0, Quad[0].dwSpecular = 0, Quad[0].pos.x = 0.0f, Quad[0].pos.y = 0.0f, Quad[0].pos.z = 0.0f;
    Quad[1].tu = 0.0f, Quad[1].tv = 0.0f, Quad[1].dwColour = 0, Quad[1].dwSpecular = 0, Quad[1].pos.x = 0.0f, Quad[1].pos.y = 0.0f, Quad[1].pos.z = 0.0f;
    Quad[2].tu = 0.0f, Quad[2].tv = 0.0f, Quad[2].dwColour = 0, Quad[2].dwSpecular = 0, Quad[2].pos.x = 0.0f, Quad[2].pos.y = 0.0f, Quad[2].pos.z = 0.0f;
    Quad[3].tu = 0.0f, Quad[3].tv = 0.0f, Quad[3].dwColour = 0, Quad[3].dwSpecular = 0, Quad[3].pos.x = 0.0f, Quad[3].pos.y = 0.0f, Quad[3].pos.z = 0.0f;
    v0.csX = 0.f, v1.csX = 0.f, v2.csX = 0.f, v3.csX = 0.f;
    v0.csY = 0.f, v1.csY = 0.f, v2.csY = 0.f, v3.csY = 0.f;
    v0.csZ = 0.f, v1.csZ = 0.f, v2.csZ = 0.f, v3.csZ = 0.f;
    TexHandle = 0;
    TexItem = NULL;
    //RERUN End
    ...
    if (ppn->GroupFlags & (int)GroupType::GRP_TEXTURE)
    {
...
{
    rotation = { 0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f,0.f };
}
...
{
    //RERUN CTD checks
    if ((!renderer) || (!owner) || (!owner->ppn) || (!owner->ppn->bspObj))
        return false;
...
bool SubEmitter::Run(RenderOTW *renderer, ParticleNode *owner)
{
    ...
    switch (pep->mode)
    {
    case PSEmitterModeEnum::PSEM_IMPACT:
        {
    ...
    case PSEmitterModeEnum::PSEM_EARTHIMPACT:
        {
            float GroundLevel = DrawableParticleSys::groundLevel;
        ...
        case PSEmitterModeEnum::PSEM_WATERIMPACT:
        {
            float GroundLevel = DrawableParticleSys::groundLevel;
        ...
        case PSEmitterModeEnum::PSEM_ONCE:
        {
        ...
        case PSEmitterModeEnum::PSEM_PERSEC:
...
void ParticleNode::Init(int ID, Tpoint *Pos, Tpoint *Vel, Tpoint *Aim, float fRotationRate)
{
    ...
    if (ppn->drawType == PSDrawType::PSDT_POLY)
    {
    ...
    {
    case PSOrientation::PSO_MOVEMENT:
        {
...
void ParticleNode::Draw(class RenderOTW *renderer, int LOD)
{
    ...
    while (Sub)
    {
        ...
        // Run the Sub part
        if ((firstSubPart)&&(!Sub->Run(renderer, this))) // RERUN added firstSubPart check
        {
            // if it's dead remove it from list
            if (Last) Last->next = Next;
...
void DrawableParticleSys::AddParticle(int Id, Tpoint *worldPos, Tpoint *v)
{
    ...
    // COBRA - RED - The Particle Filter starts here
...
// COBRA - RED - Adds a Frame References List that is stored in the Node of a Particle Item
ParticleTextureNode *DrawableParticleSys::GetFramesList(char *fn, int Frames)
{
    ...
    memset(BaseList, 0, sizeof(TextureLink) * Frames);
    List = BaseList;
...
{
    ParticleAnimationNode *n = (ParticleAnimationNode *)AnimationsList.GetHead();
...
void DrawableParticleSys::SetupTexturesOnDevice(DXContext *rc)
{
    ...
    while (ppn)
    {
        if (ppn->bspCTID)
        {
            ppn->bspObj   = new DrawableBSP(Falcon4ClassTable[ppn->bspCTID].visType[ppn->bspVisType], &pos, &rot);
        }
        else //RERUN added else
        {
            ppn->bspObj = nullptr;
        }
...
void DrawableParticleSys::ReleaseTexturesOnDevice(DXContext *rc)
{
    ...
    ParticleTextureNode *n = (ParticleTextureNode *)textureList.GetHead();
    ...
    ParticleParamNode *ppn = (ParticleParamNode *)paramList.GetHead();
...
inline float DrawableParticleSys::PS_EvalTimedLinLogFloat(float life, int &LastStage, int Count, timedFloat *input)
{
    ...
    // if past the end of stages, return last stage value
    if (LastStage >= Count - 1) 
        return input[Count - 1].value;

    // if Life past the Stage Time update to the next Stage
    if (life > input[LastStage + 1].time) 
        LastStage++;
    ...
    // limit check
    if (Idx >= LOG10_ARRAY_ITEMS) 
        Idx = LOG10_ARRAY_ITEMS - 1;
...
void DrawableParticleSys::PS_ListsInit(void)
{
    ...
    if (!PS_PPN)
    {
        ...
        memset(PS_PPN, 0, sizeof(PS_PPType) * MAX_PARTICLE_PARAMETERS);
        PS_PPNNr = 0;
    }
    ...
    if (!PS_TPN)
    {
        ...
        memset(PS_TPN, 0, sizeof(PS_TPType) * MAX_TRAIL_PARAMETERS);
        PS_TPNNr = 0;
    }
    ...
    if (!PS_PEP)
    {
        ...
        memset(PS_PEP, 0, sizeof(PS_PEPType) * MAX_EMITTERS_PARAMETERS);
        PS_PEPNr = 0;
    }
...
PS_PTR DrawableParticleSys::PS_AddItem(DWORD ListIdx)
{
    ...
    if (List.ObjectIn >= List.ListSize) 
        List.ObjectIn = 0;
...
PS_PTR DrawableParticleSys::PS_RemoveItem(DWORD ListIdx, PS_PTR Item, PS_PTR Prev)
{
    ...
    if (List.ObjectOut >= List.ListSize) 
        List.ObjectOut = 0;
...
// Function parsing and updating all Polys... responsable to kill dead ones too
...
void  DrawableParticleSys::PS_PolyRun(void)
{
    ...
    while (ptr != PS_NOPTR)
    {
        ...
        else
        {
            ...
            if (Distance > ppn.visibleDistance) 
                goto Skip;

            // Update light in parameter if an emissive poly
            if (ppn.EmitLight && Distance < LightRadius) 
                LightIn = true;

            // Perform visibility check
            if (!TheDXEngine.DX2D_GetVisibility((D3DXVECTOR3*) &Part.pos, (ppn.EmitLight) ? size : LightRadius)) 
                Visible = false;
...
// Function parsing and updating all Emitters... responsable to kill dead ones too
        ...
        switch (Emitter.PEP->mode)
        {
            ...
            case PSEmitterModeEnum::PSEM_IMPACT:
                    if (epos.z >= Part.GroundLevel) 
                        epos.z = Part.GroundLevel, qty += Emitter.PEP->rate[0].value, Alive = false;
                break;
            ...
            case PSEmitterModeEnum::PSEM_EARTHIMPACT:
                    if (epos.z >= Part.GroundLevel)
                    {
                        ...
                        if (!(gtype == COVERAGE_WATER || gtype == COVERAGE_RIVER)) 
                            qty += Emitter.PEP->rate[0].value, Alive = false;
                    }
                break;
            ...
            case PSEmitterModeEnum::PSEM_ONCE:
                    ...
                    // if gone past last stage, die..!!!
                    if (Emitter.LastStage >= Emitter.PEP->stages) 
                        Alive = false;
                break;
...
void DrawableParticleSys::PS_GenerateEmitters(PS_PTR owner, PS_PPType &PPN)
{
    ...
    if (Light == PS_NOPTR && PPN.LightRoot) 
        Light = PS_AddItem(PS_LIGHTS_IDX);
    ...
    else
    {
        ...
        {
            // Ok... check the probability to enable this emitter...
            if (Test >= PPN.ParticleEmitterMin[count] && Test < PPN.ParticleEmitterMax[count]) 
                PS_AddEmitter(owner, &PPN.emitter[count], Light);
        }
    }
}
...
{
    PS_AddParticle((int)PPN[ID]->id, Pos, Vel); //Pu239 Passing a memory address as an ID? Added ->id
}
...
{
	if ((ID < 0) || (ID >= MAX_PARTICLE_PARAMETERS)) //RERUN CTD check
		return;
    ...
    // * LINKED TRAIL *
    if (ppn.trailId >= 0) 
        PS_AddTrail(ppn.trailId, Pos, ptr);

    // * EMITTERS GENERATION *
    if (ppn.emitter[0].stages) 
        PS_GenerateEmitters(ptr, ppn);
...
void DrawableParticleSys::PS_ClustersRun(void)
{
    ...
    while (ptr != PS_NOPTR)
    {
        ...
        if (!Cluster.Static)
        {
            ...
            if (Visibility & D3DSTATUS_DEFAULT) 
                Cluster.Out = true;
        }
    }
}
...
void DrawableParticleSys::PS_TrailRun(void)
{
    ...
    while (ptr != PS_NOPTR)
    {
        ...
        // If part is dead, remove it, all last stages have been already killed/exucuted
        if (Trail.OWNER != PS_NOPTR && !(((ParticleNodeType*)PS_Lists[PS_PARTICLES_IDX].ObjectList)[Trail.OWNER]).Alive) 
            Trail.Alive = false;
        ...
        if (Trail.StartUp) 
            Trail.Origin = *(D3DXVECTOR3*)&Trail.Pos;
        ...
        if (Trail.Alive && Trail.Updated && Trail.Elapsed >= Trail.Interval)
        {
            ...
            if (Trail.OffsetX >= TPN.RndLimit) 
                Trail.OffsetX = TPN.RndLimit - XRand;

            if (Trail.OffsetY >= TPN.RndLimit) 
                Trail.OffsetY = TPN.RndLimit - XRand;
            ...
            if (Trail.Last >= Trail.Nodes) 
                Trail.Last = 0;

            if (Trail.Entry == Trail.Last)
                Trail.Entry++;

            if (Trail.Entry >= Trail.Nodes) 
                Trail.Entry = 0;
...
void DrawableParticleSys::PS_AddSubTrail(TrailSubPartType &Part, int ID, float AlphaCx, float SizeCx, Tpoint *Pos, Tpoint *Wind, Tpoint *Offset, PS_PTR CLUSTER)
{
    ...
    // check if depending on group texture
    if (TPN.GroupFlags & (int)GroupType::GRP_TEXTURE2)
        // if depending from a group, ask for a random texture node
...
void DrawableParticleSys::PS_SubTrailRun(TrailSubPartType *Trail, D3DXVECTOR3 &Origin, DWORD &Entry, DWORD &Exit, DWORD Elements, PS_PTR tpn)
{
    ...
    //RERUN added default initializer
    Color.r = TPN.Color[0].r;
    Color.g = TPN.Color[0].g;
    Color.b = TPN.Color[0].b;
    ...
    while (!TrailCompleted /*&& PS_SubTrails<14200*/)
    {
        ...
        float   Size = 0.0f; //RERUN moved here because "goto skip" can be called before Size is set or even declared. That's why goto is considered evil...
        ...
        if (TrailValid)
        {
            ...
            if (S0_Distance > IntegrateDistance) 
                Integrate = F_I32((S0_Distance - IntegrateDistance) / IntegrateDistance);
            ...
            {//RERUN start tryout as they else remain uninitialized
                // The go to next segment
                EntrySegment = false;
                RecalcP1 = true;
                goto Skip; 
            }//RERUN end tryout as they else remain uninitialized
            ...
            while (SegmentDone < SegmentSize)
            {
                ...
                SegmentStep = (ST_Distance >= (FragRadius * 1.02f)) ? SegmentSize : SegmentStep = Size / 2.0f + 0.5f;
                ...
                if (ST_Distance >= FragRadius) 
                    SegAlpha = min(255.0f, ColorNew.a * (1.0f + FragRadius / ST_Distance) * (1.0f + XAngle));
                else 
                    if (ST_Distance >= FragMixOut) 
                        SegAlpha = min(255.0f, ColorNew.a * 2 * AlphaMixCx * (1.0f + XAngle));
                    else 
                        SegAlpha = 255.0f; //RERUN added default, because SegAlpha might be used uninitialized
                ...
                if (ST_Distance <= FragRadius)
                {
                    ...
                    if (ST_Distance <= FragMixOut) Alpha = ColorNew.a; 
                    else Alpha = ColorNew.a * (1.0f  - AlphaMixCx);
...
#define On(s) if(stricmp(com,s)==0)
        ...
        On("id")
        {
            ...
            if (n)
            {
                ...
                ppn->velInherit = 1;
                ppn->drawType = PSDrawType::PSDT_NONE;
                currentemitter = -1;
                ppn->sndVol[0].value = 0;
                ppn->sndPitch[0].value = 1;
                ppn->trailId = -1;
                ppn->visibleDistance = 10000;
                ppn->GroupFlags = (int)GroupType::GRP_NONE;
                ppn->TrailName[0] = 0;
        ...
        On("trail")
        {
            ...
            if (n)
            {
                ...
                tpn->Id = -1;
                tpn->Color[0].a = 1.0f;
                tpn->GroupFlags = (int)GroupType::GRP_NONE;
            }
        ...
        if (ppn)  // * PARTICLE PARAMETER NODE *
        {
            ...
            On("animate")
            {
                ...
                ppn->drawType = PSDrawType::PSDT_POLY;
                continue;
            }
            ...
            On("texture")
            {
                ...
                if (gpn)
                {
                    ...
                    ppn->GroupFlags |= (int)GroupType::GRP_TEXTURE;
                    ppn->drawType = PSDrawType::PSDT_POLY;
                    // end here
                    continue;
                }
            ...
            if (currentemitter < 10)
            {
                ...
                if (currentemitter >= 0)
                {
                    ...
                    On("emissionmode")
                    {
                        ...
                        ppn->emitter[currentemitter].mode = static_cast<PSEmitterModeEnum>(TokenEnum(emitenums, (int)PSEmitterModeEnum::PSEM_NONE));

                        if (ppn->emitter[currentemitter].mode == PSEmitterModeEnum::PSEM_NONE)
                        {
                            char *emitenums[] = {"ONCE", "PERSEC", "IMPACT", "EARTHIMPACT", "WATERIMPACT", 0};
                            ppn->emitter[currentemitter].mode = static_cast<PSEmitterModeEnum>(TokenEnum(emitenums, (int)PSEmitterModeEnum::PSEM_ONCE));
                        }
            ...
            On("drawtype")
            {
                ...
                if (!strcmp(n, "zplane"))
                {
                    ppn->drawType = PSDrawType::PSDT_POLY, ppn->ZPoly = true;
                }
        ...
        if (GroupNode)
        {
            ...
            {
                // Check for texture
                if (GroupNode->Type == GroupType::GRP_NONE || GroupNode->Type == GroupType::GRP_TEXTURE)
                {
                    char *n = TokenStr(0);
                    ...
                    {
                        GroupNode->Type = GroupType::GRP_TEXTURE;
                        GroupNode->Items++;
                    }
                }
        ...
        if (tpn)  // * TRAIL PARAMETER NODE *
        {
            ...
            On("texture")
            {
                ...
                if (gpn)
                {
                    ...
                    // and signal the texture depends on a group
                    tpn->GroupFlags |= (int)GroupType::GRP_TEXTURE;
                    continue;
                }
                ...
                if (gpn)
                {
                    ...
                    // and signal the texture depends on a group
                    tpn->GroupFlags |= (int)GroupType::GRP_TEXTURE2;
                    continue;
                }
    ...
    // Pu239 Broken code, rewritten (best guess)
    for(int i=0;i<l;i++)
    {
    ...
    for (DWORD a = 0; a < LastTPN; a++)
    {
        ...
        PS_TPN[a].LineDistance = PS_TPN[a].Size[1] * PS_TPN[a].Size[1] * 50.0f ;
