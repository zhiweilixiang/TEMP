	...
	while(Index<MAX_DYNAMIC_LIGHTS){
		...
		if(LightList[Index].CameraDistance==MaxRange && (!Assigned)){
			...
			if(Light->Light.dltType==D3DLIGHT_SPOT){
				...
				LightList[Index].alphaX=(float)atan2(LightList[Index].Light.dvDirection.x, LightList[Index].Light.dvDirection.y);
				LightList[Index].alphaY=(float)atan2(Light->Light.dvDirection.z, sqrt(pow(LightList[Index].Light.dvDirection.y, 2) + pow(LightList[Index].Light.dvDirection.x, 2)));
			}
...
//		while(idx<MAX_DYNAMIC_LIGHTS && LightList[idx].CameraDistance<=DYNAMIC_LIGHT_INSIDE_RANGE){
	...
	while(idx<DynamicLights){
		...
		if(LightList[idx].Light.dltType==D3DLIGHT_SPOT){
			...
			float	ax=(float)atan2(Acc.d3d.x, Acc.d3d.y);
			float	ay=(float)atan2(Acc.d3d.z,sqrt((double)Square.d3d.x+ (double)Square.d3d.y));
			// transform the angles in same Sign Domain of the light angles X/Y
			if(fabs((double)ax-LightList[idx].alphaX)>PI) ax+=ax>LightList[idx].alphaX ? -2*PI : 2*PI;
			if(fabs((double)ay-LightList[idx].alphaY)>PI) ay+=ax>LightList[idx].alphaY ? -2*PI : 2*PI;
