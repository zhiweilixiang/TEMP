...
#define	INIT_S_STACK(name, max)	name.StackLevel=0; name.StackMax=max; name.Stack=(SurfaceItemType*)malloc(sizeof(SurfaceItemType)*max);memset(name.Stack, 0, sizeof(SurfaceItemType) * max);
...
public:
	...
	void SetBlipIntensity(float Intensity) {  m_BlipIntensity = Intensity; };
	void LinearFog(bool Status) { m_LinearFog = Status; }
	bool LinearFog(void) { return m_LinearFog; }
	...
	void			SetState(DX_StateType	Mode)	{ m_RenderState = Mode; }
	void			ResetState(void)				{ m_RenderState = DX_OTW; m_StatesStackLevel = 0;}
	DX_StateType	GetState(void)					{ return m_RenderState; }
