...
{
	CTextureSurface	*Ts=TexturesList, *Tsl = NULL;
	CTextureItem	*Ti = NULL, *Tl = NULL;
	
	// Release all Textures
	while(Ts != NULL){
		Ti=Ts->ItemList;
		// Release all items of the texture surface
		while(Tl=Ti){ // while Ti != NULL, assign Tl to be Ti
			Ti=Ti->Next;
			delete Tl;
			Tl = NULL; // Just to be sure, for exit reasons
		}
		Ts->Tex.FreeAll();
		Tsl=Ts;
		Ts=Ts->Next;
