...
//RERUN		if(pddsd->ddpfPixelFormat.dwRGBBitCount >= 16 && (pddsd->dwWidth == 640 || pddsd->dwWidth == 800 || pddsd->dwWidth == 1024 ||
//RERUN			(pddsd->dwWidth == 1280 && pddsd->dwHeight == 960) || pddsd->dwWidth == 1600 || HighResolutionHackFlag))
		{
			if(modeNum == 0)
			{
...
void DeviceManager::EnumDDDrivers(DeviceManager *pThis)
{
    ...
    if (lpDDEnumEx) 
		lpDDEnumEx(EnumDDCallbackEx, pThis, DDENUM_ATTACHEDSECONDARYDEVICES | DDENUM_NONDISPLAYDEVICES);
    else 
		DirectDrawEnumerate(EnumDDCallback, pThis);
