...
void Render3D::DrawSquare(ThreeDVertex* v0, ThreeDVertex* v1, ThreeDVertex* v2, ThreeDVertex* v3, int CullFlag, bool gifPicture, bool terrain) //JAM 14Sep03
{
	...
	if (!(useFirst && useLast) && !(useFirst || useLast))
			return;

	std::vector<MPRVtxTexClr_t> vDummyVector; //RERUN
	if(useFirst)
	{
		MPRVtxTexClr_t *arr[] = { v0, v1, v2, v3 };
		//RERUN context.DrawPrimitive(MPR_PRM_TRIFAN,MPR_VI_COLOR|MPR_VI_TEXTURE,count,arr,terrain); //JAM 14Sep03
		vDummyVector.push_back(*arr[0]);
		vDummyVector.push_back(*arr[1]);
		vDummyVector.push_back(*arr[2]);
		vDummyVector.push_back(*arr[3]);
		context.DrawPrimitive(MPR_PRM_TRIFAN, MPR_VI_COLOR | MPR_VI_TEXTURE, vDummyVector, terrain);
	}
	else 
	{
...
void Render3D::DrawTriangle(ThreeDVertex* v0, ThreeDVertex* v1, ThreeDVertex* v2, int CullFlag, bool gifPicture, bool terrain ) //JAM 14Sep03
{
	...
	vDummyVector.push_back(*arr[1]);
	vDummyVector.push_back(*arr[2]);
	//RERUN context.DrawPrimitive(MPR_PRM_TRIFAN,MPR_VI_COLOR|MPR_VI_TEXTURE,3,arr,terrain); //JAM 14Sep03
	context.DrawPrimitive(MPR_PRM_TRIFAN, MPR_VI_COLOR | MPR_VI_TEXTURE, vDummyVector, terrain);
}
