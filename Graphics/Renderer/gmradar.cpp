...
void RenderGMRadar::DrawFeatures( void )
{
	...
	dmatrix.rotation00 = ScaledSIN*dScaleX;	dmatrix.rotation01 =  ScaledCOS*dScaleX;
	dmatrix.rotation10 = ScaledCOS*dScaleY;	dmatrix.rotation11 = -ScaledSIN*dScaleY;
...
void RenderGMRadar::PrepareToDrawTargets( void )
{
	...
	down.M11 = 0.0f;	down.M12 = right.x;		down.M13 = up.x;
	down.M21 = 0.0f;	down.M22 = right.y;		down.M23 = up.y;
	down.M31 = 1.0f;	down.M32 = right.z;		down.M33 = up.z;
...
#else
		//RERUN context.DrawPrimitive(MPR_PRM_TRIFAN, MPR_VI_COLOR, 4, (MPRVtxTexClr_t **) &v0);
		//RERUN try them one by one.
		MPRVtxTexClr_t vDummy[4];
		// Convert the structure format
		vDummy[0].a = v0->a, vDummy[0].b = v0->b, vDummy[0].g = v0->g, vDummy[0].r = v0->r, vDummy[0].x = v0->x, vDummy[0].y = v0->y;
		vDummy[1].a = v1->a, vDummy[1].b = v1->b, vDummy[1].g = v1->g, vDummy[1].r = v1->r, vDummy[1].x = v1->x, vDummy[1].y = v1->y;
		vDummy[2].a = v2->a, vDummy[2].b = v2->b, vDummy[2].g = v2->g, vDummy[2].r = v2->r, vDummy[2].x = v2->x, vDummy[2].y = v2->y;
		vDummy[3].a = v3->a, vDummy[3].b = v3->b, vDummy[3].g = v3->g, vDummy[3].r = v3->r, vDummy[3].x = v3->x, vDummy[3].y = v3->y;
		
		std::vector<MPRVtxTexClr_t> vDummyVector;
		vDummyVector.push_back(vDummy[0]);
		vDummyVector.push_back(vDummy[1]);
		vDummyVector.push_back(vDummy[2]);
		vDummyVector.push_back(vDummy[3]);
		context.DrawPrimitive(MPR_PRM_TRIFAN, MPR_VI_COLOR, vDummyVector);
#endif
	}
}
