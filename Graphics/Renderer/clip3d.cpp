...
#else
	//RERUN context.DrawPrimitive(	MPR_PRM_TRIFAN, MPR_VI_COLOR | MPR_VI_TEXTURE,(unsigned short)(nextOut - outList),(MPRVtxTexClr_t **)outList,terrain); //JAM 14Sep03

	std::vector<MPRVtxTexClr_t> vDummyVector;
	for (unsigned int arrayItems = 0; arrayItems < (unsigned int)(nextOut - outList); arrayItems++)
	{
		vDummyVector.push_back(*outList[arrayItems]);
	}
