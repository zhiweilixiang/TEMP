...
struct	OpRecord	{ enum OpName	operation;	int percent; };
...
//		m_pRenderTarget->AttachSurfaces(output->GetDisplayDevice(), output->targetSurface(), NULL);
		...
		// and now we are using a back door to make it actually use the primary surface (HACK)
...
void RenderGMComposite::DrawComposite( Tpoint *center, float platformHdg )
{
	...
	// was rendererd.
	x = -1.0f;						y = -GM_OVERSCAN_H;
	v0.x = x*cosRot-y*sinRot+dx;	v0.y = x*sinRot+y*cosRot+dy;	v0.u = 0.0f;	v0.v = 1.0f;	// Lower left

	x = -1.0f;						y =  GM_OVERSCAN_H;
	v1.x = x*cosRot-y*sinRot+dx;	v1.y = x*sinRot+y*cosRot+dy;	v1.u = 1.0f;	v1.v = 1.0f;	// Lower right

	x =  GM_OVERSCAN_V;				y =  GM_OVERSCAN_H;
	v2.x = x*cosRot-y*sinRot+dx;	v2.y = x*sinRot+y*cosRot+dy;	v2.u = 1.0f;	v2.v = 0.0f;	// Upper right

	x =  GM_OVERSCAN_V;				y = -GM_OVERSCAN_H;
	v3.x = x*cosRot-y*sinRot+dx;	v3.y = x*sinRot+y*cosRot+dy;	v3.u = 0.0f;	v3.v = 0.0f;	// Upper left
...
bool RenderGMComposite::BackgroundGeneration( Tpoint *from, Tpoint *at, float platformHdg, int beamPercent, BOOL movingRight, bool Shaped )
{
	...
	if ((prevBeamRight != movingRight) /*|| (beamPercent < prevBeamPercent)*/ || DrawChanged) {
...
//	nTexHandle->PreLoad();	// it will be used soon
	...
	pVtx[0].x = 0.0f; pVtx[0].y = 0.0f; pVtx[0].u = fUStart; pVtx[0].v = 0.0f;
	pVtx[0].r = pVtx[0].g = pVtx[0].b = pVtx[0].a = Alpha;

	pVtx[1].x = GM_TEXTURE_SIZE; pVtx[1].y = 0.0f; pVtx[1].u = fUStop; pVtx[1].v = 0.0f;
	pVtx[1].r = pVtx[1].g = pVtx[1].b = pVtx[1].a = Alpha;

	pVtx[2].x = GM_TEXTURE_SIZE; pVtx[2].y = GM_TEXTURE_SIZE; pVtx[2].u = fUStop; pVtx[2].v = 1.0f;
	pVtx[2].r = pVtx[2].g = pVtx[2].b = pVtx[2].a = Alpha;

	pVtx[3].x = 0.0f; pVtx[3].y = GM_TEXTURE_SIZE; pVtx[3].u = fUStart; pVtx[3].v = 1.0f;
...
void RenderGMComposite::DebugDrawLeftTexture( Render2D *renderer )
{
	...
	v0.x = 1.0f;			v0.y = 1.0f;				v0.u = 0.0f;	v0.v = 0.0f;	// UL
	v1.x = GM_TEXTURE_SIZE;	v1.y = 1.0f;				v1.u = 1.0f;	v1.v = 0.0f;	// UR
	v2.x = GM_TEXTURE_SIZE;	v2.y = GM_TEXTURE_SIZE;		v2.u = 1.0f;	v2.v = 1.0f;	// BR
	v3.x = 1.0f;			v3.y = GM_TEXTURE_SIZE;		v3.u = 0.0f;	v3.v = 1.0f;	// BL

	vertArray[num++] = &v0;	SetClipFlags( &v0 );
	vertArray[num++] = &v1;	SetClipFlags( &v1 );
	vertArray[num++] = &v2;	SetClipFlags( &v2 );
	vertArray[num++] = &v3;	SetClipFlags( &v3 );
...
void RenderGMComposite::DebugDrawRadarImage( ImageBuffer *target )
{
	...
	v0.b = 0.0f;	v0.g = 0.0f;	v0.r = 1.0f;
	v1.b = 0.0f;	v1.g = 1.0f;	v1.r = 0.0f;
	v2.b = 1.0f;	v2.g = 0.0f;	v2.r = 0.0f;
	v3.b = 0.0f;	v3.g = 1.0f;	v3.r = 1.0f;
	c[0].b = 1.0f;	c[0].g = 1.0f;	c[0].r = 1.0f;
	c[1].b = 1.0f;	c[1].g = 1.0f;	c[1].r = 1.0f;
}