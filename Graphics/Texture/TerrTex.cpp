...
void TextureDB::Cleanup(void)
{
    ...
    //RERUN check this ShiAssert(IsReady());
    //RERUN check ShiAssert(TextureSets);
...
void TextureDB::Load(SetEntry* pSet, TileEntry* pTile, int res, bool forceNoDDS)
{
    ...
    else
    {
        ...
        if (res == 1)
            filename[strlen(texturePath)] = 'H'; //RERUN was M
        else if (res == 0)
            filename[strlen(texturePath)] = 'H'; // RERUN was L
...
bool TextureDB::DumpImageToFile(TileEntry* pTile, DWORD *palette, int res, bool bForce)
{
	...
	if (res == 1)
		szFileName[strlen(texturePathD)] = 'H'; //RERUN was M
    else if (res == 0)
		szFileName[strlen(texturePathD)] = 'H'; // RERUN was L
