...
void FarTexDB::Cleanup(void)
{
	...
	if (texArray != NULL)
	{
		for (i = 0; (texCount > 0) && (i < ((DWORD)texCount)); ++i)
		{
			Deactivate(i);
			Free(i);
		}
	...
	ShiAssert(LoadedTextureCount == 0); //RERUN observed between 1 and 4
...
void FarTexDB::Request(TextureID texID)
{
	...
	if (!texArray)
	{
		LeaveCriticalSection(&cs_textureList); //RERUN
		return;
	}
...
void FarTexDB::Release(TextureID texID)
{
	...
	ShiAssert(texID >= (TextureID) 0);
	ShiAssert(texID < (TextureID) texCount);
